<?php
    ...
    $objRs->queryopen("SELECT U_AWARDENCODER,U_AWARDAPPROVER FROM USERS WHERE USERID='".$_SESSION["userid"]."'");
    if ($objRs->queryfetchrow("NAME")) {
            $page->privatedata["encoder"] = $objRs->fields["U_AWARDENCODER"];
            $page->privatedata["approver"] = $objRs->fields["U_AWARDAPPROVER"];
    }
...
//            $page->popup->additem("popupCopyDocumentFrom","Canvassing","CopyFromCanvassingGPSLGUPurchasing()");
            $page->popup->additem("popupCopyDocumentFrom","Philgeps Publication","CopyFromPhilGepsGPSLGUPurchasing()");
    }
	...
	if ($page->getitemstring("docstatus")!="D") {
		...
		$page->businessobject->items->seteditable("u_date",false);
		$page->businessobject->items->seteditable("u_forrecordingonly",false);
