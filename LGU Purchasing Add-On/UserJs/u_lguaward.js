                ...
                default:
                    switch (column) {
                         case "u_profitcentername":
                                if (getInput(column)!="") {
                                        result = page.executeFormattedQuery("select profitcenter, profitcentername from profitcenters where profitcentername = '"+getInput(column)+"'");	
                                        if (result.getAttribute("result")!= "-1") {
                                                if (parseInt(result.getAttribute("result"))>0) {
                                                        ...
                                                        page.statusbar.showError("Invalid Profit Center Name.");	
                                                        return false;
                                                }
                                        } else {
                                                setInput("u_profitcenter","");
					...
					if (getInput(column)!="") {
						...
						result = page.executeFormattedQuery("select docno,u_dueamount from u_lguobligationslips where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and docstatus='O' and docno = '"+getInput(column)+"'");	
						if (result.getAttribute("result")!= "-1") {
							if (parseInt(result.getAttribute("result"))>0) {
								setInput("u_obrno",result.childNodes.item(0).getAttribute("docno"));
								setInputAmount("u_obramount",result.childNodes.item(0).getAttribute("u_dueamount"));
							} else {
								setInput("u_obrno","");
								setInputAmount("u_obramount",0);
...
function onElementCFLGetParamsLGUPurchasing(Id,params) {
    switch (Id) {
			...
			params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("20`50`18`18`18`18")); 			
		...
		case "df_u_profitcentername":
			...
			params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("Description`Profit Center")); 			
...
function computeTotalGPSLGUPurchasing() {
	var rc = 0, totalamountitems = 0,totalamountservice = 0,totalcontractamt = 0, rc1 = 0;
	
	rc =  getTableRowCount("T1");
	for (i = 1; i <= rc; i++) {
		if (isTableRowDeleted("T1",i)==false) {
			totalamountitems += getTableInputNumeric("T1","u_cost",i);
			totalcontractamt += getTableInputNumeric("T1","u_contractamt",i);
		}
	}
...
//	setInputAmount("u_totalcontractamt",totalcontractamt );
	setInputAmount("u_totalcontractamt",totalamountitems + totalamountservice );
}
...
function CopyFromPhilGepsGPSLGUPurchasing() {
            OpenPopup(1040,390,"./udp.php?&objectcode=u_copyfromphilgeps&df_doctype="+getInput("u_doctype")+"","Copy From Philgeps Publication");
}
function CopyFromCanvassingGPSLGUPurchasing() {
            OpenPopup(1040,390,"./udp.php?&objectcode=u_copyfromcanvassing&df_doctype="+getInput("u_doctype")+"","Copy From Canvassing");
}
