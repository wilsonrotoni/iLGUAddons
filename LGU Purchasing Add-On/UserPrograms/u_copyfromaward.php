...
//    $page->events->add->afterDefault("onAfterDefault");
    ...
    $page->objectcode = "u_copyfromaward";
    $page->paging->formid = "./UDP.php?&objectcode=u_copyfromaward";
    $page->formid = "./UDO.php?&objectcode=u_lgureceipts";
    $page->objectname = "Copy From Notice of Award";
...
//    $objGridDetail->width = 760;
    ...
    if ($page->getvarstring("df_doctype")=="I") {
            $objrs->queryopenext("SELECT a.branch,a.docno,a.u_date,CONCAT('Base On: ',a.docno) as remarks,a.u_contractexpdate as u_duedate,a.u_profitcentername,a.u_profitcenter FROM u_lguaward a INNER JOIN u_lguawarditems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='$company' AND A.DOCSTATUS  = 'O'  $filterExp  GROUP BY a.branch,a.docno", $objGridDetailDocno->sortby,"",$objGridDetailDocno->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
    } else {
            $objrs->queryopenext("SELECT a.branch,a.docno,a.u_date,CONCAT('Base On: ',a.docno) as remarks,a.u_contractexpdate as u_duedate,a.u_profitcentername,a.u_profitcenter FROM u_lguaward a INNER JOIN u_lguawardservice b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='$company' AND A.DOCSTATUS  = 'O'  $filterExp  GROUP BY a.branch,a.docno", $objGridDetailDocno->sortby,"",$objGridDetailDocno->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
    }
   ...
   function onFormSubmitReturn(action,success,error) {
	...
	for (i = 1; i <= rc; i++) {
		if(getTableInput("T20","rowstat",i) != "X") {
			if(getTableInput("T20","checked",i)=="1") {
			result = page.executeFormattedQuery("SELECT a.branch,b.u_itemcode,b.u_itemdesc,b.u_itemsubgroup,'' as u_glacctno,b.u_unitissue,b.u_openquantity,a.docno,b.lineid,b.docid,'LGU_Award'as objectcode,a.u_remarks,b.u_unitcost,b.u_cost  FROM u_lguaward a INNER JOIN u_lguawarditems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='" + getGlobal("company") + "' AND b.u_openquantity != 0 AND a.docno = '"+getTableInput("T20","docno",i)+"' union all SELECT a.branch,'' as u_itemcode,b.u_itemdesc,'' as u_itemsubgroup,b.u_glacctno,b.u_unitissue,b.u_openquantity,a.docno,b.lineid,b.docid,'LGU_Purchase Request'as objectcode,a.u_remarks,b.u_unitcost,b.u_cost  FROM u_lguaward a INNER JOIN u_lguawardservice b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='" + getGlobal("company") + "' AND b.u_openquantity != 0 AND a.docno = '"+getTableInput("T20","docno",i)+"' ");	 			
                            ...
                            for (var iii = 1; iii <= getTableRowCount("T20"); iii++) {
                                    if(getTableInput("T20","rowstat",iii) != "X") {
                                            if(getTableInput("T20","checked",iii)==1) {
                                                result = page.executeFormattedQuery("SELECT u_projcode, u_bpcode, u_bpname, u_profitcenter,u_profitcentername,u_procmode  FROM u_lguaward WHERE company='" + getGlobal("company") + "' and docno = '"+getTableInput("T20","docno",iii)+"'");	 			
                                                if (result.getAttribute("result")!= "-1") {
                                                    if (parseInt(result.getAttribute("result"))>0) {
                                                ...
                                                remarks += "Base on Notice of Award " + getTableInput("T20","docno",iii) + ". ";
                                            }
                                    }
                            }
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;Notice of Award List&nbsp;</td>
