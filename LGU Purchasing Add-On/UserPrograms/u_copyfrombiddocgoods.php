...
//    $page->events->add->afterDefault("onAfterDefault");
    ...
    $page->objectcode = "u_copyfrombiddocgoods";
    $page->paging->formid = "./UDP.php?&objectcode=u_copyfrombiddocgoods";
    $page->formid = "./UDO.php?&objectcode=u_lgubiddocgoods";
    $page->objectname = "Copy From Biddocs Goods";
    ...
    $objGridDetailDocno->addcolumn("u_poprofitcenter");
    ...
    $objGridDetailDocno->columntitle("u_poprofitcenter","Order To");
    ...
    $objGridDetailDocno->columnwidth("u_poprofitcenter",15);
...
//    $objGridDetail->width = 760;
    ...
    if ($page->getvarstring("df_doctype")=="I") {
            $objrs->queryopenext("SELECT a.branch,a.docno,a.u_date,CONCAT('Base On: ',a.docno) as remarks,a.u_duedate,a.u_profitcentername,a.u_poprofitcenter FROM u_lgupurchaserequests a INNER JOIN u_lgupurchaserequestitems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='$company' AND A.DOCSTATUS  = 'O'  $filterExp  GROUP BY a.branch,a.docno", $objGridDetailDocno->sortby,"",$objGridDetailDocno->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
    } else {
            $objrs->queryopenext("SELECT a.branch,a.docno,a.u_date,CONCAT('Base On: ',a.docno) as remarks,a.u_duedate,a.u_profitcentername,a.u_poprofitcenter FROM u_lgupurchaserequests a INNER JOIN u_lgupurchaserequestservice b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='$company' AND A.DOCSTATUS  = 'O'  $filterExp  GROUP BY a.branch,a.docno", $objGridDetailDocno->sortby,"",$objGridDetailDocno->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
    }
        ...
        while ($objrs->queryfetchrow("NAME")) {
            ...
            $objGridDetailDocno->setitem(null,"u_poprofitcenter",$objrs->fields["u_poprofitcenter"]);
   ...
   function onFormSubmitReturn(action,success,error) {
	...
	for (i = 1; i <= rc; i++) {
		if(getTableInput("T20","rowstat",i) != "X") {
			if(getTableInput("T20","checked",i)=="1") {
			result = page.executeFormattedQuery("SELECT a.branch,b.u_itemcode,b.u_itemdesc,'' as u_glacctno,b.u_unitissue,b.u_openquantity,a.docno,b.lineid,b.docid,'LGU_Purchase Request'as objectcode,a.u_remarks,b.u_unitcost,b.u_cost,b.u_remarks as u_detailremarks  FROM u_lgupurchaserequests a INNER JOIN u_lgupurchaserequestitems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='" + getGlobal("company") + "' AND b.u_openquantity != 0 AND a.docno = '"+getTableInput("T20","docno",i)+"' union all SELECT a.branch,'' as u_itemcode,b.u_itemdesc,b.u_glacctno,b.u_unitissue,b.u_openquantity,a.docno,b.lineid,b.docid,'LGU_Purchase Request'as objectcode,a.u_remarks,b.u_unitcost,b.u_cost,b.u_remarks as u_detailremarks  FROM u_lgupurchaserequests a INNER JOIN u_lgupurchaserequestservice b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='" + getGlobal("company") + "' AND b.u_openquantity != 0 AND a.docno = '"+getTableInput("T20","docno",i)+"' ");	 			
                            ...
                            for (var iii = 1; iii <= getTableRowCount("T20"); iii++) {
                                    if(getTableInput("T20","rowstat",iii) != "X") {
                                            if(getTableInput("T20","checked",iii)==1) {
                                                result = page.executeFormattedQuery("SELECT u_profitcenter,u_profitcentername,u_procmode,u_projcode,u_projname  FROM u_lgupurchaserequests WHERE company='" + getGlobal("company") + "' and docno = '"+getTableInput("T20","docno",iii)+"'");	 			
                                                if (result.getAttribute("result")!= "-1") {
                                                    if (parseInt(result.getAttribute("result"))>0) {
                                                            for (iiii = 0; iiii < result.childNodes.length; iiii++) {
                                                                if (window.opener.getVar("objectcode")=="U_LGUPHILGEPS") {
                                                                    window.opener.setInput("u_projcode",result.childNodes.item(iiii).getAttribute("u_projcode"));
                                                                    window.opener.setInput("u_projname",result.childNodes.item(iiii).getAttribute("u_projname"));
                                                                }
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;Biddocs Goods List&nbsp;</td>
