<?php
    ...
    $page->objectcode = "u_copyfromgrpo";
    $page->paging->formid = "./UDP.php?&objectcode=u_copyfromgrpo";
    $page->formid = "./UDO.php?&objectcode=u_lgureceipts";
    $page->objectname = "Copy From Goods Receipts";
   ...
   function onFormSubmitReturn(action,success,error) {
	...
	for (i = 1; i <= rc; i++) {
		if(getTableInput("T20","rowstat",i) != "X") {
			if(getTableInput("T20","checked",i)=="1") {
			result = page.executeFormattedQuery("SELECT a.branch,b.u_itemcode,b.u_itemdesc,b.u_itemsubgroup,b.u_openquantity,a.docno,b.lineid,b.docid,'LGU_Purchase Request'as objectcode,a.u_remarks FROM u_lgupurchaseorder a INNER JOIN u_lgupurchaseorderitems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='" + getGlobal("company") + "' AND b.u_openquantity != 0 AND a.docno = '"+getTableInput("T20","docno",i)+"'");	 			
			if (result.getAttribute("result")!= "-1") {
					if (parseInt(result.getAttribute("result"))>0) {
						for (iii = 0; iii < result.childNodes.length; iii++) {
							...
							data["unitprice"] = formatNumericPrice(0.00,"amount");
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;Goods Receipts List&nbsp;</td>
