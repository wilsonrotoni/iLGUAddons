<?php
    ...
    $page->objectcode = "u_copyfromsplitpo";
    $page->paging->formid = "./UDP.php?&objectcode=u_copyfromsplitpo";
    $page->formid = "./UDO.php?&objectcode=u_lgureceipts";
    $page->objectname = "Copy From Progress Billing";
...
//    $objGridDetail->width = 760;
    ...
    $objrs->queryopenext("SELECT a.branch,a.docno,a.u_date,CONCAT('Base On: ',a.docno) as remarks,a.u_duedate,a.u_profitcentername FROM u_lgusplitpo a INNER JOIN u_lgusplitpoitems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='$company' AND A.DOCSTATUS  = 'O'  $filterExp  GROUP BY a.branch,a.docno", $objGridDetailDocno->sortby,"",$objGridDetailDocno->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
   ...
   function onFormSubmitReturn(action,success,error) {
	...
	for (i = 1; i <= rc; i++) {
		if(getTableInput("T20","rowstat",i) != "X") {
			if(getTableInput("T20","checked",i)=="1") {
			result = page.executeFormattedQuery("SELECT a.branch,b.u_itemcode,b.u_itemdesc,b.u_itemsubgroup,b.u_openquantity,a.docno,b.lineid,b.docid,'LGU_Progress Billing'as objectcode,a.u_remarks,b.u_unitcost,b.u_cost,b.u_remarks as u_detailremarks FROM u_lgusplitpo a INNER JOIN u_lgusplitpoitems b ON b.company = a.company AND b.branch = a.branch AND b.docid = a.docid WHERE a.company='" + getGlobal("company") + "' AND b.u_openquantity != 0 AND a.docno = '"+getTableInput("T20","docno",i)+"'");	 			
			if (result.getAttribute("result")!= "-1") {
					if (parseInt(result.getAttribute("result"))>0) {
						for (iii = 0; iii < result.childNodes.length; iii++) {
							...
							data["unitprice"] = formatNumericAmount(result.childNodes.item(iii).getAttribute("u_unitcost"));
                            ...
                            for (var i = 1; i <= getTableRowCount("T10"); i++) {
                                    if(getTableInput("T10","rowstat",i) != "X") {
                                            if(getTableInput("T10","checked",i)=="Y") {
                                                    ...
                                                    data["u_unitcost"] = formatNumericPrice(getTableInputNumeric("T10","unitprice",i));
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;Progress Billing List&nbsp;</td>
