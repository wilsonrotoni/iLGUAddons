<?php
	...
	require_once("./series.php");
 	require_once("../Addons/GPS/LGU Queueing Tablet Add-On/UserProcs/utils/u_queueingprinter.php"); 
	...
	if(isset($_GET['group'])){
		...
		if ($obju_LGUQueGroups->getbykey($_GET['group'])) {
			...
			if ($tagno>0 && $actionReturn) {
				if (!printQueueNo($obju_LGUQueGroups->name,$tagno,$_GET['mobileno'])) {
					$msg = $_SESSION["errormessage"];
				} else $objConnection->commit();
			} else {
				$msg = $_SESSION["errormessage"];
