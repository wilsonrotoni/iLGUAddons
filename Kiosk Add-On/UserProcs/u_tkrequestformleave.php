	...
	$objRs->queryopen("SELECT a.u_branch FROM u_premploymentdeployment a WHERE a.code = '".$_SESSION["userid"]."' AND a.u_stat = 'Main'");
		if ($objRs->recordcount() > 0) {
			while ($objRs->queryfetchrow()) {
				...
				$page->setitem("u_leavestatus","Successful");
				$page->setitem("name","Request By ".$_SESSION["username"]);
...
function onAfterEditGPSKiosk() { 
	...
	$page->setitem("name","Request By ".$_SESSION["username"]);
	$page->businessobject->items->seteditable("u_leavefrom",false);
	$page->businessobject->items->seteditable("u_leaveto",false);
...
function onDrawGridColumnLabelGPSKiosk($tablename,$column,$row,&$label) {
	...
	switch ($tablename) {
		case "T1":
			if ($column=="u_date_filter") {
				$objRs->queryopen("SELECT DATE_FORMAT('".formatDateToDB($label)."', '%W %M %d, %Y') as name");
...
//$page->businessobject->items->seteditable("u_leavedays",false);
...
$page->businessobject->items->seteditable("u_leavebal",false);
$page->businessobject->items->seteditable("u_leavedays",false);
$page->businessobject->items->seteditable("u_appdate",false);

$page->businessobject->items->setcfl("u_appdate","Calendar");
$page->businessobject->items->setcfl("u_leavefrom","Calendar");
$page->businessobject->items->setcfl("u_leaveto","Calendar");
...
$objGrids[0]->columnwidth("u_date_filter",20);
$objGrids[0]->columnwidth("u_scheduleassign",25);
$objGrids[0]->columnwidth("u_leavedays",12);
$objGrids[0]->columnwidth("u_am",5);
$objGrids[0]->columnwidth("u_lv",5);
$objGrids[0]->columnwidth("u_pm",5);
$objGrids[0]->columninput("u_pm","type","checkbox");
$objGrids[0]->columninput("u_pm","value",1);
$objGrids[0]->columninput("u_lv","type","checkbox");
$objGrids[0]->columninput("u_lv","value",1);
$objGrids[0]->columninput("u_am","type","checkbox");
$objGrids[0]->columninput("u_am","value",1);
$objGrids[0]->columninput("u_scheduleassign","type","text");
$objGrids[0]->columncfl("u_scheduleassign","OpenCFLfs()");
