	...
	$objRs->queryopen("SELECT a.u_branch FROM u_premploymentdeployment a WHERE a.code = '".$_SESSION["userid"]."' AND a.u_stat = 'Main'");
		if ($objRs->recordcount() > 0) {
			while ($objRs->queryfetchrow()) {
				...
				$page->setitem("name","Request Off-set - ".$_SESSION["username"]);
				$page->setitem("code", getNextSeriesNoByBranch($page->objectcode,"PRIMARY",formatDateToDB(currentdate()),$objConnection,false));
				$page->setitem("u_tk_wd",0);
			}
		}
...
function onDrawGridColumnLabelGPSKiosk($tablename,$column,$row,&$label) {
	...
	switch ($tablename) {
		case "T1":
			if ($column=="u_type_filter") {
				$objRs->queryopen("SELECT fieldvaluedesc as name FROM udfvalues WHERE tablename = 'u_tkrequestformoffsetlist' AND fieldname = 'type_filter' and fieldvalue='$label'");
				if ($objRs->queryfetchrow("NAME"))	$label = $objRs->fields["name"];
			} else if ($column=="u_type_request") {
				$objRs->queryopen("SELECT fieldvaluedesc as name FROM udfvalues WHERE tablename = 'u_tkrequestformoffsetlist' AND fieldname = 'type_request' and fieldvalue='$label'");
				if ($objRs->queryfetchrow("NAME"))	$label = $objRs->fields["name"];
			}
			break;
		default:
...
$page->businessobject->items->seteditable("u_tk_wd",false);
$page->businessobject->items->seteditable("u_tk_ot",false);
$page->businessobject->items->seteditable("u_appdate",false);

$page->businessobject->items->setcfl("u_appdate","Calendar");
//$page->businessobject->items->setcfl("u_tkdate","Calendar");
...
$objGrids[0]->columncfl("u_date_filter","Calendar");
$objGrids[0]->columnwidth("u_hrs",15);
$objGrids[0]->columnwidth("u_type_filter",15);
$objGrids[0]->columnwidth("u_date_filter",12);
$objGrids[0]->columnwidth("u_type_request",15);
