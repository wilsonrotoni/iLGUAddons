...
function onAfterDefaultGPSKiosk() { 
	...
	$objRs = new recordset(NULL,$objConnection);
	$objRs->queryopen("SELECT b.u_branch,a.u_whpd FROM u_premploymentinfo a LEFT OUTER JOIN u_premploymentdeployment b ON b.company = a.company AND b.branch = a.branch AND b.code = a.code WHERE a.code = '".$_SESSION["userid"]."' AND b.u_stat = 'Main'");
		if ($objRs->recordcount() > 0) {
			while ($objRs->queryfetchrow()) {
				...
				$page->setitem("u_tk_wd",formatNumericAmount($objRs->fields[1]));
...
$page->businessobject->items->seteditable("u_tot_hrs",false);
...
$page->businessobject->items->setcfl("u_tkdate1","Calendar");
