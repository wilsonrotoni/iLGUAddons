	...
	$objRs->queryopen("SELECT a.u_branch FROM u_premploymentdeployment a WHERE a.code = '".$_SESSION["userid"]."' AND a.u_stat = 'Main'");
		if ($objRs->recordcount() > 0) {
			while ($objRs->queryfetchrow()) {
				...
				$page->setitem("name","Request Official Business - ".$_SESSION["username"]);
				$page->setitem("code", getNextSeriesNoByBranch($page->objectcode,"PRIMARY",formatDateToDB(currentdate()),$objConnection,false));
			}
		}
	...
	switch ($tablename) {
		case "T1":
			if ($column=="u_type_filter") {
				$objRs->queryopen("SELECT fieldvaluedesc as name FROM udfvalues WHERE tablename = 'u_tkrequestformoballlist' AND fieldname = 'type_filter' and fieldvalue='$label'");
...
$schema["ob_type"] = createSchemaUpper("ob_type");
$schema["ob_time1"] = createSchemaTime("ob_time1");
$schema["ob_time2"] = createSchemaTime("ob_time2");
$schema["ob_hrs"] = createSchemaAmount("ob_hrs");
$schema["ob_place"] = createSchema("ob_place");

$objGrids[0]->columncfl("u_date_filter","Calendar");
$objGrids[0]->columnwidth("u_date_filter",12);
$objGrids[0]->columnwidth("u_type_filter",12);
$objGrids[0]->columnwidth("u_obfromtime",10);
$objGrids[0]->columnwidth("u_obtotime",10);
$objGrids[0]->columnwidth("u_ob_tot_hrs",12);
$objGrids[0]->columnwidth("u_ob_wd",12);
$objGrids[0]->columnwidth("u_ob_ot",12);
