...
function onAfterDefaultGPSKiosk() {
	...
	$objRs = new recordset(NULL,$objConnection);
	$objRs->queryopen("SELECT a.u_branch,IFNULL(b.u_otdefault,'') FROM u_premploymentdeployment a INNER JOIN u_prglobalsetting b ON b.company = a.company AND b.branch = a.branch WHERE a.code = '".$_SESSION["userid"]."' AND a.u_stat = 'Main'");
		if ($objRs->recordcount() > 0) {
			while ($objRs->queryfetchrow()) {
				...
				$page->setitem("name","Request Over Time - ".$_SESSION["username"]);
				if($objRs->fields[1] != "") {
					$page->businessobject->items->seteditable("u_otfromtime",false);
					$page->setitem("u_otfromtime",formatTimeToHttp($objRs->fields[1]));
				}
				//$page->setitem("code", getNextSeriesNoByBranch($page->objectcode,"PRIMARY",formatDateToDB(currentdate()),$objConnection,false));
			}
		}
...
$page->businessobject->items->seteditable("u_othrs",false);
$page->businessobject->items->seteditable("u_otmins",false);
$page->businessobject->items->seteditable("u_othours",false);
$page->businessobject->items->seteditable("u_appdate",false);

$page->businessobject->items->setcfl("u_appdate","Calendar");
$page->businessobject->items->setcfl("u_otdate","Calendar");
