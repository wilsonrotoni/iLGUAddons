		...
		if(strtolower($action)=="u_update") {
			...
			for($i=1;$i<= $objGrid->recordcount;$i++) {
				if($page->getcolumnstring("checkedT1r".$i) == "Y"){
						if ($obju_REQUEST->getbysql("CODE='".$page->getcolumnstring("requestnoT1r".$i)."'")) {
								...
								if ($obju_History->getbysql("U_REQUESTNO='".$page->getcolumnstring("requestnoT1r".$i)."' AND CREATEDBY = '".$_SESSION["userid"]."'")) {
								} else {
								...
								if ($objRsGlobalSetting->recordcount() >= 1) {
									...
									$objRsFromNext->queryopen("SELECT b.u_email FROM u_tkapproverfileassigned a INNER JOIN u_hremploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_stagename WHERE a.code = '".$page->getcolumnstring("approvercodeT1r".$i)."' AND a.u_stageno = '".$page->getcolumnstring("nextstagenoT1r".$i)."'");
									if($objRsFromNext->queryfetchrow()) { $FromAddress = $objRsFromNext->fields[0]; }
									
									$objRsEmp->queryopen("SELECT CONCAT(LEFT(u_fname,1),'. ',u_lname) FROM u_premploymentinfo WHERE code = '".$page->getcolumnstring("empidT1r".$i)."'");
									...
									try {
										...
										if($page->getitemstring("status") == 'A') {
											$mail->Subject = "Notification: ".$EmpName." Request No. ".$page->getcolumnstring("requestnoT1r".$i)." is Approved";
										} else if($page->getitemstring("status") == 'D') {
											$mail->Subject = "Notification: ".$EmpName." Request No. ".$page->getcolumnstring("requestnoT1r".$i)." is Rejected";
										}
