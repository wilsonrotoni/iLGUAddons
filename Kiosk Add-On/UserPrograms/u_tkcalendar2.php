<?php
	...
	*/
	function onDrawCalendarFilter() {
		...
		echo "<select id=\"empid\" name=\"empid\">\n";
	...
	function onGetCalendarEvents($month, $year) {
		...
		$objRs->queryopen("SELECT x.u_days,x.code,x.time1,x.date_now,x.u_empid,x.status_now,x.timeto,x.timefrom,x.sort
							FROM (SELECT *
								...
								UNION ALL
									SELECT DAY(a.u_appdate) as u_days,
										 a.u_appdate as date_now,
										 'Note' as Request_type,
										 9 as xsort
									
									FROM u_tkrequestformnote a
									INNER JOIN u_premploymentinfo b ON b.company = a.company
									  AND b.branch = a.branch
									  AND b.code = a.u_empid
									LEFT JOIN (SELECT u_requestno,COUNT(code) as counters
								...
								UNION ALL
									SELECT DAY(a.u_appdate) as u_days,
										 a.u_appdate as date_now,
										 'OB-All' as Request_type,
										 10 as xsort
									
									FROM u_tkrequestformoball a
									INNER JOIN u_premploymentinfo b ON b.company = a.company
									  AND b.branch = a.branch
									  AND b.code = a.u_empid
									LEFT JOIN (SELECT u_requestno,COUNT(code) as counters
									...
									LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company
										...
										AND c1.u_stageno = IFNULL(x.counters,0)+1
									WHERE a.u_empid = '$empid'
										AND a.u_status != 1) as x 
									GROUP BY x.date_now,x.request_type) as rr
								UNION ALL
									SELECT *
								   FROM (SELECT xx.u_days,
												 xx.xsort as code,
												 'Request Done' as time1,
												 xx.date_now,
												 '$empid' as u_empid,
												 '' as status_now,
												 'Form' as timefrom,
												 xx.request_type as timeto,
												 xx.date_now as sort

									FROM (SELECT DAY(aa.u_date_filter) as u_days,
										 aa.u_date_filter as date_now,
										 'Leave' as Request_type,
										 2 as xsort
									
									FROM u_tkrequestformleave a
									INNER JOIN u_tkrequestformleavegrid aa ON aa.company = a.company
									...
									WHERE a.u_empid = '$empid'
									  ...
									  AND a.u_status = 1
								UNION ALL
									SELECT DAY(a.u_otdate) as u_days,
										 a.u_otdate as date_now,
										 'O.T.' as Request_type,
										 1 as xsort
									...
									WHERE a.u_empid = '$empid'
									  AND a.u_status = 1
								UNION ALL
									SELECT DAY(a.u_tkdate) as u_days,
										 a.u_tkdate as date_now,
										 'O.B' as Request_type,
										 5 as xsort
									...
									WHERE a.u_empid = '$empid'
									  AND a.u_status = 1
								UNION ALL
									SELECT DAY(a.u_loanfrom) as u_days,
										 a.u_loanfrom as date_now,
										 'Loan' as Request_type,
										 3 as sort
									...
									WHERE a.u_empid = '$empid'
									  AND a.u_status = 1
								UNION ALL
									SELECT DAY(aa.u_date) as u_days,
										 aa.u_date as date_now,
										 'Time Adj.' as Request_type,
										 4 as sort

									FROM u_tkrequestformtimeadjustment a
									INNER JOIN u_tkrequestformtimeadjustmentitems aa ON aa.company = a.company
									  AND aa.branch = a.branch
									  AND aa.code = a.code
									INNER JOIN u_premploymentinfo b ON b.company = a.company
									  AND b.branch = a.branch
									...
									WHERE a.u_empid = '$empid'
									  ...
									  AND a.u_status = 1
								UNION ALL
									SELECT DAY(aa.u_date_filter) as u_days,
										 aa.u_date_filter as date_now,
										 'Off-Set' as Request_type,
										 6 as xsort
									
									FROM u_tkrequestformoffset a
									INNER JOIN u_tkrequestformoffsetlist aa ON aa.company = a.company
									  AND aa.branch = a.branch
									  AND aa.code = a.code
									INNER JOIN u_premploymentinfo b ON b.company = a.company
									  AND b.branch = a.branch
									...
									WHERE a.u_empid = '$empid'
									  AND a.u_status = 1
								 UNION ALL
								    SELECT DAY(a.u_offdate) as u_days,
										 a.u_offdate as date_now,
										 'Off-Set Hrs' as Request_type,
										 7 as xsort
								...
								UNION ALL
									...
									FROM u_tkrequestformobset a
									INNER JOIN u_tkrequestformobsetlist aa ON aa.company = a.company
									  AND aa.branch = a.branch
									  AND aa.code = a.code
									INNER JOIN u_premploymentinfo b ON b.company = a.company
									  AND b.branch = a.branch
								...
								UNION ALL
									...
									FROM u_tkrequestformoball a
									INNER JOIN u_tkrequestformoballlist aa ON aa.company = a.company
									  AND aa.branch = a.branch
									  AND aa.code = a.code
									INNER JOIN u_premploymentinfo b ON b.company = a.company
									  AND b.branch = a.branch
									...
									INNER JOIN u_tkapproverfile c ON c.company = b.company
									  ...
									  AND c.code = b.u_employstatus2
									WHERE a.u_empid = '$empid'
									  AND a.u_status = 1) as xx
									GROUP BY xx.date_now,xx.request_type) as ss
								UNION ALL
									SELECT a.u_days,
										   a.u_scheduleassign as code,
										   if(c.u_other = 'R','Restday',CONCAT(t1.name,' - ',t2.name)) as time1,
										   CONCAT(a.u_year,'-',a.u_month,'-',IF(LENGTH(a.u_days) = 1,CONCAT('0',a.u_days),a.u_days)) as date_now,
										   a.u_empid,
										   'X' as status_now,
										   IFNULL(t1.name,'Rest') as timefrom,
										   if(c.u_other = 'R','Restday','Schedule') as timeto,
										   0 as sort
									...
									LEFT OUTER JOIN u_tktimemasterfiles t1 ON t1.company = b.company
									  ...
									  AND t1.code = LEFT(b.u_fromtime,5)
									LEFT OUTER JOIN u_tktimemasterfiles t2 ON t2.company = b.company
									  AND t2.branch = b.branch
									  AND t2.code = LEFT(b.u_totime,5)
									LEFT OUTER JOIN u_tkschedulelist c ON c.company = a.company
		...
		while ($objRs->queryfetchrow("NAME")) {
			...
			$objRsColor->queryopen("SELECT u_r_ot,
										   u_r_leave,
										   u_r_offset,
										   u_r_ob,
										   u_r_timeadj,
				...
				while ($objRsColor->queryfetchrow("NAME")) {
					if($objRs->fields["timeto"] == "Time Adj.") {
						if($objRs->fields["time1"] == "Request") {
							$eventdata[$day]["color"][] = $objRsColor->fields["u_r_timeadj"];
						} else if($objRs->fields["time1"] == "Request Done") {
			...
			$eventdata[$day]["onclick"][] = "OpenPopupu_TKOpenRequests('".$objRs->fields["timeto"]."','".$objRs->fields["code"]."','".$objRs->fields["sort"]."','".$objRs->fields["status_now"]."','".$objRs->fields["time1"]."')";
...
<script>
	var sf_keys;
	function OpenPopupu_TKOpenRequests(types,keys,sortz,codes,modes) {
		sf_keys = keys;
		if(types == "Time Adj.") {
			if(modes == "Request Done") {
				showRequestListsDone(types,sortz);
			} else {
	...
	function showRequestLists(type_request,daysdate){
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,a.u_loanfrom as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,IFNULL(cc.name,xx.u_remarks) as 'approver',CONCAT('Loan Dates : ',DATE_FORMAT(a.u_loanfrom,'%m/%d/%y'),' and ',DATE_FORMAT(a.u_loanto,'%m/%d/%y'), ' Reason : ',a.u_loanreason) as 'remarks' FROM u_tkrequestformloan a INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code LEFT JOIN (SELECT u_requestno,u_remarks FROM u_tkapproverhistory WHERE u_status = 'D' GROUP BY u_requestno) as xx ON xx.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND IF(a.u_status = 1,a.u_loanfrom,a.u_appdate) = '"+daysdate+"' AND a.u_status != 1 GROUP BY a.code,IF(a.u_status = 1,a.u_loanfrom,a.u_appdate)");
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,aa.u_date as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,IFNULL(cc.name,xx.u_remarks) as 'approver',CONCAT('Time Adjustment For Incorrect/Missing Time In/Out Reason : ',a.u_tareason) as 'remarks' FROM u_tkrequestformtimeadjustment a INNER JOIN u_tkrequestformtimeadjustmentitems aa ON aa.company = a.company AND aa.branch = a.branch AND aa.code = a.code INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code LEFT JOIN (SELECT u_requestno,u_remarks FROM u_tkapproverhistory WHERE u_status = 'D' GROUP BY u_requestno) as xx ON xx.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_tastatus = 'Successful' AND a.u_empid = '"+getGlobal("userid")+"' AND IF(a.u_status = 1,aa.u_date,a.u_appdate) = '"+daysdate+"' AND a.u_status != 1 GROUP BY a.code,IF(a.u_status = 1,aa.u_date,a.u_appdate)");
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,a.u_tkdate as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,IFNULL(cc.name,xx.u_remarks) as 'approver',CONCAT('Work Hrs :',ROUND(a.u_tk_wd,2),' O.B. Dates : ',DATE_FORMAT(a.u_tkdate,'%m/%d/%y'), ' Reason : ',a.u_assreason) as 'remarks' FROM u_tkrequestformass a INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code LEFT JOIN (SELECT u_requestno,u_remarks FROM u_tkapproverhistory WHERE u_status = 'D' GROUP BY u_requestno) as xx ON xx.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND IF(a.u_status = 1,a.u_tkdate,a.u_appdate) = '"+daysdate+"' AND a.u_status != 1 GROUP BY a.code,IF(a.u_status = 1,a.u_tkdate,a.u_appdate)");
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,a.u_offdate as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,IFNULL(cc.name,xx.u_remarks) as 'approver',CONCAT('Work Hrs :',ROUND(a.u_offhrs,2),' Off-set Dates : ',DATE_FORMAT(a.u_offdate,'%m/%d/%y'), ' Reason : ',a.u_offreason) as 'remarks' FROM u_tkrequestformoffsethrs a INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code LEFT JOIN (SELECT u_requestno,u_remarks FROM u_tkapproverhistory WHERE u_status = 'D' GROUP BY u_requestno) as xx ON xx.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND IF(a.u_status = 1,a.u_offdate,a.u_appdate) = '"+daysdate+"' AND a.u_status != 1  GROUP BY a.code,IF(a.u_status = 1,a.u_offdate,a.u_appdate)");
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,a.u_appdate as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus, IFNULL(cc.name,xx.u_remarks) as 'approver',CONCAT('Request Note :', a.u_note) as 'remarks' FROM u_tkrequestformnote a INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code LEFT JOIN (SELECT u_requestno,u_remarks FROM u_tkapproverhistory WHERE u_status = 'D' GROUP BY u_requestno) as xx ON xx.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND a.u_appdate = '"+daysdate+"' AND a.u_status != 1  GROUP BY a.code, a.u_appdate");
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,aa.u_date_filter as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,IFNULL(cc.name,xx.u_remarks) as 'approver',CONCAT('Total OB All Request :',COUNT(aa.code), ' Reason : ',a.u_assreason) as 'remarks' FROM u_tkrequestformoball a INNER JOIN u_tkrequestformoballlist aa ON aa.company = a.company AND aa.branch = a.branch AND aa.code = a.code INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code LEFT JOIN (SELECT u_requestno,u_remarks FROM u_tkapproverhistory WHERE u_status = 'D' GROUP BY u_requestno) as xx ON xx.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND IF(a.u_status = 1,aa.u_date_filter,a.u_appdate) = '"+daysdate+"' AND a.u_status != 1  GROUP BY a.code,IF(a.u_status = 1,aa.u_date_filter,a.u_appdate)");
...
function showRequestListsDone(type_request,daysdate){
		var result, data = new Array();
		clearTable("T10",true);
		showPopupFrame("popupFrameDetail");
		if (type_request == 'O.T.' ) {
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate, a.u_appdate as u_appdate, IF(a.u_status = 1,'Approved', IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,cc.name as 'approver',CONCAT('Request Note :',a.u_note) as 'remarks' FROM u_tkrequestformnote a INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND a.u_appdate = '"+daysdate+"' AND a.u_status = 1  GROUP BY a.code, a.u_appdate");
		...
		result = page.executeFormattedQuery("SELECT a.code,a.u_appdate as appdate,aa.u_date_filter as u_appdate,IF(a.u_status = 1,'Approved',IF(a.u_status = 2,'Denied',IF(a.u_status = -1,'Cancelled','Pending'))) as vstatus,cc.name as 'approver',CONCAT('Total OB All Request :',COUNT(aa.code), ' Reason : ',a.u_assreason) as 'remarks' FROM u_tkrequestformoball a INNER JOIN u_tkrequestformoballlist aa ON aa.company = a.company AND aa.branch = a.branch AND aa.code = a.code INNER JOIN u_premploymentinfo b ON b.company = a.company AND b.branch = a.branch AND b.code = a.u_empid LEFT JOIN (SELECT u_requestno,COUNT(code) as counters FROM u_tkapproverhistory GROUP BY u_requestno) as x ON x.u_requestno = a.code INNER JOIN u_tkapproverfile c ON c.company = b.company AND c.branch = b.branch AND c.code = b.u_employstatus2 LEFT JOIN u_tkapproverfileassigned c1 ON c1.company = c.company AND c1.branch = c.branch AND c1.code = c.code AND c1.u_stageno = IFNULL(x.counters,0)+1 LEFT OUTER JOIN u_premploymentinfo cc ON cc.company = a.company AND cc.branch = a.branch AND cc.code = c1.u_stagename WHERE a.company = '"+getGlobal("company")+"' AND a.branch = '"+getGlobal("branch")+"' AND a.u_empid = '"+getGlobal("userid")+"' AND IF(a.u_status = 1,aa.u_date_filter,a.u_appdate) = '"+daysdate+"' AND a.u_status = 1  GROUP BY a.code,IF(a.u_status = 1,aa.u_date_filter,a.u_appdate)");
