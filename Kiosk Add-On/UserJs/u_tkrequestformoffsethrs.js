...
function onPageSubmitGPSKiosk(action) {
	if (action=="a" || action=="sc") {
		...
		if (isInputEmpty("u_offdate")) return false;
		if (isInputEmpty("u_profitcenter")) return false;
		if (isInputEmpty("u_offreason")) return false;
	} else if (action=="ccn") {
...
switch (table) {
		default:
			switch(column) {
				...
				case "u_offtotime":
				case "u_offdate":
					if (getInput("u_offfromtime")!="" && getInput("u_offtotime")!="" && getInput("u_offdate")!="") {
						var daysFrom = formatDateToDB(getInput("u_offdate"))+" "+getInput("u_offfromtime");
						var daysTo = formatDateToDB(getInput("u_offdate"))+" "+getInput("u_offtotime");
					result = page.executeFormattedQuery("SELECT if(TIMESTAMPDIFF(MINUTE,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')) >= 0,TIMESTAMPDIFF(MINUTE,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')),TIMESTAMPDIFF(MINUTE,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_ADD(DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s'),INTERVAL 1 DAY))) as code,if(TIMESTAMPDIFF(HOUR,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')) >= 0,TIMESTAMPDIFF(HOUR,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')),TIMESTAMPDIFF(HOUR,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_ADD(DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s'),INTERVAL 1 DAY))) as hrs,if(TIMESTAMPDIFF(MINUTE,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')) >= 0,TIMESTAMPDIFF(MINUTE,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')),TIMESTAMPDIFF(MINUTE,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_ADD(DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s'),INTERVAL 1 DAY)))-(if(TIMESTAMPDIFF(HOUR,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')) >= 0,TIMESTAMPDIFF(HOUR,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s')),TIMESTAMPDIFF(HOUR,DATE_FORMAT('"+daysFrom+"','%Y-%m-%d %H:%i:%s'),DATE_ADD(DATE_FORMAT('"+daysTo+"','%Y-%m-%d %H:%i:%s'),INTERVAL 1 DAY)))*60) as mins");
						if (result.getAttribute("result")!= "-1") {
							if (parseInt(result.getAttribute("result"))>0) {
								setInputAmount("u_offhrs",result.childNodes.item(0).getAttribute("code")/60);
								setInputAmount("u_offhr",result.childNodes.item(0).getAttribute("hrs"));
								setInputAmount("u_offmin",result.childNodes.item(0).getAttribute("mins"));
							} else {
								setInput("u_offhrs",0);
								setInput("u_offhr",0);
								setInput("u_offmin",0);
