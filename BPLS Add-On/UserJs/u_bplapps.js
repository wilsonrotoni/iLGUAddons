...
function computeTotalAssessment() {
	var rc = getTableRowCount("T5"),total=0,totalpenalty = 0 ,totalinterest = 0 ,btax=0,btax1=0,taxrc=0, firefee = 0, firetotalamount = 0,firegrandtotal = 0;
	...
	for (i = 1; i <= rc; i++) {
        ...
        if (firefee > 0) {
                ...
                if (getPrivate("incfirebusiness")=="1") {
                     for (xxx = 1; xxx <=rc; xxx++) {
                        if (isTableRowDeleted("T5",xxx)==false) {
                            if (getTableInput("T5","u_feecode",xxx)==getPrivate("fireinspectionfeecode") && getTableInput("T5","u_year",xxx) >= getPrivate("incfirestartyear")) {
...
//                    if (fireinspectionfeerc==0) {
//                            var data = new Array();
//                            data["u_year"] = getTableInput("T1","u_year");
//                            data["u_feecode"] = getPrivate("fireinspectionfeecode");
//                            data["u_feedesc"] = getPrivate("fireinspectionfeedesc");
//                            data["u_common"] = 1;
//                            data["u_surcharge"] = formatNumericAmount(0);
...
//                    }
		...
		if (isTableRowDeleted("T5",i)==false) {
			...
			if (getTableInput("T5","u_feecode",i)==getPrivate("fireinspectionfeecode")) {
				firegrandtotal+= getTableInputNumeric("T5","u_amount",i);
			}
		}
	}
