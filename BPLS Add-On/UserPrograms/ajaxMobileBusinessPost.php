<?php
	...
	switch ($_GET["type"]) {
		case "ClearanceInspectionPictures":
			...
			file_put_contents("c:\users\BusinessInspectionPictures2.txt",$_GET["data"]);
			file_put_contents("c:\users\BusinessInspectionPictures.txt",serialize($obj));
			...
			foreach($obj['data'] as $data) {
				...
				mkdirr("..//Images/$company/$branch/U_BPLINS/" . $data["docno"] ."/Attachments/");
				$file = fopen("..//Images/$company/$branch/U_BPLINS/" . $data["docno"] ."/Attachments/". $data["name"], "wb"); // 
				fwrite($file, $binary);
		...
		case "ClearanceInspections":
			...
			$obju_FSIns = new documentschema_br(null,$objConnection,"u_bplins");
