<?php
	...
	switch ($_GET["type"]) {
		case "ClearanceInspections":
			...
			$objRs->queryopen("select a.docno, c.u_btelno,a.u_inspectdate, concat(c.u_lastname,', ',c.u_firstname,' ',c.u_middlename) as u_owner, ifnull(a.u_inspecttime,'') as u_inspecttime, a.u_businessname, a.u_address, a.u_recommendation, a.u_inspectorremarks, a.u_inspectbystatus, a.u_inspectorremarkhistory, c.u_tlastname as u_authrep, c.u_btelno as u_contactno from u_bplins a inner join u_bplapps c on c.company=a.company and c.branch=a.branch and c.docno=a.u_appno inner join u_bplinspectors b on b.code=a.u_inspector and b.u_userid='".$_GET["userid"]."' where a.company='LGU' and a.branch='main' and a.docstatus='O' $docnoExp"); 
