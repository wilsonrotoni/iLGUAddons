	...
	switch ($objTable->dbtable) {
		...
		case "u_bpltaxbill":
			...
			if ($objTable->docstatus=="Approved" && $objTable->fields["DOCSTATUS"]!=$objTable->docstatus) {
                                ...
                                $actionReturn = onCustomEventUpdateBPLAppsdocumentschema_brGPSBPLS($objTable,true);
//					$actionReturn = onCustomEventupdateBuildingAppsdocumentschema_brGPSBPLS($objTable,true);
			}
//			if ($actionReturn) $actionReturn = onCustomEventupdatebplmddocumentschema_brGPSBPLS($objTable);
...
//                            $actionReturn = onCustomEventupdatebplmddocumentschema_brGPSBPLS($objTable,true);
				...
				if ($objTable->docstatus=="Approved" && $objTable->fields["DOCSTATUS"]!=$objTable->docstatus) {
                                    ...
                                    if($objRs_uLGUSetup->fields["u_bplcombinereqassessment"]==1){
                                        $objLGUBills = new documentschema_br(null,$objConnection,"u_lgubills");
                                        while(true){
                                            if ($objLGUBills->getbysql("u_custno='".$objTable->getudfvalue("u_acctno")."' AND DOCSTATUS = 'O' AND U_PREASSBILL=0 AND u_bplupdatebill = 0 AND U_MODULE IN('Business Permit Tax Bill','Business Permit(Fire Dept)','Business Permit')")) {
                                                    $objLGUBills->docstatus =  "CN";
                                                    $actionReturn = $objLGUBills->update($objLGUBills->docno,$objLGUBills->rcdversion);
                                            } else {
                                                    break;
...
//                                    echo 'wilson';
			...
			break;
      	case "u_bplpermittemp":
      		if ($objTable->docstatus=="AP" && $objTable->fields["DOCSTATUS"]!=$objTable->docstatus && $objTable->fields["DOCSTATUS"]=="O") {
	                     $actionReturn = onCustomEventcreatebillContractorsTaxdocumentschema_brGPSBPLS($objTable);
	        }
	         /***** Re-Assessment*****/
			if  ($objTable->docstatus=="O" && ($objTable->fields["DOCSTATUS"]=="AP" || $objTable->fields["DOCSTATUS"]=="DP")) { 
				...
				if ($actionReturn && $objTable->fields["DOCSTATUS"]=="AP") {
                                    $objLGUBills = new documentschema_br(null,$objConnection,"u_lgubills");
                                        while(true){
                                            if ($objLGUBills->getbysql("U_APPNO='".$objTable->docno."' AND DOCSTATUS = 'O' AND U_PREASSBILL=0 AND u_bplupdatebill = 0 AND U_MODULE IN('Building Permit Contractors Tax')")) {
                                                    $actionReturn = $objLGUBills->executesql("DELETE FROM U_LGUBILLITEMS WHERE COMPANY='$objLGUBills->company' AND BRANCH='$objLGUBills->branch' AND DOCID='$objLGUBills->docid'",true);
                                                    if ($actionReturn) $actionReturn = $objLGUBills->delete();
                                            } else {
                                                    //$page->docstatus=$objTable->fields["DOCSTATUS"];
		...
		case "u_bplapps":
			...
			if ($objTable->docstatus=="Approved" && $objTable->fields["DOCSTATUS"]!=$objTable->docstatus) {
                        ...
                        //Generate QR
			if ($objTable->docstatus=="Paid" && $objTable->fields["DOCSTATUS"]!=$objTable->docstatus) {
                                    
                                    $objBPLMDs = new masterdataschema_br(null,$objConnection,"u_bplmds");
                                        // how to save PNG codes to server

                                      //  $tempDir = "C:\\Image\\Temp\\";
                                        $tempDir = "";
                                         mkdirr("../Images/".$_SESSION["company"]."/".$_SESSION["mainbranch"]."/BPLSQR/");
                                        $tempDir =  "../Images/".$_SESSION["company"]."/".$_SESSION["mainbranch"]."/BPLSQR/";
                                        $codeContents = 'https://api.bacoor.gov.ph/bizper/or_no.php?accountnumber=' .  $objTable->getudfvalue("u_appno");
                                       
                                        $fileName = $objTable->getudfvalue("u_appno").'.png';
...
//                                        echo $pngAbsoluteFilePath;
                                        ...
                                        if (!file_exists($pngAbsoluteFilePath)) {
                                            ...
                                            if ($objBPLMDs->getbysql("CODE='".$objTable->getudfvalue("u_appno")."'")) {
                                                $objBPLMDs->setudfvalue("u_isgeneratedqr", 1);
                                                $actionReturn = $objBPLMDs->update($objBPLMDs->code,$objBPLMDs->rcdversion);
                                                if (!$actionReturn) break;
                                            }
                                        } else {
                                        }
		...
		case "u_bpltaxbill":
			...
			if ($objTable->docstatus=="Approved" && $objTable->fields["DOCSTATUS"]!=$objTable->docstatus) {
                        ...
                        /***** Re-Assessment*****/
			if  (($objTable->docstatus=="Encoding" && ($objTable->fields["DOCSTATUS"]=="Approved" || $objTable->fields["DOCSTATUS"]=="Disapproved"))) { 
				...
				if ($actionReturn && $objTable->fields["DOCSTATUS"]=="Approved") {
                                    $objLGUBills = new documentschema_br(null,$objConnection,"u_lgubills");
                                        while(true){
                                            if ($objLGUBills->getbysql("U_CUSTNO='".$objTable->getudfvalue("u_acctno")."' AND DOCSTATUS = 'O' AND U_PREASSBILL=0 AND u_bplupdatebill = 0 AND U_MODULE IN('Business Permit Tax Bill','Business Permit(Fire Dept)')")) {
                                                    $actionReturn = $objLGUBills->executesql("DELETE FROM U_LGUBILLITEMS WHERE COMPANY='$objLGUBills->company' AND BRANCH='$objLGUBills->branch' AND DOCID='$objLGUBills->docid'",true);
                                                    if ($actionReturn) $actionReturn = $objLGUBills->delete();
                                            } else {
                                                    //$page->docstatus=$objTable->fields["DOCSTATUS"];
                                ...
                                if ($obju_BPLApps->getbykey($objTable->getudfvalue("u_appno"))) {
                                    $obju_BPLApps->docstatus="Encoding";
...
//                            $actionReturn = onCustomEventupdateBuildingAppsdocumentschema_brGPSBPLS($objTable,true);
				...
				if ($objTable->docstatus=="Approved" && $objTable->fields["DOCSTATUS"]=="Encoding") {
                                    ...
                                    if ($objTable->getudfvalue("u_billseqno") == "") {
                                        ...
                                        $actionReturn = onCustomEventUpdateBPLAppsdocumentschema_brGPSBPLS($objTable);
					$actionReturn = onCustomEventcreatefirebilldocumentschema_brGPSBPLS($objTable);
...
function onCustomEventcreatebillCombineAppandReqdocumentschema_brGPSBPLS($objTable) {
	...
	if (!$objRs_uLGUSetup->queryfetchrow("NAME")) {
        ...
        } else if ($objTable->dbtable == "u_bpltaxbill"){
            	$objRs_uBilyears->queryopen("SELECT cast(u_year as signed) as u_year FROM U_BPLTAXBILLFEES a where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 GROUP BY A.U_YEAR ");
        }
	while ($objRs_uBilyears->queryfetchrow("NAME")) {
                ...
                $duedate = "";
                    while (true) {
                        ...
                        if ($objTable->dbtable == "u_bplapps") {
                            $objLGUBills->setudfvalue("u_module","Business Permit");
                            $assdate = $objTable->getudfvalue("u_decisiondate");
                        } else if ($objTable->dbtable == "u_bpltaxbill"){
                            $objLGUBills->setudfvalue("u_module","Business Permit Tax Bill");
                            $assdate = $objTable->getudfvalue("u_assdate");
                        }
                        $d = new DateTime($assdate);
                        $objLGUBills->setudfvalue("u_profitcenter","BPL");
                        ...
                        if ($objTable->dbtable == "u_bplapps") {
                            ...
                            $objLGUBills->setudfvalue("u_custno",$objTable->getudfvalue("u_acctno"));
                        }
                        $objLGUBills->setudfvalue("u_appno",$objTable->docno);
                        $objLGUBills->setudfvalue("u_custname",$objTable->getudfvalue("u_businessname"));
				...
				case 2:
					$objLGUBills->setudfvalue("u_docdate",$objRs_uBilyears->fields["u_year"]."-04-01");
                                                ...
                                                if ($objTable->getudfvalue("u_paymode")=="S") {
                                                        ...
                                                        $duedate = dateadd("d",$objRs_uLGUSetup->fields["U_BPLDUEDAY"]-1,$startdate);
                                        ...
                                        if ($assdate > $duedate) $objLGUBills->setudfvalue("u_duedate",$d->format('Y-m-t'));
                                        else $objLGUBills->setudfvalue("u_duedate",$duedate);
					break;
				case 3:
				...
				case 4:
					...
					break;
                        ...
                        if ($objTable->dbtable == "u_bplapps") {
                            ...
                            $objRs->queryopen(" SELECT U_FEECODE, U_FEEDESC, SUM(U_AMOUNT) as U_AMOUNT, U_INSTALLMENT, U_PENALTY, U_TAXBASE, U_BUSINESSLINE, U_SEQNO FROM (
                                                    SELECT A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT, B.U_INSTALLMENT,0 AS U_PENALTY,U_TAXBASE,U_BUSINESSLINE,U_SEQNO FROM U_BPLTAXBILLFEES A
                                                    INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE 
                                                    WHERE A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 AND B.U_ENVIRONMENTAL = 0 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."'
                                                    UNION ALL 
                                                    SELECT B.U_PENALTYCODE AS U_FEECODE,B.U_PENALTYDESC AS U_FEEDESC,(A.U_SURCHARGE + A.U_INTEREST) AS U_AMOUNT, 0 AS U_INSTALLMENT,1 AS U_PENALTY,U_TAXBASE,U_BUSINESSLINE,U_SEQNO  FROM U_BPLTAXBILLFEES A
                                                    INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE
                                                    WHERE A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  (A.U_SURCHARGE + A.U_INTEREST) >0 AND B.U_ENVIRONMENTAL = 0 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."' 
                                                    UNION ALL
                                                    SELECT A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT * -1 AS U_AMOUNT, B.U_INSTALLMENT,0 AS U_PENALTY,0 AS U_TAXBASE,U_BUSINESSLINE, 0 AS U_SEQNO FROM U_BPLTAXBILLCREDITS A
                                                    INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE 
                                                    WHERE A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 AND B.U_ENVIRONMENTAL = 0 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."' AND A.U_SELECTED = 1
                                                    UNION ALL 
                                                    SELECT B.U_PENALTYCODE AS U_FEECODE,B.U_PENALTYDESC AS U_FEEDESC,(A.U_SURCHARGE + A.U_INTEREST) * -1 AS U_AMOUNT, 0 AS U_INSTALLMENT,1 AS U_PENALTY,0 AS U_TAXBASE,U_BUSINESSLINE, 0 AS  U_SEQNO  FROM U_BPLTAXBILLCREDITS A
                                                    INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE
                                                    WHERE A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  (A.U_SURCHARGE + A.U_INTEREST) >0 AND B.U_ENVIRONMENTAL = 0 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."' AND A.U_SELECTED = 1 ) AS X
                                                GROUP BY U_FEECODE,U_BUSINESSLINE");
                        }
                        $totalamount=0;
                        while ($objRs->queryfetchrow("NAME")) {
                                if ($ctr>1 && $objRs->fields["U_INSTALLMENT"]==0) continue;
                                ...
                                $objLGUBillItems->setudfvalue("u_businessline",$objRs->fields["U_BUSINESSLINE"]);
                                    if ($objRs->fields["U_INSTALLMENT"]==0) {
                                            ...
                                            $amount = round($objRs->fields["U_AMOUNT"]/$div,2);
                                            $totalamount+=round($objRs->fields["U_AMOUNT"]/$div,2);
                                    }
                                    if ($amount <= 0) return raiseError("Invalid amount [".$objLGUBillItems->getudfvalue("u_amount")."] for fee code [".$objRs->fields["U_FEECODE"]."].");
                                    
                                    $objLGUBillItems->setudfvalue("u_amount",$amount);
...
function onCustomEventcreatebillforEnvironmentaldocumentschema_brGPSBPLS($objTable) {
	...
	if (!$objRs_uLGUSetup->queryfetchrow("NAME")) {
        ...
        if ($objTable->dbtable == "u_bplapps") {
                ...
                $objRs_uBilyears->queryopen("SELECT u_year FROM U_BPLTAXBILLFEES a INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0  AND B.U_ENVIRONMENTAL = 1 GROUP BY A.U_YEAR ");
        }
	while ($objRs_uBilyears->queryfetchrow("NAME")) {
                $ctr=0;
                $alertmsg="";
				...
				case 4:
					...
					break;
                        ...
                        if ($objTable->dbtable == "u_bplapps") {
                            ...
                            $objRs->queryopen("SELECT U_FEECODE, U_FEEDESC, SUM(U_AMOUNT) AS U_AMOUNT, U_INSTALLMENT,U_SEQNO FROM (
                                                    SELECT A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT, B.U_INSTALLMENT,A.U_SEQNO FROM U_BPLTAXBILLFEES A INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 AND B.U_ENVIRONMENTAL = 1 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."'
                                                    UNION ALL
                                                    SELECT A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT * -1 AS U_AMOUNT, B.U_INSTALLMENT,0 AS U_SEQNO FROM U_BPLTAXBILLCREDITS A INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 AND B.U_ENVIRONMENTAL = 1 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."' ) AS X
                                               GROUP BY U_FEECODE ");
                        }
                        $totalamount=0;
                        while ($objRs->queryfetchrow("NAME")) {
		...
		if ($objTable->getudfvalue("u_paymode")=="A") $objLGUBills->setudfvalue("u_remarks","Business Permit & Licensing - ".date('Y') );
 		$objLGUBills->setudfvalue("u_alertmobileno",$objTable->getudfvalue("u_alertmobileno"));
                ...
                if ($objTable->dbtable == "u_bplapps") {
                    ...
                    $objRs->queryopen("SELECT A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT, B.U_INSTALLMENT FROM U_BPLTAXBILLFEES A INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0");
                }
		$totalamount=0;
		while ($objRs->queryfetchrow("NAME")) {
			//var_dump($objRs->fields);
	...
	$objRs = new recordset (NULL,$objConnection);
        ...
        if($objZoningApps->getbykey($objTable->fields["U_REQAPPNO"])){
            if ($objZoningApps->getudfvalue("u_acctno") != '' && $objZoningApps->getudfvalue("u_acctno") != $objTable->getudfvalue("u_appno")) {
                return raiseError("Account no doesnt match with Zoning application.");
            }
	...
	$objRs = new recordset (NULL,$objConnection);
        ...
        if($objBuildingApps->getbykey($objTable->fields["U_REQAPPNO"])){
            if ($objBuildingApps->getudfvalue("u_acctno") != '' && $objBuildingApps->getudfvalue("u_acctno") != $objTable->getudfvalue("u_appno")) {
                return raiseError("Account no doesnt match with Building application.");
            }
...
function onCustomEventUpdateBPLAppsdocumentschema_brGPSBPLS($objTable,$edit=false) {
	global $objConnection;
	$actionReturn = true;
	
	$objRs = new recordset (NULL,$objConnection);
	$objRs1 = new recordset (NULL,$objConnection);
        $objBPLAppsNew = new documentschema_br(null,$objConnection,"u_bplapps");
        
        $objBPLApps = new documentschema_br(null,$objConnection,"u_bplapps");
        $objBPLAppLines = new documentlinesschema_br(null,$objConnection,"u_bplapplines");
        $objBPLAppFees = new documentlinesschema_br(null,$objConnection,"u_bplappfees");
        
        if ($objBPLApps->getbykey($objTable->getudfvalue("u_appno"))) {
                $objBPLApps->docstatus="CTO_Approved";
                $objBPLApps->setudfvalue("u_capital",$objTable->getudfvalue("u_capital"));
                $objBPLApps->setudfvalue("u_nonessential",$objTable->getudfvalue("u_nonessential"));
                $objBPLApps->setudfvalue("u_btaxamount",$objTable->getudfvalue("u_btaxamount"));
                $objBPLApps->setudfvalue("u_ctcasstotal",$objTable->getudfvalue("u_ctcasstotal"));
                $objBPLApps->setudfvalue("u_fireasstotal",$objTable->getudfvalue("u_fireasstotal"));
                $objBPLApps->setudfvalue("u_asstotal",$objTable->getudfvalue("u_asstotal"));
                $objBPLApps->setudfvalue("u_prevappno",$objTable->getudfvalue("u_prevappno"));
                $objBPLApps->setudfvalue("u_appdate",$objTable->getudfvalue("u_assdate"));
                $objBPLApps->setudfvalue("u_isonlineapp",$objTable->getudfvalue("u_isonlineapp"));
                $objBPLApps->setudfvalue("u_approvedby",$objTable->getudfvalue("u_approvedby"));
                $objBPLApps->setudfvalue("u_approveddatetime",$objTable->getudfvalue("u_approveddatetime"));
                $objBPLApps->setudfvalue("u_businesschar",$objTable->getudfvalue("u_businesschar"));
                $objBPLApps->setudfvalue("u_businesskind",$objTable->getudfvalue("u_businesskind"));
                $objBPLApps->setudfvalue("u_assyearto",$objTable->getudfvalue("u_assyearto"));
                $objBPLApps->setudfvalue("u_billseqno",$objTable->getudfvalue("u_billseqno"));
                $objBPLApps->setudfvalue("u_reqappno",$objTable->getudfvalue("u_reqappno"));
                $objBPLApps->setudfvalue("u_paymode",$objTable->getudfvalue("u_paymode"));
                $objBPLApps->setudfvalue("u_envpaymode",$objTable->getudfvalue("u_envpaymode"));

                $actionReturn = $objBPLApps->executesql("DELETE FROM U_BPLAPPFEES WHERE COMPANY='$objBPLApps->company' AND BRANCH='$objBPLApps->branch' AND DOCID='$objBPLApps->docid'",true);
                $actionReturn = $objBPLApps->executesql("DELETE FROM U_BPLAPPLINES WHERE COMPANY='$objBPLApps->company' AND BRANCH='$objBPLApps->branch' AND DOCID='$objBPLApps->docid'",true);
//                    if ($actionReturn) $actionReturn = $objBPLApps->delete();
                    
                    $objRs->queryopen("SELECT A.U_YEAR,A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT, A.U_INTEREST, A.U_SURCHARGE, A.U_TAXBASE, A.U_BUSINESSLINE, A.U_COMMON, A.U_QUANTITY, A.U_REGULATORY, A.U_SEQNO FROM U_BPLTAXBILLFEES A where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' ORDER BY A.U_SEQNO");
                    while ($objRs->queryfetchrow("NAME")) {
                        $objBPLAppFees->prepareadd();
                        $objBPLAppFees->docid = $objBPLApps->docid;
                        $objBPLAppFees->lineid = getNextIdByBranch("u_bplappfees",$objConnection);
                        $objBPLAppFees->setudfvalue("u_year",$objRs->fields["U_YEAR"]);
                        $objBPLAppFees->setudfvalue("u_feecode",$objRs->fields["U_FEECODE"]);
                        $objBPLAppFees->setudfvalue("u_feedesc",$objRs->fields["U_FEEDESC"]);
                        $objBPLAppFees->setudfvalue("u_amount",$objRs->fields["U_AMOUNT"]);
                        $objBPLAppFees->setudfvalue("u_interest",$objRs->fields["U_INTEREST"]);
                        $objBPLAppFees->setudfvalue("u_surcharge",$objRs->fields["U_SURCHARGE"]);
                        $objBPLAppFees->setudfvalue("u_taxbase",$objRs->fields["U_TAXBASE"]);
                        $objBPLAppFees->setudfvalue("u_businessline",$objRs->fields["U_BUSINESSLINE"]);
                        $objBPLAppFees->setudfvalue("u_common",$objRs->fields["U_COMMON"]);
                        $objBPLAppFees->setudfvalue("u_quantity",$objRs->fields["U_QUANTITY"]);
                        $objBPLAppFees->setudfvalue("u_regulatory",$objRs->fields["U_REGULATORY"]);
                        $objBPLAppFees->setudfvalue("u_seqno",$objRs->fields["U_SEQNO"]);
                        $objBPLAppFees->privatedata["header"] = $objBPLApps;
                        $actionReturn = $objBPLAppFees->add();
                        if (!$actionReturn) break;
                    }
                    
                    $objRs1->queryopen("SELECT A.U_YEAR, A.U_BUSINESSLINE, A.U_UNITCOUNT, A.U_BTAXLINETOTAL, A.U_TAXCLASS, A.U_CAPITAL, A.U_ESSENTIAL, A.U_NONESSENTIAL, A.U_SURCHARGE, A.U_INTEREST, A.U_LASTYRGRSALES FROM U_BPLTAXBILLLINES A where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' ");
                    while ($objRs1->queryfetchrow("NAME")) {
                        $objBPLAppLines->prepareadd();
			$objBPLAppLines->docid = $objBPLApps->docid;
			$objBPLAppLines->lineid = getNextIdByBranch("u_bplapplines",$objConnection);
			$objBPLAppLines->setudfvalue("u_year",$objRs1->fields["U_YEAR"]);
			$objBPLAppLines->setudfvalue("u_businessline",$objRs1->fields["U_BUSINESSLINE"]);
			$objBPLAppLines->setudfvalue("u_unitcount",$objRs1->fields["U_UNITCOUNT"]);
			$objBPLAppLines->setudfvalue("u_btaxlinetotal",$objRs1->fields["U_BTAXLINETOTAL"]);
			$objBPLAppLines->setudfvalue("u_taxclass",$objRs1->fields["U_TAXCLASS"]);
			$objBPLAppLines->setudfvalue("u_capital",$objRs1->fields["U_CAPITAL"]);
			$objBPLAppLines->setudfvalue("u_essential",$objRs1->fields["U_ESSENTIAL"]);
			$objBPLAppLines->setudfvalue("u_nonessential",$objRs1->fields["U_NONESSENTIAL"]);
			$objBPLAppLines->setudfvalue("u_surcharge",$objRs1->fields["U_SURCHARGE"]);
			$objBPLAppLines->setudfvalue("u_interest",$objRs1->fields["U_INTEREST"]);
			$objBPLAppLines->setudfvalue("u_lastyrgrsales",$objRs1->fields["U_LASTYRGRSALES"]);
			$objBPLAppLines->privatedata["header"] = $objBPLApps;
			$actionReturn = $objBPLAppLines->add();
			if (!$actionReturn) break;
                    }
                    
                    $actionReturn = $objBPLApps->update($objBPLApps->docno,$objBPLApps->rcdversion);
                    
        } else {
                    if($objBPLApps->getbykey($objTable->getudfvalue("u_prevappno"))) {
                            $objBPLAppsNew->prepareadd();
                            $objBPLAppsNew->docid = getNextIDByBranch("u_bplapps",$objConnection);
                            $objBPLAppsNew->docno = getNextNoByBranch("u_bplapps","",$objConnection);
                            $objBPLAppsNew->docstatus = "CTO_Approved";
                            $objBPLAppsNew->copyudfsvalue($objBPLApps);
                            $objBPLAppsNew->setudfvalue("u_apptype",$objTable->getudfvalue("u_apptype"));
                            $objBPLAppsNew->setudfvalue("u_capital",$objTable->getudfvalue("u_capital"));
                            $objBPLAppsNew->setudfvalue("u_nonessential",$objTable->getudfvalue("u_nonessential"));
                            $objBPLAppsNew->setudfvalue("u_btaxamount",$objTable->getudfvalue("u_btaxamount"));
                            $objBPLAppsNew->setudfvalue("u_ctcasstotal",$objTable->getudfvalue("u_ctcasstotal"));
                            $objBPLAppsNew->setudfvalue("u_fireasstotal",$objTable->getudfvalue("u_fireasstotal"));
                            $objBPLAppsNew->setudfvalue("u_asstotal",$objTable->getudfvalue("u_asstotal"));
                            $objBPLAppsNew->setudfvalue("u_prevappno",$objTable->getudfvalue("u_prevappno"));
                            $objBPLAppsNew->setudfvalue("u_appdate",$objTable->getudfvalue("u_assdate"));
                            $objBPLAppsNew->setudfvalue("u_isonlineapp",$objTable->getudfvalue("u_isonlineapp"));
                            $objBPLAppsNew->setudfvalue("u_approvedby",$objTable->getudfvalue("u_approvedby"));
                            $objBPLAppsNew->setudfvalue("u_approveddatetime",$objTable->getudfvalue("u_approveddatetime"));
                            $objBPLAppsNew->setudfvalue("u_businesschar",$objTable->getudfvalue("u_businesschar"));
                            $objBPLAppsNew->setudfvalue("u_businesskind",$objTable->getudfvalue("u_businesskind"));
                            $objBPLAppsNew->setudfvalue("u_assyearto",$objTable->getudfvalue("u_assyearto"));
                            $objBPLAppsNew->setudfvalue("u_billseqno",$objTable->getudfvalue("u_billseqno"));
                            $objBPLAppsNew->setudfvalue("u_reqappno",$objTable->getudfvalue("u_reqappno"));
                            $objBPLAppsNew->setudfvalue("u_paymode",$objTable->getudfvalue("u_paymode"));
                            $objBPLAppsNew->setudfvalue("u_envpaymode",$objTable->getudfvalue("u_envpaymode"));
                            $objBPLAppsNew->setudfvalue("u_empcount",$objTable->getudfvalue("u_empcount"));
                            $objBPLAppsNew->setudfvalue("u_fempcount",$objTable->getudfvalue("u_fempcount"));
                            $objBPLAppsNew->setudfvalue("u_mempcount",$objTable->getudfvalue("u_mempcount"));
                            $objBPLAppsNew->setudfvalue("u_paycount",0);
                            $objTable->setudfvalue("u_appno",$objBPLAppsNew->docno);

                            $objRs->queryopen("SELECT A.U_YEAR,A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT, A.U_INTEREST, A.U_SURCHARGE, A.U_TAXBASE, A.U_BUSINESSLINE, A.U_COMMON, A.U_QUANTITY, A.U_REGULATORY, A.U_SEQNO FROM U_BPLTAXBILLFEES A where COMPANY='".$_SESSION["company"]."' AND BRANCH='".$_SESSION["branch"]."' AND DOCID='$objTable->docid' ORDER BY A.U_SEQNO");
                                while ($objRs->queryfetchrow("NAME")) {
                                    $objBPLAppFees->prepareadd();
                                    $objBPLAppFees->docid = $objBPLAppsNew->docid;
                                    $objBPLAppFees->lineid = getNextIdByBranch("u_bplapplines",$objConnection);
                                    $objBPLAppFees->setudfvalue("u_year",$objRs->fields["U_YEAR"]);
                                    $objBPLAppFees->setudfvalue("u_feecode",$objRs->fields["U_FEECODE"]);
                                    $objBPLAppFees->setudfvalue("u_feedesc",$objRs->fields["U_FEEDESC"]);
                                    $objBPLAppFees->setudfvalue("u_amount",$objRs->fields["U_AMOUNT"]);
                                    ...
                                    $objBPLAppFees->setudfvalue("u_regulatory",$objRs->fields["U_REGULATORY"]);
                                    $objBPLAppFees->setudfvalue("u_seqno",$objRs->fields["U_SEQNO"]);
                                    $objBPLAppFees->privatedata["header"] = $objBPLAppsNew;
                                    $actionReturn = $objBPLAppFees->add();
                                    if (!$actionReturn) break;
                                }

                            $objRs->queryopen("SELECT A.U_YEAR, A.U_BUSINESSLINE, A.U_UNITCOUNT, A.U_BTAXLINETOTAL, A.U_TAXCLASS, A.U_CAPITAL, A.U_ESSENTIAL, A.U_NONESSENTIAL, A.U_SURCHARGE, A.U_INTEREST, A.U_LASTYRGRSALES FROM U_BPLTAXBILLLINES A where COMPANY='".$_SESSION["company"]."' AND BRANCH='".$_SESSION["branch"]."' AND DOCID='$objTable->docid'");
                                while ($objRs->queryfetchrow("NAME")) {
                                    $objBPLAppLines->prepareadd();
                                    $objBPLAppLines->docid = $objBPLAppsNew->docid;
                                    $objBPLAppLines->lineid = getNextIdByBranch("u_bplapplines",$objConnection);
                                    $objBPLAppLines->setudfvalue("u_year",$objRs->fields["U_YEAR"]);
                                    $objBPLAppLines->setudfvalue("u_businessline",$objRs->fields["U_BUSINESSLINE"]);
                                    $objBPLAppLines->setudfvalue("u_unitcount",$objRs->fields["U_UNITCOUNT"]);
                                    $objBPLAppLines->setudfvalue("u_btaxlinetotal",$objRs->fields["U_BTAXLINETOTAL"]);
                                    $objBPLAppLines->setudfvalue("u_taxclass",$objRs->fields["U_TAXCLASS"]);
                                    $objBPLAppLines->setudfvalue("u_capital",$objRs->fields["U_CAPITAL"]);
                                    $objBPLAppLines->setudfvalue("u_essential",$objRs->fields["U_ESSENTIAL"]);
                                    $objBPLAppLines->setudfvalue("u_nonessential",$objRs->fields["U_NONESSENTIAL"]);
                                    $objBPLAppLines->setudfvalue("u_surcharge",$objRs->fields["U_SURCHARGE"]);
                                    $objBPLAppLines->setudfvalue("u_interest",$objRs->fields["U_INTEREST"]);
                                    $objBPLAppLines->setudfvalue("u_lastyrgrsales",$objRs->fields["U_LASTYRGRSALES"]);
                                    $objBPLAppLines->privatedata["header"] = $objBPLAppsNew;
                                    $actionReturn = $objBPLAppLines->add();
                                    if (!$actionReturn) break;
                                }

                            if ($actionReturn) $actionReturn = $objBPLAppsNew->add();
                    }
            }
...
function onCustomEventAuditedGrossdocumentschema_brGPSBPLS($objTable,$edit=false) {
	...
	$obju_BPLTaxBill = new documentschema_br(null,$objConnection,"u_bpltaxbill");
	$obju_BPLTaxBillLines = new documentlinesschema_br(null,$objConnection,"u_bpltaxbilllines");
	...
	$objRs_uAuditedGross->queryopen("SELECT A.U_BUSINESSLINECODE,A.U_TAXCLASS,A.U_UNAUDITEDGROSS,A.U_BTAXAMOUNT,A.U_AUDITEDGROSS,A.U_AUDITEDBTAXAMOUNT,A.U_PAIDAMOUNT,A.U_PAIDQTR FROM U_BPLUPDGROSSLINES a INNER JOIN U_BPLUPDGROSS B ON A.DOCID = B.DOCID AND A.BRANCH = B.BRANCH AND A.COMPANY = B.COMPANY WHERE A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AUDITEDGROSS > 0  AND A.U_AUDITEDBTAXAMOUNT > 0 ");
        while ($objRs_uAuditedGross->queryfetchrow("NAME")) {
            ...
            if ($objTable->getudfvalue("u_basetypenm")=="U_BPLAPPS")   $module="Business Permit"; 
            if ($objTable->getudfvalue("u_basetypenm")=="U_BPLTAXBILL")   $module= "Business Permit Tax Bill";
            while (true) {
                $ctr++;
                    switch ($ctr) {
                        case 2:
                            if ($ctr >= $qtr) {
                                if ($qtr == $ctr)  $amount = ($auditedbtaxamount * $qtr) - $paidamount;
                                    ...
                                    $objRs->queryopen("SELECT A.DOCNO,B.U_ITEMCODE,B.DOCID,B.LINEID,B.U_AMOUNT,B.U_TAXBASE,B.U_BUSINESSLINE FROM U_LGUBILLS A
                                            ...
                                            AND A.COMPANY = '".$_SESSION["company"]."' AND A.BRANCH = '".$_SESSION["branch"]."' and a.u_module = '".$module."' ");
            ...
            if ($obju_BPLApps->getbykey($objTable->getudfvalue("u_apprefno")) && $objTable->getudfvalue("u_basetypenm") =="U_BPLAPPS" ) {
            ...
            if ($obju_BPLTaxBill->getbykey($objTable->getudfvalue("u_apprefno")) && $objTable->getudfvalue("u_basetypenm") =="U_BPLTAXBILL" ) {
                if ($obju_BPLTaxBillLines->getbysql("DOCID='$obju_BPLTaxBill->docid' AND U_BUSINESSLINE='".$objRs_uAuditedGross->fields["U_BUSINESSLINECODE"]."' AND U_YEAR = '".$objTable->getudfvalue("u_year")."' ")) {
                    $obju_BPLTaxBillLines->setudfvalue("u_nonessential", $objRs_uAuditedGross->fields["U_AUDITEDGROSS"]);
                    $obju_BPLTaxBillLines->setudfvalue("u_btaxlinetotal", $objRs_uAuditedGross->fields["U_AUDITEDBTAXAMOUNT"]);
                    $actionReturn = $obju_BPLTaxBillLines->update($obju_BPLTaxBillLines->docid,$obju_BPLTaxBillLines->lineid,$obju_BPLTaxBillLines->rcdversion);
                    if (!$actionReturn) break;
                } 
            }
	}
...
function onCustomEventcreatebillContractorsTaxdocumentschema_brGPSBPLS($objTable) {
	global $objConnection;
	$actionReturn = true;
	...
	$objLGUBillItems = new documentlinesschema_br(null,$objConnection,"u_lgubillitems");
        ...
        $objRs_uBilyears = new recordset(null,$objConnection);
                $objRs_uBilyears->queryopen("SELECT u_year FROM U_BPLPERMITTEMPFEES a INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 GROUP BY A.U_YEAR ");
	...
	while ($objRs_uBilyears->queryfetchrow("NAME")) {
                    ...
                    $objLGUBills->docid = getNextIdByBranch("u_lgubills",$objConnection);
                    $objLGUBills->docstatus = "O";
                    $objLGUBills->setudfvalue("u_profitcenter","BLDG");
                    $objLGUBills->setudfvalue("u_module","Building Permit Contractors Tax");
                    $objLGUBills->setudfvalue("u_paymode","A");
                   	$objLGUBills->setudfvalue("u_custno",$objTable->getudfvalue("u_bldgno"));
                    $objLGUBills->setudfvalue("u_appno",$objTable->docno);
                    $objLGUBills->setudfvalue("u_custname",$objTable->getudfvalue("u_ownername"));
                 	$objLGUBills->setudfvalue("u_payqtr",4);
                 	$objLGUBills->setudfvalue("u_remarks","Building Permit(Contractors Tax) - ".$objRs_uBilyears->fields["u_year"]);
                 	$objLGUBills->setudfvalue("u_docdate",$objTable->getudfvalue("u_docdate"));
			
                        $objRs->queryopen("SELECT A.U_FEECODE, A.U_FEEDESC, A.U_AMOUNT, B.U_INSTALLMENT,A.U_SEQNO FROM U_BPLPERMITTEMPFEES A INNER JOIN U_LGUFEES B ON B.CODE=A.U_FEECODE where A.COMPANY='".$_SESSION["company"]."' AND A.BRANCH='".$_SESSION["branch"]."' AND A.DOCID='$objTable->docid' AND  A.U_AMOUNT>0 AND A.U_YEAR = '".$objRs_uBilyears->fields["u_year"]."' ");
                        $totalamount=0;
                        while ($objRs->queryfetchrow("NAME")) {
                                $objLGUBillItems->prepareadd();
                                $objLGUBillItems->docid = $objLGUBills->docid;
