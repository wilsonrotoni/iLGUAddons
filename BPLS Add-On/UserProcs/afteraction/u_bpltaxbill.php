...
//        var_dump($page->getitemstring("docstatus"));
	...
	if ($page->privatedata["bplassessor"]=="1" && ($page->getitemstring("docstatus")=="Encoding" || $page->getitemstring("docstatus")=="Assessed")){
        ...
        if ($page->getitemstring("u_acctno")!= "") {
            $objRs->queryopen("SELECT MAX(U_PAYYEAR) as U_PAYYEAR FROM U_BPLLEDGER  WHERE COMPANY='".$_SESSION["company"]."' and BRANCH='".$_SESSION["branch"]."' AND U_ACCTNO = '".$page->getitemstring("u_acctno")."' AND U_ISCANCELLED <> 1");
            if ($objRs->queryfetchrow("NAME")) {
                    $page->setitem("lastpayyear",$objRs->fields["U_PAYYEAR"]);
                    $objRs->queryopen("SELECT MAX(U_QUARTER) as U_PAYQTR FROM U_BPLLEDGER  WHERE COMPANY='".$_SESSION["company"]."' and BRANCH='".$_SESSION["branch"]."' AND U_ACCTNO = '".$page->getitemstring("u_acctno")."' AND U_ISCANCELLED <> 1 AND U_PAYYEAR = '".$page->getitemstring("lastpayyear")."' and U_FEEID IN ('1','5','0001') ");
                        if ($objRs->queryfetchrow("NAME")) {
        ...
        $objGrids[1]->addbutton("u_AddMultipleFees","[Add Multiple Fees]","u_MultipleFeesGPSBPLS()","left");
...
//        $page->popup->additem("popupPrintTo","Insert Payment History","u_InsertPaymentHistoryGPSBPLS()");
	...
	$objGrids[2]->columninput("u_selected","value",1);
        ...
        if (intval(substr(str_replace("/","",$page->getitemstring("u_assdate")),0,4)) > intval(1220) ){
            $page->businessobject->items->seteditable("u_paymode",false);
            $page->businessobject->items->seteditable("u_envpaymode",false);
        }
	if (isEditMode()) {
...
//                                }	
                                ...
                                $page->businessobject->items->seteditable("u_withmiscellaneous",false);
