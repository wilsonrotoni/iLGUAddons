			...
			if ($objTable->getudfvalue("u_docseries")!="-1" && $objTable->getudfvalue("u_docseries")!="") {
				...
				$obju_POSRegisters = new masterdataschema_br(null,$objConnection,"u_lguposregisters");
                                ...
                                if ($obju_POSRegisters->getbysql("U_USERID = '".$objTable->getudfvalue("u_userid")."' AND U_STATUS = 'O'")) {
                                        ...
                                        } else return raiseError("Terminal series doesnt exist. if problem persist please contact the administrator.");
                                } else return raiseError("Open register doesnt exist. if problem persist please contact the administrator.");
			}
		...
		case "u_lgupos":
                        if ($objTable->getudfvalue("u_docseries")!="-1") {
                            ...
                            $issuedto = '';
                                ...
                                if ($objLGUReceiptItems->getbysql("U_REFNO = '".$objTable->getudfvalue("u_docseries")."' and u_issuedto = '".$issuedto."'")) {
                                        ...
                                        if (!$actionReturn) break;
                                } else  return raiseError("Invalid user id.[".$issuedto."]. Please reload the page. if problem persist contact administrator.");
                        }
			...
			if ($objTable->getudfvalue("u_autoqueue")==1) {
				...
				if ($actionReturn) $actionReturn = onCustomEventPostAcctgSharingdocumentschema_brGPSLGU($objTable);
                        ...
                        if ($actionReturn && $objTable->getudfvalue("u_status")!="D" &&  $objTable->getudfvalue("u_module") == "Business Permit Tax Bill" ) {
                                if ($actionReturn) $actionReturn = onCustomEventPostBusinessPermitLedgerdocumentschema_brGPSLGU($objTable);
                                if ($actionReturn) $actionReturn = onCustomEventupdateBPLTaxCreditsdocumentschema_brGPSLGU($objTable);
                        }
                        break;
	}
	return $actionReturn;
...
function onCustomEventupdateBPLTaxCreditsdocumentschema_brGPSLGU($objTable) {
	global $objConnection;
	$actionReturn = true;
	
//	if ($objTable->docstatus=="CN") return true;
	
	$obju_BPLTaxCredits = new masterdataschema_br(null,$objConnection,"u_bpltaxcredits");
        $objRs2 = new recordset (NULL,$objConnection);
        $objRs2->queryopen("SELECT r.u_acctno,r.u_year,r.u_feecode,r.u_businessline from u_lgupos a inner join u_lguposbillnos a1 on a.docid = a1.docid and a.branch = a1.branch and a.company = a1.company inner join u_lgubills b on a1.u_billno = b.docno and a1.company = b.company and a1.branch = b.branch inner join u_bpltaxbill c on b.u_appno = c.docno inner join u_bpltaxbillcredits r on c.docid = r.docid and c.company = r.company and c.branch = r.branch where a.company='".$_SESSION["company"]."' and a.branch='".$_SESSION["branch"]."' and a.docno='$objTable->docno' and r.u_selected=1 order by r.u_year");
            while ($objRs2->queryfetchrow("NAME")) {
                    if($obju_BPLTaxCredits->getbysql("U_ACCTNO = '".$objRs2->fields["U_ACCTNO"]."' AND U_YEAR='".$objRs2->fields["U_YEAR"]."' AND U_FEECODE='".$objRs2->fields["U_FEECODE"]."' AND U_BUSINESSLINE = '".$objRs2->fields["U_BUSINESSLINE"]."'")){
                        if($objTable->getudfvalue("u_status")=="CN") $obju_BPLTaxCredits->setudfvalue("u_status","O");
                        else $obju_BPLTaxCredits->setudfvalue("u_status","CN");     

                        $actionReturn = $obju_BPLTaxCredits->update($obju_BPLTaxCredits->code,$obju_BPLTaxCredits->rcdversion);
                    }
                if (!$actionReturn) break;
             }
            ...
            if ($objTable->getudfvalue("u_module") == "Real Property Tax Bill") {
                if ($objTable->getudfvalue("u_status")=="CN") {
                    $objRs->queryopen("select r.u_docno as u_arpno, p.u_kind, max(r.u_yrto) as u_yrto,min(u_yrfr) as u_yrfr,min(r.u_payqtr) as u_qtrfr,max(r.u_payqtr) as u_qtrto,r.u_paymode from u_lgupos a 
                                        ...
                                        group by r.u_docno,r.u_paymode,r.u_yrfr,r.u_yrto order by r.u_yrto desc");
                } else {
                     $objRs->queryopen("select r.u_docno as u_arpno, p.u_kind, max(r.u_yrto) as u_yrto,min(u_yrfr) as u_yrfr,min(r.u_payqtr) as u_qtrfr,max(r.u_payqtr) as u_qtrto,r.u_paymode from u_lgupos a 
                                        inner join u_lgubills b on a.u_billno = b.docno and a.company = b.company and a.branch = b.branch
                                        inner join u_rptaxbill c on b.u_appno = c.docno and b.company = c.company and b.branch = c.branch
                                        ...
                                        inner join u_rptaxbillpins p on c.docid = p.docid and r.u_docno = p.u_docno
                                        where a.company='".$_SESSION["company"]."' and a.branch='".$_SESSION["branch"]."' and a.docno='$objTable->docno' and r.u_selected=1 
                                        group by r.u_docno,r.u_paymode,r.u_yrfr,r.u_yrto order by r.u_yrto");
                }
            } else {
...
function onCustomEventUpdateLGUBillsdocumentschema_brGPSLGU($objTable,$delete=false) {
	...
	$obju_Rptaxes = new documentschema_br(null,$objConnection,"u_rptaxes");
        ...
        $obju_BPLPermitTemp = new documentschema_br(null,$objConnection,"u_bplpermittemp");
        $obju_ZoningLocational = new documentschema_br(null,$objConnection,"u_locationalclrapps");
        $obju_FireSafety = new documentschema_br(null,$objConnection,"u_fsapps");
        $obju_LGUBills = new documentschema_br(null,$objConnection,"u_lgubills");
        ...
        if ($obju_LGUBills->getbykey($objTable->getudfvalue("u_billno"))) {
            if ($delete) {
                    ...
                    if ($actionReturn && $obju_LGUBills->getudfvalue("u_module")=="Business Permit Tax Bill") {
                           if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
                                if ($obju_BPLTaxBill->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                    if ($obju_LGUBills->docstatus=="C") {
                                                $obju_BPLTaxBill->docstatus="Paid";
                                    }
                                        $actionReturn = $obju_BPLTaxBill->update($obju_BPLTaxBill->docno,$obju_BPLTaxBill->rcdversion);
                                        
                                    if ($obju_BPLApps->getbykey($obju_BPLTaxBill->getudfvalue("u_appno"))) {
                                        if ($obju_LGUBills->docstatus=="C") {
                                                    $obju_BPLApps->docstatus="Paid";
                                        }
                                        $actionReturn = $obju_BPLApps->update($obju_BPLApps->docno,$obju_BPLApps->rcdversion);
                    ...
                    if ($actionReturn && $obju_LGUBills->getudfvalue("u_module")=="Building Permit Contractors Tax") {
                           if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
                                if ($obju_BPLPermitTemp->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                    if ($obju_LGUBills->docstatus=="C") {
                                                $obju_BPLPermitTemp->docstatus="Paid";
                                    }
                                        $actionReturn = $obju_BPLPermitTemp->update($obju_BPLPermitTemp->docno,$obju_BPLPermitTemp->rcdversion);
                                } else return raiseError("Unable to find Contractors Tax Application No.[".$obju_LGUBills->getudfvalue("u_appno")."].");	
                            }
                    }
                    if ($actionReturn && $obju_LGUBills->getudfvalue("u_module")=="Zoning Locational Clearance Tax") {
                           if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
                                if ($obju_ZoningLocational->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                    if ($obju_LGUBills->docstatus=="C") {
                                                $obju_ZoningLocational->docstatus="Paid";
                                    }
                                        $actionReturn = $obju_ZoningLocational->update($obju_ZoningLocational->docno,$obju_ZoningLocational->rcdversion);
                                } else return raiseError("Unable to find Contractors Tax Application No.[".$obju_LGUBills->getudfvalue("u_appno")."].");	
                            }
                    }
                    if ($actionReturn && $obju_LGUBills->getudfvalue("u_module")=="Fire Safety") {
                           if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
                                if ($obju_FireSafety->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                    if ($obju_LGUBills->docstatus=="C") {
                                                $obju_FireSafety->docstatus="Paid";
                                    }
                                        $actionReturn = $obju_FireSafety->update($obju_FireSafety->docno,$obju_FireSafety->rcdversion);
                                } else return raiseError("Unable to find Fire Safety Application No.[".$obju_LGUBills->getudfvalue("u_appno")."].");	
                            }
                    }
                ...
                if ($obju_LGUBills->getbykey($objRs->fields["u_billno"])) {
                        if ($delete) {
                                ...
                                if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
                                    ...
                                    if ($obju_LGUBills->getudfvalue("u_module")=="Business Permit Tax Bill") {
                                            if ($obju_BPLTaxBill->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                                    if ($obju_LGUBills->fields["DOCSTATUS"]=="C") {
                                                        if ($obju_BPLApps->getbykey($obju_BPLTaxBill->getudfvalue("u_appno"))) {
                                                            $obju_BPLApps->setudfvalue("u_paycount",$obju_BPLApps->getudfvalue("u_paycount")-1);
                                                                if ($obju_BPLApps->getudfvalue("u_paycount")==0 && ($obju_BPLApps->docstatus=="Printing" || $obju_BPLApps->docstatus=="Paid")) {
                                                                        $obju_BPLTaxBill->docstatus="Approved";
                                                                        $actionReturn = $obju_BPLTaxBill->update($obju_BPLTaxBill->docno,$obju_BPLTaxBill->rcdversion);
                                    ...
                                    if ($obju_LGUBills->getudfvalue("u_module")=="Building Permit Contractors Tax") {
                                        if ($obju_BPLPermitTemp->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                             if ($obju_LGUBills->fields["DOCSTATUS"]=="C") {
                                                $obju_BPLPermitTemp->docstatus="AP";
                                                $actionReturn = $obju_BPLPermitTemp->update($obju_BPLPermitTemp->docno,$obju_BPLPermitTemp->rcdversion);
                                             }
                                        }
                                    }
                                        
                                    if ($obju_LGUBills->getudfvalue("u_module")=="Zoning Locational Clearance Tax") {
                                        if ($obju_ZoningLocational->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                             if ($obju_LGUBills->fields["DOCSTATUS"]=="C") {
                                                $obju_ZoningLocational->docstatus="AP";
                                                $actionReturn = $obju_ZoningLocational->update($obju_ZoningLocational->docno,$obju_ZoningLocational->rcdversion);
                                             }
                                        }
                                    }
                                    if ($obju_LGUBills->getudfvalue("u_module")=="Fire Safety") {
                                        if ($obju_FireSafety->getbykey($obju_LGUBills->getudfvalue("u_appno"))) {
                                             if ($obju_LGUBills->fields["DOCSTATUS"]=="C") {
                                                $obju_FireSafety->docstatus="AP";
                                                $actionReturn = $obju_FireSafety->update($obju_FireSafety->docno,$obju_FireSafety->rcdversion);
                                             }
                                        }
                                    }
...
//                                            $obju_LGUBills->setudfvalue("u_dueamount",bcsub($obju_LGUBills->getudfvalue("u_dueamount"),($objRs2->fields["U_PAIDAMOUNT"]-$objTable->getudfvalue("u_penaltyamount")),2));
                                ...
                                if ($actionReturn && $obju_LGUBills->getudfvalue("u_module")=="Zoning Locational Clearance Tax") {
                                        if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
                                             ...
                                             } else return raiseError("Unable to find Zoning Locational Application No.[".$obju_LGUBills->getudfvalue("u_appno")."].");	
                                         }
                                }
                                if ($actionReturn && $obju_LGUBills->getudfvalue("u_module")=="Fire Safety") {
                                        if ($actionReturn && $obju_LGUBills->getudfvalue("u_appno")!="") {
...
function onCustomEventPostRPTLedgerdocumentschema_brGPSLGU($objTable,$delete=false) {
	...
	$obju_RPPaymentHistory = new documentschema_br(null,$objConnection,"u_paymenthistory");
        ...
        $objRs->queryopen("SELECT
                            ...
                            ,0 as SEF
                            ,SUM(D.U_SHTPENALTY) as BasicPenalty
                            ,0 as SEFPenalty
                            ,0 as BasicDiscount
                            ,0 as SEFDiscount
                            ,SUM(D.U_SHT) + SUM(D.U_SHTPENALTY) as U_LINETOTAL
                            ,D.u_paymode
                            ,IF(D.U_PAYMODE = 'A',1,MIN(D.U_PAYQTR)) AS PAYQTRFROM
                            ,MAX(D.U_PAYQTR) AS PAYQTRFROM
                            ,a.CreatedBy
                            ...
                            E.u_pin
                            ,D1.u_docno as u_refno
                            ,CONCAT(D1.U_TDNO,'(SHT TAX CREDIT)') AS U_TDNO
                            ,A.docno as CurrentOR
                            ,C.U_DECLAREDOWNER as u_declaredowner
                            ,A.u_custname as PaidBy
                            ,A.u_date as CurrentDate
                            ,A.u_paidamount as Paidamount
                            ,concat(E.u_barangay,'(SHT TAX CREDIT)') as barangay
                            ,if(E.u_kind = 'LAND','L',if(E.u_kind = 'BUILDING','B','M')) as Type
                            ,E.u_assvalue
                            ,IF(min(d1.U_YEAR) = max(d1.U_YEAR),min(d1.U_YEAR),concat(min(d1.U_YEAR),'-',max(d1.U_YEAR))) as taxyear
                            ,MIN(D1.U_YEAR) AS u_yrfr
                            ,MAX(D1.U_YEAR) AS u_yrto
                            ,SUM(D1.U_SHT) * -1 as BasicTax
                            ,0 as SEF
                            ,0 as BasicPenalty
                            ,0 as SEFPenalty
                            ,0 as BasicDiscount
                            ,0 as SEFDiscount
                            ,SUM(D1.U_SHT) * -1  as U_LINETOTAL
                            ,'A' AS u_paymode
                            ,'1' AS PAYQTRFROM
                            ,'4' AS PAYQTRTO
                            ,a.CreatedBy
                            ...
                            INNER JOIN U_LGUBILLS B on A.u_billno = B.docno and A.company = B.company and A.branch = B.branch
                            INNER JOIN U_RPTAXBILL C on B.u_appno = C.docno AND B.COMPANY = C.COMPANY AND B.BRANCH = C.BRANCH
                            INNER JOIN U_RPTAXBILLCREDITS D1 ON D1.COMPANY = C.COMPANY AND D1.BRANCH = C.BRANCH AND D1.DOCID = C.DOCID AND D1.U_SELECTED = 1 AND D1.U_SHT > 0
                            INNER JOIN U_RPTAXBILLPINS E ON E.COMPANY = C.COMPANY AND E.BRANCH = C.BRANCH AND E.DOCID = C.DOCID AND D1.U_DOCNO = E.U_DOCNO AND E.U_SELECTED = 1
                            LEFT JOIN U_RPFAAS1 R1 ON D1.U_DOCNO = R1.DOCNO AND D1.COMPANY = R1.COMPANY AND D1.BRANCH = R1.BRANCH
                            LEFT JOIN U_RPFAAS2 R2 ON D1.U_DOCNO = R2.DOCNO AND D1.COMPANY = R2.COMPANY AND D1.BRANCH = R2.BRANCH
                            LEFT JOIN U_RPFAAS3 R3 ON D1.U_DOCNO = R3.DOCNO AND D1.COMPANY = R3.COMPANY AND D1.BRANCH = R3.BRANCH
...
function onCustomEventPostBusinessPermitLedgerdocumentschema_brGPSLGU($objTable) {
	...
	$obju_BusinessLedger = new documentschema_br(null,$objConnection,"u_bplledger");
        $objRs = new recordset(null,$objConnection);
        $objRs->queryopen("SELECT B.U_BASEDOCNO,B.U_BASELINEID,A.CREATEDBY,D.U_DOCDATE,D.U_DUEDATE,SUBSTRING(D.U_REMARKS,-4,4) AS U_YEAR,IFNULL(F1.U_APPNO,F.DOCNO) AS DOCNO,A.DOCNO as U_ORNO, A.U_DATE,E.U_BUSINESSLINE,B.U_ITEMCODE,E.U_TAXBASE,IF(E.U_PENALTY = 0,E.U_AMOUNT,0) AS U_AMOUNTDUE,B.U_LINETOTAL AS U_AMOUNTPAID ,IF(E.U_PENALTY = 1,B.U_LINETOTAL,0)AS U_PENALTYAMT,D.U_PAYMODE,D.U_PAYQTR,IFNULL(F1.U_ACCTNO,F.U_APPNO) AS U_APPNO,B.U_ITEMDESC,E.U_SEQNO FROM U_LGUPOS A
                            ...
                            LEFT JOIN U_BPLTAXBILL F1 ON D.U_APPNO = F1.DOCNO AND D.COMPANY = F1.COMPANY AND D.BRANCH = F1.BRANCH
