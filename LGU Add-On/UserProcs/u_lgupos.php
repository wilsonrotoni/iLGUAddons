...
<link rel="stylesheet" type="text/css" href="css/mainheader2.css">
...
<link rel="stylesheet" type="text/css" href="css/tblbox2.css">
...
<style type="text/css">
    .lblobjs { FONT-FAMILY: Calibri Light}
</style>
</head>
...
function onAfterDefaultGPSPOS() { 
	...
	$objRs3 = new recordset(null,$objConnection);
        $objRs3->queryopen("select CODE from U_LGUCOLLECTORS WHERE U_USERID='".$_SESSION["userid"]."'");
            if ($objRs3->queryfetchrow("NAME")) {
                $page->setitem("u_collector",$objRs3->fields["CODE"]);
            }
	...
	$objRs->queryopen("SELECT A.CODE, B.U_AUTOSERIES,B.U_ISSUEDOCNO,B.DOCNO AS U_SERIESDOCNO,B.U_DOCSERIES,B.U_DOCSERIESNAME  FROM U_LGUPOSREGISTERS A
                            ...
                            WHERE A.BRANCH='".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.U_USERID='".$_SESSION['userid']."' AND A.U_STATUS = 'O'  AND C.U_ISBURIAL = 0 AND B.U_NEXTNO <= B.U_LASTNO  ORDER BY B.U_ISDEFAULT DESC");
...
//		}	
	...
	$page->setitem("u_ftduedate",'01/20/'.date('Y'));
	$page->setitem("u_date",currentdate());
	$page->setitem("u_ctccitizenship","Filipino");
	...
	$page->setitem("u_terminalid",$_SESSION["userid"]);
	if ($autoseries==1) {
...
$page->businessobject->items->setcfl("u_ctcdob","Calendar");
$page->businessobject->items->setcfl("u_ttdosdate","Calendar");
$page->businessobject->items->setcfl("u_ftduedate","Calendar");
$page->businessobject->items->setcfl("u_custno","OpenCFLbusinesspartners()");
...
$page->businessobject->items->seteditable("u_bpasappno",false);
...
//$objGrids[0]->columnvisibility("u_basedocno",false);
...
$objGrids[0]->columnvisibility("u_businessline",false);
...
$objGrids[5]->columncfl("u_billno","OpenCFLfs()");
$objGrids[5]->columnwidth("u_billno",30);
$objGrids[5]->columnattributes("u_remarks","disabled");
$objGrids[5]->columnattributes("u_amount","disabled");
...
if ($page->getvarstring("formType")!="popup") $page->resize->addinput("totalamountdisplay",450,-1);
