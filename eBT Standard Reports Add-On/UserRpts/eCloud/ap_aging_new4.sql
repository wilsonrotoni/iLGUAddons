...
DROP PROCEDURE IF EXISTS `ap_aging_new4` $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ap_aging_new4`(
...
CREATE TEMPORARY TABLE  `pdcpayments` (
  ...
  INSERT
    INTO pdcpayments (COMPANY,BRANCH,BPCODE,ACCTNO,PDCAMOUNT)
      SELECT a.company,a.branch,a.bpcode,a.refno, sum(a.rebate + a.amount) as pdcamount
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company, branch, objectcode, docno, BALANCE, DOCDATE, paydocno, 'BANKING'
      from (SELECT a.company,a.branch,a.objectcode, a.docno, sum(b.amount - b.penaltypaid)*-1 as BALANCE, c.VALUEDATE AS DOCDATE, c.docno as paydocno
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company, branch, objectcode, docno, sum(BALANCE), DOCDATE, paydocno, 'BANKING'
      from (SELECT a.company,a.branch,a.objectcode,d.docno, sum(b.amount - b.penaltypaid)*-1 as BALANCE, c.VALUEDATE AS DOCDATE, c.docno as paydocno
              ...
              group by d.DOCNO
        union all
          SELECT a.company,a.branch,a.objectcode,d.docno, sum(b.amount) as BALANCE, c.DOCDATE, c.docno as paydocno
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company,branch,objectcode,docno, sum(BALANCE), DOCDATE, paydocno, 'BANKING'
      from (SELECT a.company,a.branch,a.objectcode,a.docno, sum(b.amount - b.penaltypaid) as BALANCE, c.VALUEDATE AS DOCDATE, c.docno as paydocno
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company,branchcode,objectcode,docno, sum(BALANCE), DOCDATE, PAYDOCNO, 'BANKING'
      from (
            SELECT a.company,a.branchcode,a.objectcode,a.docno, if(a.collfor = 'DP',sum(b.amount - b.penaltypaid)*-1, sum(b.amount - b.penaltypaid)) as BALANCE, c.VALUEDATE AS DOCDATE, a.docno as paydocno
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company,branchcode,objectcode,docno, sum(BALANCE), DOCDATE, PAYDOCNO, 'BANKING'
      from (SELECT a.company,a.branchcode,a.objectcode,a.docno, sum(b.amount - b.penaltypaid) as BALANCE, c.DOCDATE, c.docno as PAYDOCNO
              from payments a, collectionsinvoices b, collections c
              where b.company = a.company
                and b.branch = a.branchcode
                and b.refno = a.docno
...
call sp_processtime('A/P Journals','start');
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
        SELECT a.company,a.branch,a.objectcode,a.docno, sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc))*-1 as BALANCE, c.DOCDATE, c.docno as paydocno, 'JV'
...
call sp_processtime('Journal Journals','start');
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
        SELECT a.company,a.branch,d.objectcode,d.docno, sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc))*-1 as BALANCE, c.DOCDATE, c.docno AS paydocno, 'JV'
...
call sp_processtime('DP/RS Journals','start');
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
        SELECT a.company,a.branchcode,a.objectcode,a.docno, sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc))*-1 as BALANCE, c.DOCDATE, c.docno as paydocno, 'JV'
...
call sp_processtime('Sales Returns','start');
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
       SELECT a.company,a.branch,a.objectcode,a.docno, sum(b.BASEAMOUNT), b.DOCDATE, b.DOCNO as paydocno, 'APCM'
...
call sp_processtime('A/P Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branch,a.objectcode,a.docno, 0 - sum(b.amount) as advancepayment, c.VALUEDATE AS DOCDATE
        from apinvoices a, paymentinvoices b, payments c
        where b.company = a.company
        and b.branch = a.branch
        and b.refno = a.docno
...
call sp_processtime('Journal Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branch,a.objectcode,d.docno, 0 - sum(b.amount) as advancepayment, c.VALUEDATE AS DOCDATE
        from journalvouchers a, journalvoucheritems d, paymentinvoices b, payments c
...
call sp_processtime('DP/RS Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branchcode,a.objectcode,a.docno, 0 - sum(b.amount - b.penaltypaid) as advancepayment, c.VALUEDATE AS DOCDATE
        from payments a, collectionsinvoices b, collections c
        where b.company = a.company
        and b.branch = a.branchcode
        and b.refno = a.docno
...
call sp_processtime('Advances Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branchcode,a.objectcode,a.docno, sum(b.amount) as advancepayment, c.VALUEDATE AS DOCDATE
        from payments a, paymentinvoices b, payments c
        where b.company = a.company
        and b.branch = a.branchcode
        and b.refno = a.docno
...
call sp_processtime('A/P Advance Journals','start');
  INSERT
    INTO advjv (COMPANY,BRANCH,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branch,a.docno, 0 - sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as advancepayment, c.DOCDATE
        from apinvoices a, journalvoucheritems b, journalvouchers c
        where b.company = a.company
        and b.branch = a.branch
        and b.refno = a.docno
...
call sp_processtime('Journal Advance Journals','start');
  INSERT
    INTO advjv (COMPANY,BRANCH,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branch,d.docno, 0 - sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as advancepayment, c.DOCDATE
        from journalvouchers a, journalvoucheritems d, journalvoucheritems b, journalvouchers c
        where b.company = d.company
        and b.branch = d.branch
        and b.refno = d.docno
...
call sp_processtime('DP/RS Advance Journals','start');
  INSERT
    INTO advjv (COMPANY,BRANCH,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branchcode,a.docno, 0 - sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as advancepayment, c.DOCDATE
        from payments a, journalvoucheritems b, journalvouchers c
        where b.company = a.company
        and b.branch = a.branchcode
        and b.refno = a.docno
...
INSERT
    ...
    SELECT apinvoices.COMPANY, apinvoices.BRANCH, apinvoices.OBJECTCODE, apinvoices.DOCNO,
        ...
        between 1 and 30 then (apinvoices.totalamount -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(apinvoices.DOCDUEDATE, '%y-%m-%d'))
        between 61 and 90 then (apinvoices.totalamount -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
...
INSERT
    ...
    SELECT journalvoucheritems.COMPANY, journalvoucheritems.BRANCH, journalvoucheritems.OBJECTCODE, journalvoucheritems.DOCNO,
        ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) - if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        ...
        then ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) - if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else
        if(((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))<0,
        ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)),0) end
        ,0) as 'current',
        ...
        between 1 and 30 then ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D7',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d'))
        between 31 and 60 then ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D15',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d'))
        between 61 and 90 then ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
        ...
        then ((if(journalvoucheritems.currency = 'RM', journalvoucheritems.credit - journalvoucheritems.debit, journalvoucheritems.credit_fc - journalvoucheritems.debit_fc)) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'UP30',
...
INSERT
    ...
    SELECT apcreditmemos.COMPANY, apcreditmemos.BRANCH, apcreditmemos.OBJECTCODE, apcreditmemos.DOCNO,
        ...
        between 1 and 30 then (apcreditmemos.totalamount -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D7',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(apcreditmemos.DOCDUEDATE, '%y-%m-%d'))
        between 61 and 90 then (apcreditmemos.totalamount -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
...
INSERT
    ...
    SELECT payments.COMPANY, payments.BRANCHCODE, payments.OBJECTCODE, payments.DOCNO,
        ((payments.PAIDAMOUNT-payments.DUEAMOUNT) - if (balances.BALANCE is null, 0, balances.BALANCE) -
        ...
        then ((payments.PAIDAMOUNT-payments.DUEAMOUNT) - if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else
        if(((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))<0,
        ((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        ...
        between 1 and 30 then ((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.DOCDUEDATE, '%y-%m-%d'))
        between 31 and 60 then ((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else 0.00 end,
        0) as 'D15',
        if(((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.DOCDUEDATE, '%y-%m-%d'))
        between 61 and 90 then ((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else 0.00 end,
        0) as 'D30',
        if(((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        ...
        then ((payments.PAIDAMOUNT-payments.DUEAMOUNT) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else 0.00 end,
        0) as 'UP30',
        (payments.PAIDAMOUNT-payments.DUEAMOUNT) PAIDAMOUNT
        FROM payments
...
INSERT
    INTO concatfield (COMPANY,BRANCH,OBJECTCODE, DOCNO,DUEAMOUNT,advpay,current,D7,D15,D30,UP30,AMOUNT)
    SELECT payments.COMPANY, payments.BRANCHCODE, payments.OBJECTCODE, payments.DOCNO,
        sum(paymentinvoices.amount) as DUEAMOUNT,
        0 AS advpay,
        sum(paymentinvoices.amount) as 'current',
        0 as 'D7',
        0 as 'D15',
        0 as 'D30',
        0 as 'UP30',
...
INSERT
    ...
    SELECT c.COMPANY, c.BRANCH, c.OBJECTCODE, c.DOCNO,
        sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as DUEAMOUNT,
        0 AS advpay,
        sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as 'current',
        0 as 'D7',
        0 as 'D15',
        0 as 'D30',
        0 as 'UP30',
        sum(if(b.currency = 'RM', b.debit, b.debit_fc))
    from apinvoices a, journalvoucheritems b, journalvouchers c
        where b.company = a.company
        and b.branch = a.branch
        and b.refno = a.docno
...
call sp_processtime('Advances Journals as Advances','start');
  INSERT
    INTO concatfield (COMPANY,BRANCH,OBJECTCODE, DOCNO,DUEAMOUNT,advpay,current,D7,D15,D30,UP30,AMOUNT)
        SELECT a.company,a.branchcode, c.OBJECTCODE,c.docno, sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as DUEAMOUNT,0 AS advpay,
        sum(if(b.currency = 'RM', b.credit - b.debit, b.credit_fc - b.debit_fc)) as 'current',
        0 as 'D7',
        0 as 'D15',
        0 as 'D30',
...
INSERT
    INTO mainfield (COMPANY,BRANCH,BRANCHNAME,BPCODE,BPNAME,CURRENCY,REFNO,BILLTOADDRESS,ITEMCODE,OBJECTCODE,DOCNO,DOCDATE,DOCDUEDATE,
    ...
    SELECT apinvoices.COMPANY,
        ...
        WHERE apinvoices.trxtype<>'POS'
          ...
          AND (pi_suppno='' or (pi_suppno<>'' and apinvoices.bpcode=pi_suppno))
          AND (pi_currency='' or (pi_currency<>'' and apinvoices.currency=pi_currency))
        GROUP BY apinvoices.DOCNO
        ORDER BY apinvoices.docno ASC;
...
INSERT
    ...
    SELECT apcreditmemos.COMPANY,
        ...
        and apcreditmemos.docstatus in ('O','C')
          ...
          AND (concatfield.DUEAMOUNT<>0 or (concatfield.DUEAMOUNT=0 and concatfield.advpay<>0))
          AND (pi_suppno='' or (pi_suppno<>'' and apcreditmemos.bpcode=pi_suppno))
          AND (pi_currency='' or (pi_currency<>'' and apcreditmemos.currency=pi_currency))
        GROUP BY apcreditmemos.DOCNO
        ORDER BY apcreditmemos.docno ASC;
...
INSERT
    INTO mainfield (COMPANY,BRANCH,BRANCHNAME,BPCODE,BPNAME,CURRENCY,BILLTOADDRESS,ITEMCODE,OBJECTCODE,DOCNO,DOCDATE,DOCDUEDATE,
    DUEAMOUNT,aging,fisrtmonth,secondndmonth,thirdmonth,advpay,current,D7,D15,D30,UP30,
    PDC,cname,AMOUNT)
...
if pi_output='' then
    SELECT
      ...
      a.BPCODE,
      ct.suppname BPNAME,
      concat(ct.suppname,' (',a.BPCODE,')') as BPNAMECODE,
      a.CURRENCY,
      a.REFNO,
      a.BILLTOADDRESS,
      a.DOCNO,
...
INSERT
    INTO mainfield2 (COMPANY,BRANCH,BRANCHNAME,BPCODE,BPNAME,CURRENCY,REFNO,OBJECTCODE,DOCNO,DOCDATE,DOCDUEDATE,aging,fisrtmonth,secondndmonth,thirdmonth,cname,AMOUNT,SEQ,PAYDOCNO,PAYDOCDATE)
    SELECT mf.COMPANY,
        mf.BRANCH,
        mf.BRANCHNAME,
        mf.BPCODE,
...
INSERT
    INTO mainfield (COMPANY,BRANCH,BRANCHNAME,BPCODE,BPNAME,CURRENCY,REFNO,OBJECTCODE,DOCNO,DOCDATE,DOCDUEDATE,aging,fisrtmonth,secondndmonth,thirdmonth,cname,AMOUNT,SEQ,PAYDOCNO,PAYDOCDATE)
     select COMPANY,BRANCH,BRANCHNAME,BPCODE,BPNAME,CURRENCY,REFNO,OBJECTCODE,DOCNO,DOCDATE,DOCDUEDATE,aging,fisrtmonth,secondndmonth,thirdmonth,cname,AMOUNT,SEQ,PAYDOCNO,PAYDOCDATE from mainfield2;
    SELECT
      a.COMPANY,
      a.cname AS 'COMPANYNAME',
      a.BRANCH,
