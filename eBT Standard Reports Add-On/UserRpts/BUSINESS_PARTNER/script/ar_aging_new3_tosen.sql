...
DROP PROCEDURE IF EXISTS `ar_aging_new3_tosen` $$
CREATE PROCEDURE `ar_aging_new3_tosen`(
...
CREATE TEMPORARY TABLE  `pdcpayments` (
    ...
    `CHECKDATE` DATE NULL,
    `BANKBRANCH` varchar(100) NULL default '',
    `PDCAMOUNT2` NUMERIC(18,6) NULL default '0',
    PRIMARY KEY  (`COMPANY`,`BRANCH`,`ACCTNO`)
  ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
  INSERT
    INTO pdcpayments ()
      SELECT a.company,a.branch,a.bpcode,a.refno,
        sum(a.rebate + a.amount + a.discamount) as pdcamount,
        b.checkdate,
        CONCAT(b.bank,'/',b.bankbranch) bankbranch,
        sum(b.amount) as pdcamount2
...
INSERT
    ...
    SELECT arinvoices.COMPANY, arinvoices.BRANCH, arinvoices.OBJECTCODE, arinvoices.DOCNO,
        ...
        case when datediff(pi_date, date_format(arinvoices.docdate, '%y-%m-%d')) <=30
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(arinvoices.docdate, '%y-%m-%d'))
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(arinvoices.docdate, '%y-%m-%d'))
        between 91 AND 120 then (arinvoices.totalamount -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(arinvoices.docdate, '%y-%m-%d')) >= 121
...
INSERT
    ...
    SELECT journalvoucheritems.COMPANY, journalvoucheritems.BRANCH, journalvoucheritems.OBJECTCODE, journalvoucheritems.DOCNO,
        ...
        case when datediff(pi_date, date_format(journalvouchers.docdate, '%y-%m-%d')) <=30
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.docdate, '%y-%m-%d'))
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.docdate, '%y-%m-%d'))
        between 91 AND 120 then ((journalvoucheritems.debit-journalvoucheritems.credit) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.docdate, '%y-%m-%d')) >= 121
...
INSERT
    ...
    SELECT arcreditmemos.COMPANY, arcreditmemos.BRANCH, arcreditmemos.OBJECTCODE, arcreditmemos.DOCNO,
        ...
        case when datediff(pi_date, date_format(arcreditmemos.docdate, '%y-%m-%d')) <=30
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(arcreditmemos.docdate, '%y-%m-%d'))
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(arcreditmemos.docdate, '%y-%m-%d'))
        between 91 AND 120 then (arcreditmemos.totalamount -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(arcreditmemos.docdate, '%y-%m-%d')) >= 121
...
INSERT
    ...
    SELECT collections.COMPANY, collections.BRANCHCODE, collections.OBJECTCODE, collections.DOCNO,
        ...
        case when datediff(pi_date, date_format(collections.docdate, '%y-%m-%d')) <=30
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(collections.docdate, '%y-%m-%d'))
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(collections.docdate, '%y-%m-%d'))
        between 91 AND 120 then (collections.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(collections.docdate, '%y-%m-%d')) >= 121
...
INSERT
    ...
    SELECT payments.COMPANY, payments.BRANCHCODE, payments.OBJECTCODE, payments.DOCNO,
        ...
        case when datediff(pi_date, date_format(payments.docdate, '%y-%m-%d')) <=30
        ...
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.docdate, '%y-%m-%d'))
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.docdate, '%y-%m-%d'))
        between 91 AND 120 then (payments.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else 0.00 end,
        0) as 'D30',
        if((payments.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.docdate, '%y-%m-%d')) >= 121
...
if pi_output='' then
    ...
    SELECT
      ...
      a.BPCODE,
      a.BPNAME AGENAME,
      if(a.BPREFNO = '' or a.BPREFNO is null, a.DOCNO, a.BPREFNO) REFNO,
      a.BILLTOADDRESS,
      a.DOCNO LINK,
      a.DOCDATE ADATE,
      a.DOCDUEDATE,
      a.AMOUNT,
      a.DUEAMOUNT TOTALAMOUNT,
      a.AGING,
      @DATE:=TIMESTAMPDIFF(DAY,a.docdate,pi_date) AGE,

       IF(@DATE <= 30 or a.DUEAMOUNT < 0,a.DUEAMOUNT,0) as 'CAMOUNT',
       IF(@DATE BETWEEN 31 AND 60 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) as '31AMOUNT',
       IF(@DATE BETWEEN 61 AND 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) as '61AMOUNT',
       IF(@DATE BETWEEN 91 AND 120 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) as '91AMOUNT',
       IF(@DATE >= 121 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) as '121AMOUNT',
      ...
      sp.salesperson,
      sp.salespersonname SALES,
      if(ar.u_cmremarks = '', ar.remarks, ar.u_cmremarks) REMARKS,
      if(ar.u_arstat = '' or ar.u_arstat is null, 1, ar.u_arstat) u_arstat
    ...
    FROM mainfield a
         ...
         INNER JOIN arinvoices ar ON a.company=ar.COMPANY and a.branch=ar.BRANCH and a.docno = ar.docno
         LEFT OUTER JOIN salespersons sp ON ar.salesperson = sp.salesperson
...
elseif pi_output='cas' then
    ...
    SELECT
      ...
      a.DOCDUEDATE,
      sum(a.AMOUNT) AMOUNT,
      sum(a.DUEAMOUNT) TOTALAMOUNT,
      a.AGING,
      @DATE:=TIMESTAMPDIFF(DAY,a.docdate,pi_date) AGE,

       sum(IF(@DATE <= 29 or a.DUEAMOUNT < 0,a.DUEAMOUNT,0)) as 'CAMOUNT',
       sum(IF(@DATE BETWEEN 30 AND 59 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)) as '31AMOUNT',
       sum(IF(@DATE BETWEEN 60 AND 89 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)) as '61AMOUNT',
       sum(IF(@DATE >= 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)) as '91AMOUNT',
       sum(IF(@DATE >= 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)) as '121AMOUNT',
      ...
      when LEFT(pt.paymenttermNAME,2) <= 29 then
      IF(@DATE BETWEEN 30 AND 59 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) +
      IF(@DATE BETWEEN 60 AND 89 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) +
      IF(@DATE >= 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)

      when LEFT(pt.paymenttermNAME,2) BETWEEN 30 AND 59 then
      IF(@DATE BETWEEN 60 AND 89 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0) +
      IF(@DATE >= 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)

      when LEFT(pt.paymenttermNAME,2) BETWEEN 60 AND 89 then
      IF(@DATE >= 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)

      when LEFT(pt.paymenttermNAME,2) >= 90 OR pt.paymentterm = 10 then
      IF(@DATE >= 90 and a.DUEAMOUNT > 0,a.DUEAMOUNT,0)

      end) as 'Total Amount Over Credit Term',

      sum(a.DUEAMOUNT) - ct.CREDITLIMIT as 'Total Amount Over Limit',
      cg.groupname
    ...
    FROM mainfield a
         ...
         INNER JOIN arinvoices ar ON a.company=ar.COMPANY and a.branch=ar.BRANCH and a.docno = ar.docno
         LEFT OUTER JOIN salespersons sp ON ar.salesperson = sp.salesperson
         LEFT OUTER JOIN customergroups cg ON cg.custgroup = ct.custgroup
...
INSERT
    ...
    SELECT
      ...
      a.BPCODE,
      a.BPNAME,
      ad.street BILLTOADDRESS,
      a.DOCNO,
      if(a.BPREFNO = '' or a.BPREFNO is null, a.DOCNO, a.BPREFNO) BPREFNO,
      a.DOCDATE,
      a.DOCDUEDATE,
      a.AMOUNT,
      a.DUEAMOUNT,
      @DATE:=TIMESTAMPDIFF(DAY,a.docdate,pi_date) AGING,
      ...
      pt.paymenttermNAME as term,
      pt.extradays,
      ct.CREDITLIMIT as creditlimit,SEQ,
      if(col.REFNO = '' or col.REFNO is null, PAYDOCNO, col.REFNO) PAYDOCNO,
      PAYDOCDATE,
      sp.salesperson,
      sp.salespersonname,
      colc.bank, CONCAT(colc.bank,'/',colc.bankbranch) bankbranch, colc.checkno, colc.checkdate,
      pdc.CHECKDATE CHECKDATE2, pdc.BANKBRANCH BANKBRANCH2,
      ctp.name contactperson


    FROM mainfield a
         LEFT OUTER JOIN customers ct ON ct.company=a.COMPANY and ct.branch=a.BRANCH and ct.custno = a.BPCODE
         LEFT OUTER JOIN customercontacts ctp ON ctp.company=a.COMPANY and ctp.branch=a.BRANCH and ctp.custno = a.BPCODE and ctp.contactperson = ct.dfltcontactperson
         LEFT OUTER JOIN paymentterms pt ON pt.paymentterm = ct.paymentterm
         LEFT OUTER JOIN addresses ad ON ad.company=a.COMPANY and ad.branch=a.BRANCH and ad.refid = a.BPCODE
                                         and ct.dfltbillto = ad.addressname and ad.reftype = 'CUSTOMER' and ad.addresstype = 0

         INNER JOIN arinvoices ar ON a.company=ar.COMPANY and a.branch=ar.BRANCH and a.docno = ar.docno
         LEFT OUTER JOIN salespersons sp ON ar.salesperson = sp.salesperson
         LEFT OUTER JOIN customergroups cg ON cg.custgroup = ct.custgroup
         LEFT OUTER JOIN collections col ON a.company=col.COMPANY and a.branch=col.BRANCHCODE and a.PAYDOCNO = col.docno
         LEFT OUTER JOIN collectionscheques colc ON a.company=colc.COMPANY and a.branch=colc.BRANCH and a.PAYDOCNO = colc.docno
         LEFT OUTER JOIN pdcpayments pdc ON a.COMPANY = pdc.COMPANY AND a.BRANCH = pdc.BRANCH AND a.DOCNO = pdc.ACCTNO
