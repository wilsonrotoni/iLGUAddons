<?php
	...
	$pageHeader = "Barangay Collections";
	unset($enumyear[""]);
	$page->restoreSavedValues();	
	
	$page->objectcode = "U_YBLIPRLIST";
	$page->paging->formid = "./UDP.php?&objectcode=u_brgycollections_grid";
...
";
 ...
 sum(if(day(docdate)=1 and dropship=0,quantity,0)) as d1_qty,
  ...
  from ( $unionExp ) as x group by itemgroupname order by `all".iif($page->getitemstring("valopt")!="value","_qty","")."` desc ");
	...
	$page->resize->addgridobject($objGrid,0,85); 
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
      ...
      <td align=right><label <?php genCaptionHtml($schema["byopt"],"") ?>>Display By</label>&nbsp;<select <?php genSelectHtml($schema["byopt"],array("loadenumbyopt","","")) ?> ></select>&nbsp;&nbsp;<input type="text" size=2 alt='Auto Refresh Minutes' <?php genInputTextHtml($schema["automins"]) ?> /></td>
	  </tr>
	</table></td></tr>	
<tr class="fillerRow10px"><td></td></tr>
