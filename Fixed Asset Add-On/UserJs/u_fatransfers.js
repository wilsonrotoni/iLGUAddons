...
function onPageSubmitGPSFixedAsset(action) {
	if (action=="a" || action=="sc") {
		...
		if (isInputEmpty("u_tftobranch")) return false;
		if (isInputEmpty("u_tftodepartment")) return false;
		if (isInputEmpty("u_tftoempid")) return false;
		if (isInputEmpty("u_tftoempname")) return false;
		if (isInputEmpty("u_tftype")) return false;
				...
				case "u_facode":
					...
					if (result.getAttribute("result")!= "-1") {
						if (parseInt(result.getAttribute("result"))>0) {
							...
							setInput("u_remainlife",result.childNodes.item(0).getAttribute("u_remainlife"));	
							setInput("u_acquidate",formatDateToHttp(result.childNodes.item(0).getAttribute("u_acquidate")));	
							setInput("u_depredate",formatDateToHttp(result.childNodes.item(0).getAttribute("u_depredate")));	
							...
							setInput("u_branch",result.childNodes.item(0).getAttribute("u_branch"));	
							setInput("u_department",result.childNodes.item(0).getAttribute("u_department"));	
							setInput("u_empid",result.childNodes.item(0).getAttribute("u_empid"));	
							setInput("u_empname",result.childNodes.item(0).getAttribute("u_empname"));	
							...
							setInput("u_serialno",result.childNodes.item(0).getAttribute("u_serialno"));	
							setInput("u_mfrserialno",result.childNodes.item(0).getAttribute("u_mfrserialno"));	
							setInput("u_property1",result.childNodes.item(0).getAttribute("u_property1"));	
							setInput("u_property2",result.childNodes.item(0).getAttribute("u_property2"));	
							setInput("u_property3",result.childNodes.item(0).getAttribute("u_property3"));	
							setInput("u_property4",result.childNodes.item(0).getAttribute("u_property4"));	
							setInput("u_property5",result.childNodes.item(0).getAttribute("u_property5"));	
						} else {
							setInput("u_faname","");	
							setInput("u_faclass","");	
							setInputAmount("u_cost",0);	
