		...
		case "T1":
                        if (row==0) {
                            if (isTableInputChecked(table,column)) {
                                    ...
                                    enableTableInput("T1","u_contraglacctno",-1);
                                    enableTableInput("T1","u_cost",-1);
                                    enableTableInput("T1","u_assettype",-1);
                                    enableTableInput("T1","u_assetcode",-1);
                                    enableTableInput("T1","u_profitcenter",-1);
                                    enableTableInput("T1","u_projcode",-1);
                                    if (getPrivate("famgmnt")!="BR") enableTableInput("T1","u_branch",-1);
                                    enableTableInput("T1","u_department",-1);
                                    enableTableInput("T1","u_empid",-1);
                            } else {
                                    uncheckedTableInput(table,column,-1);
                                    disableTableInput("T1","u_contraglacctno",-1);
                                    disableTableInput("T1","u_cost",-1);
                                    disableTableInput("T1","u_assettype",-1);
                                    disableTableInput("T1","u_assetcode",-1);
                                    disableTableInput("T1","u_profitcenter",-1);
                                    disableTableInput("T1","u_projcode",-1);
                                    if (getPrivate("famgmnt")!="BR") disableTableInput("T1","u_branch",-1);
                                    disableTableInput("T1","u_department",-1);
                                    disableTableInput("T1","u_empid",-1);
                            }
                        }else{
                                switch (column) {
                                        case "u_selected":
...
function onElementCFLGetParamsGPSFixedAsset(id,params) {
	switch (id) {
                case "df_employee":
                            ...
                            params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("Employee Code.`Employee Name")); 			
...
//				params["params"] += "&cflsortby=a.u_custno";
                ...
                case "df_department":
                            ...
                            params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("Deparment Code.`Deparment Name")); 			
                            params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("30`70")); 			
                            params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("`")); 			
//				params["params"] += "&cflsortby=a.u_custno";
                            break;
...
function ApplytoGridDepartmentEmployee() {
        showAjaxProcess();
        for (var i = 1; i <= getTableRowCount("T1"); i++) {
            if (isTableRowDeleted("T1",i)==false) {
                    if(getTableInput("T1","u_selected",i)=="1") {
                            setTableInput("T1","u_department",getInput("department"),i);
                            result = ajaxxmlvalidateemployees(getInput("employee"));
                            if (result.getAttribute("result") == '0') {
                                    setTableInput(table,"u_empid","",row);
                                    setTableInput(table,"u_empname","",row);
                                    setStatusMsg('Invalid Employee ID!');
                            ...
                            setTableInput("T1","u_empid",result.childNodes.item(0).getAttribute("empid"),i);
                            setTableInput("T1","u_empname",result.childNodes.item(0).getAttribute("fullname"),i);
                    }
            }
        }
        hideAjaxProcess();
    }
function ApplytoGridPropertyTag() {
	var none=true;
    var proptagno = '';
    if(getTableRowCount("T1")>0) {
            for(i = 1; i <=getTableRowCount("T1"); i++) {
            	if (isTableRowDeleted("T1",i)==false) {
            		if(getTableInput("T1","u_selected",i)=="1"){
                        none = false;
                        proptagno = (getInputNumeric("startproptag") + (i-1)).toString().padStart1(5,'0');
              
                        setTableInput("T1","u_property1",proptagno,i);
                	}
            	}
