		...
		case "u_faacquis":
			if ($actionReturn) {
                            if ($objTable->fields["DOCSTATUS"] != $objTable->docstatus && $objTable->docstatus == "O") {
                                 $actionReturn = onCustomEventPostAcquisitiondocumentschema_brGPSFixedAsset($objTable);
...
function onCustomEventPostAcquisitiondocumentschema_brGPSFixedAsset($objTable){
    global $objConnection;
    global $page;
    $actionReturn = true;
        ...
        $obju_LGUPurchaseDelivery = new documentschema_br(null, $objConnection, "u_lgupurchasedelivery");
...
//					$actionReturn = $objRs->executesql("update u_faxmtal set u_status='A' where company='".$_SESSION["company"]."' and branch='".$obju_FAManAcquiItems->getudfvalue("u_srcbr")."' and lineid='".$obju_FAManAcquiItems->getudfvalue("u_xmtallineid")."'",true);
                    if ($obju_LGUPurchaseDeliveryItems->getbysql("DOCID='" . $obju_FAManAcquiItems->getudfvalue("u_srcline") . "' AND LINEID='" . $obju_FAManAcquiItems->getudfvalue("u_xmtallineid") . "' and company='".$_SESSION["company"]."' and branch='".$obju_FAManAcquiItems->getudfvalue("u_srcbr")."'")) {
                      //  if (!$delete) {
                            $obju_LGUPurchaseDeliveryItems->setudfvalue("u_openquantity", $obju_LGUPurchaseDeliveryItems->getudfvalue("u_openquantity") - 1);
                       // } else {
                       //     $obju_LGUPurchaseDeliveryItems->setudfvalue("u_openquantity", $obju_LGUPurchaseRequestItems->getudfvalue("u_openquantity") + 1);
                       // }
                        if ($obju_LGUPurchaseDeliveryItems->getudfvalue("u_openquantity") <= 0) {
                            $obju_LGUPurchaseDeliveryItems->setudfvalue("u_openquantity", 0);
                            $obju_LGUPurchaseDeliveryItems->setudfvalue("u_linestatus", "C");
                        }
                    ...
                    if ($actionReturn) {
                        if ($obju_LGUPurchaseDelivery->getbykey($obju_FAManAcquiItems->getudfvalue("u_srcdoc"))) {
                            $objRs2 = new recordset(null, $objConnection);
                                $objRs2->queryopen("select SUM(U_OPENQUANTITY) AS U_OPENQUANTITY from u_lgupurchaserequestitems where company='" . $_SESSION["company"] . "' and branch='" . $obju_FAManAcquiItems->getudfvalue("u_srcbr") . "' and docid='$obju_LGUPurchaseDelivery->docid' ");
                            while ($objRs2->queryfetchrow("NAME")) {
                                if ($objRs2->fields["U_OPENQUANTITY"] <= 0) {
                                    $obju_LGUPurchaseDelivery->docstatus = "C";
                                    $actionReturn = $obju_LGUPurchaseDelivery->update($obju_LGUPurchaseDelivery->docno, $obju_LGUPurchaseDelivery->rcdversion);
                                }
                            }
                        }
                    }
