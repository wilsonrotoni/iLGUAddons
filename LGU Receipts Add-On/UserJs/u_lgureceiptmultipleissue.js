...
function onPageSubmitGPSLGUReceipts(action) {
        if (action=="a" || action=="sc") {
            ...
            if (isInputNegative("u_totalnoofreceipt")) return false;
        }
	return true;
}
        ...
        switch (table) {
                    ...
                    switch (column) {
                            case "u_receiptlineid":
                                if (getTableInput(table,column)!="") {
                                    var result = page.executeFormattedQuery("select * from u_lgureceiptitems where lineid='"+getTableInput(table,column)+"' AND U_ISSUEDTO = ''");
                                    if (result.getAttribute("result")!= "-1") {
                                        if (parseInt(result.getAttribute("result"))>0) {
                                            ...
                                            setTableInput(table,"u_noofreceipt",result.childNodes.item(0).getAttribute("u_noofreceipt"));
                                            setTableInput(table,"u_available",result.childNodes.item(0).getAttribute("u_available"));
                                        } else {
                                            setTableInput(table,"u_docseries","");
                                            setTableInput(table,"u_receiptlineid","");
                                            setTableInput(table,"u_receiptfrm","");
                                            setTableInput(table,"u_receiptto","");
                                            setTableInput(table,"u_form","");
                                            setTableInput(table,"u_noofreceipt","");
                                            setTableInput(table,"u_available","");
            ...
            default:
                switch (column) {
                    ...
                    case "u_receiptto":
                        if(getInput(column)!=""){
                            if(column=="u_receiptfrm") var result = page.executeFormattedQuery("select * from u_lgureceiptitems where u_receiptfrm='"+getInput(column)+"' and u_form like '%"+getInput("u_form")+"%' AND U_ISSEDTO = ''");
                            else var result = page.executeFormattedQuery("select * from u_lgureceiptitems where u_receiptto='"+getInput(column)+"' and u_form like '%"+getInput("u_form")+"%' AND U_ISSEDTO = ''");
                            ...
                            if (result.getAttribute("result")!= "-1") {
                                if (parseInt(result.getAttribute("result"))>0) {
                                    ...
                                    setInput("u_noofreceipt",result.childNodes.item(0).getAttribute("u_noofreceipt"));
                                    setInput("u_available",result.childNodes.item(0).getAttribute("u_available"));
                                } else {
                                    ...
                                    setInput("u_form","");
                                    setInput("u_noofreceipt","");
                                    setInput("u_available","");
        ...
        var params = new Array();
            switch (element) {
                case "df_u_receiptfrmT1":
                    params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select u_receiptfrm,u_receiptto,u_noofreceipt from u_lgureceiptitems WHERE U_AVAILABLE > 0 AND U_ISSUEDTO = ''")); 
                ...
                case "df_u_receipttoT1":
                    params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select u_receiptto,u_receiptfrm,u_noofreceipt from u_lgureceiptitems WHERE U_AVAILABLE > 0 AND U_ISSUEDTO = ''")); 
                    params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("To`From`No")); 			
                    params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("20`20`10")); 			
                    params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("``")); 	
                    break;
                case "df_u_receiptlineidT1":
                    params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select lineid,u_receiptfrm,u_receiptto,u_form,u_noofreceipt from u_lgureceiptitems WHERE U_AVAILABLE > 0 AND U_ISSUEDTO = ''")); 
...
function onTableBeforeInsertRowGPSLGUReceipts(table) {
	switch(table){
            case "T1":
                if (isTableInputEmpty(table,"u_available")) return false;
                if (isTableInputEmpty(table,"u_receiptlineid")) return false;
                if (isTableInputNegative(table,"u_available")) return false;
                if (isTableInputNegative(table,"u_receiptlineid")) return false;
                break;
        }
	return true;
}
...
function computeTotalReceipt() {
        var rc =  getTableRowCount("T1"),total = 0;
            for (xxx = 1; xxx <=rc; xxx++) {
                    if (isTableRowDeleted("T1",xxx)==false) {
                            total += getTableInputNumeric("T1","u_available",xxx)
                    }
            }
         setInput("u_totalnoofreceipt",total);
}
