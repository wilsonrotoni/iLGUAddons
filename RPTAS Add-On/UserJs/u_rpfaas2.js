			...
			switch(column) {
				...
				case "u_prevtdno":
					if (getTableInput(table,column)!="") {
						if (column=="u_prevarpno") var result = page.executeFormattedQuery("select * from u_rpfaas2 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and docno='"+getTableInput(table,column)+"' and u_expyear=0");	
						else var result = page.executeFormattedQuery("select * from u_rpfaas2 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_tdno='"+getTableInput(table,column)+"' and u_expyear=0");	
...
//                    alert(column);
			switch(column) {
				...
				case "u_landtdno":
                                    if (getInput(column)!="") {
							var result = page.executeFormattedQuery("select docno,u_ownername,u_tctno,u_surveyno,u_lotno,u_block,u_pin from u_rpfaas1 where u_tdno='"+getInput(column)+"'");	
							if (result.getAttribute("result")!= "-1") {
								if (parseInt(result.getAttribute("result"))>0) {
									setInput("u_landowner",result.childNodes.item(0).getAttribute("u_ownername"));
									...
									setInput("u_block",result.childNodes.item(0).getAttribute("u_block"));
                                                                        if(getInput("docstatus")=="Encoding"|| getInput("docstatus")==""){
                                                                            setInput("u_prefix",result.childNodes.item(0).getAttribute("u_pin"));
									}
                                                                        var result1 = page.executeFormattedQuery("select sum(b.u_sqm) as sqm from u_rpfaas1a a inner join u_rpfaas1b b on a.docid = b.docid and a.company = b.company and a.branch = b.branch where a.u_arpno='"+result.childNodes.item(0).getAttribute("docno")+"' and a.company='"+getGlobal("company")+"' and a.branch='"+getGlobal("branch")+"'");	
                                                                        if (parseInt(result1.getAttribute("result"))>0) {
                                                                            setInput("u_sqm",formatNumericAmount(result1.childNodes.item(0).getAttribute("sqm")));
                                                                        }
									
								} else {
									setInput("u_landowner","");
									setInput("u_tctno","");
									setInput("u_surveyno","");
									setInput("u_lotno","");
									page.statusbar.showError("Invalid Td No. setup is required");	
									return false;
								}
							} else {
                                                                        setInput("u_landowner","");
			...
			switch (column) {
				case "edit":
					if (row==0) {
						if (getInput("docstatus")=="Encoding") {
							var targetObjectId = '';
							OpenLnkBtn(1024,620,'./udo.php?objectcode=u_rpfaas2a' + '' + '&targetId=' + targetObjectId ,targetObjectId);
						} else page.statusbar.showWarning("You can only add new appraisal if status is encoding.");
					} else {
						selectTableRow("T101",row);	
						targetObjectId = 'edit';
						OpenLnkBtn(1050,660,'./udo.php?objectcode=u_rpfaas2a' + '' + '&targetId=' + targetObjectId ,targetObjectId);
					}
					break;
			}
			break;
...
function onElementCFLGetParamsGPSRPTAS(Id,params) {
	switch (Id) {
		case "df_u_prevarpnoT3":
			params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select docno, u_tdno,u_pin, u_ownername from u_rpfaas2 where u_expyear=0")); 
			...
			break;
		case "df_u_prevtdnoT3":
			params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select u_tdno,docno, u_pin, u_ownername from u_rpfaas2 where u_expyear=0")); 
			...
			break;
                case "df_u_ownertin":
                        ...
                        break;
                case "df_u_landtdno":
                        params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select u_tdno,u_pin,u_ownername from u_rpfaas1 where u_tdno <> '' ")); 
                        params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("TaxDec #`PIN`Name")); 			
                        params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("15`15`30")); 			
                        params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("``")); 			
                        break;
	}
	return params;
...
function u_forRecommendGPSRPTAS() {
	...
	if (isInputEmpty("u_assesseddate")) return false;
...
function getPrevFaas(field,value) {
	if (field=='arpno') {
		var result = page.executeFormattedQuery("select * from u_rpfaas2 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and docno='"+value+"'");	
	} else {
		var result = page.executeFormattedQuery("select * from u_rpfaas2 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_pin='"+value+"' order by u_effdate desc");	
	}
	if (result.getAttribute("result")!= "-1") {
		if (parseInt(result.getAttribute("result"))>0) {
			if (result.childNodes.item(0).getAttribute("docstatus")=="Encoding") {
				...
				setInput("u_tctno",result.childNodes.item(0).getAttribute("u_tctno"));
				setInput("u_landtdno",result.childNodes.item(0).getAttribute("u_landtdno"));
				setInput("u_surveyno",result.childNodes.item(0).getAttribute("u_surveyno"));
				setInput("u_lotno",result.childNodes.item(0).getAttribute("u_lotno"));
				setInput("u_block",result.childNodes.item(0).getAttribute("u_block"));
				setInput("u_sqm",result.childNodes.item(0).getAttribute("u_sqm"));
				setInput("u_building",result.childNodes.item(0).getAttribute("u_building"));
				setInput("u_structuretype",result.childNodes.item(0).getAttribute("u_structuretype"));
				setInput("u_cct",result.childNodes.item(0).getAttribute("u_cct"));
				setInput("u_ccidate",formatDateToHttp(result.childNodes.item(0).getAttribute("u_ccidate")));
				setInput("u_coidate",formatDateToHttp(result.childNodes.item(0).getAttribute("u_coidate")));
				setInput("u_floorcount",result.childNodes.item(0).getAttribute("u_floorcount"));
				setInput("u_bpno",result.childNodes.item(0).getAttribute("u_bpno"));
				setInput("u_bpdate",formatDateToHttp(result.childNodes.item(0).getAttribute("u_bpdate")));
				setInput("u_startyear",result.childNodes.item(0).getAttribute("u_startyear"));
				setInput("u_endyear",result.childNodes.item(0).getAttribute("u_endyear"));
				setInput("u_occyear",result.childNodes.item(0).getAttribute("u_occyear"));
				setInput("u_renyear",result.childNodes.item(0).getAttribute("u_renyear"));
