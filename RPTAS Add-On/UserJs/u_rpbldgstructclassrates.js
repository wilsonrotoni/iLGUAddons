...
function onPageSubmitGPSRPTAS(action) {
    if (action=="a" || action=="sc") {
        ...
        if (isInputEmpty("u_structuretype")) return false;
        if (isInputEmpty("u_subclass")) return false;
        if (isInputNegative("u_unitvalue")) return false;
    }
}
	...
	switch (table) {
            default:
                    switch (column) {
                            ...
                            case "u_subclass":
                                    if (getTableInput(table,"u_structuretype")!="" && getTableInput(table,"u_subclass")!="") {
                                            setTableInput(table,"code",getTableInput(table,"u_kind")+" - "+getTableInput(table,"u_structuretype")+" - "+getTableInput(table,"u_subclass") + " - "+getTableInput(table,"u_revisionyear") );
                                            setTableInput(table,"name",getTableInput(table,"u_kind")+" - "+getTableInput(table,"u_structuretype")+" - "+getTableInput(table,"u_subclass") + " - "+getTableInput(table,"u_revisionyear") );
                                    } else {
...
function onTableResetRowGPSRPTAS(table) {
	switch (table) {
		case "T1":
			...
			enableTableInput(table,"u_group");
	...
	switch (table) {
		case "T1":
			if (getTableRowStatus(table,row)!="N") {
				...
				disableTableInput(table,"u_group");
				focusTableInput(table,"u_seqno");
