...
function onElementValidateGPSRPTAS(element,column,table,row) {
		...
		case "T1":
			switch (column) {
                            ...
                            case "u_tdno":
                                if(getInput("searchby")== "L") var result = page.executeFormattedQuery("select docno,u_bilyear,u_trxcode,u_pin,u_barangay,'L' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location from u_rpfaas1 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_varpno='"+getTableInput(table,column)+"' ");
                                else if(getInput("searchby")== "B") var result = page.executeFormattedQuery("Select docno,u_bilyear,u_trxcode,u_pin,u_barangay,'B' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location  from u_rpfaas2 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_varpno='"+getTableInput(table,column)+"'");
                                else var result = page.executeFormattedQuery("Select docno,u_bilyear,u_trxcode,u_pin,u_barangay,'M' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location from u_rpfaas3 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_varpno='"+getTableInput(table,column)+"' ");
                                    if (result.getAttribute("result")!= "-1") {
                                            if (parseInt(result.getAttribute("result"))>0) {
                                                ...
                                                setTableInput(table,"u_bilyear",result.childNodes.item(0).getAttribute("u_bilyear"));
                            ...
                            case "u_docno":
                                ...
                                else var result = page.executeFormattedQuery("Select u_varpno,docno,u_trxcode,u_pin,u_barangay,'M' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location  from u_rpfaas3 where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and docno='"+getTableInput(table,column)+"'");
                                    if (result.getAttribute("result")!= "-1") {
                                        if (parseInt(result.getAttribute("result"))>0) {
                                                if(result.childNodes.item(0).getAttribute("u_kind")=="L"){
                ...
                default :
                    switch(column){
                        ...
                        case "u_orno":
                                var result = page.executeFormattedQuery("select u_date,u_totalamount from u_lgupos where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and docno='"+getInput(column)+"' ");
                                    if (result.getAttribute("result")!= "-1") {
                                            if (parseInt(result.getAttribute("result"))>0) {
                                                setInput("u_ordate",formatDateToHttp(result.childNodes.item(0).getAttribute("u_date")));
                                                setInput("u_oramount",formatNumericAmount(result.childNodes.item(0).getAttribute("u_totalamount")));
                                            }else{
                                                setInput("u_ordate","");
                                                setInput("u_oramount",formatNumericAmount(0));
                                                page.statusbar.showError("Invalid OR #.");	
                                                return false;
                                            }
                                    } else {
                                        setInput("u_ordate","");
        ...
        switch (Id) {
            case "df_u_docnoT1":
                       if(getInput("searchby")=="L"){
                            params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select docno,u_varpno,'Land' as kind, u_pin, u_ownername from u_rpfaas1 where  docstatus in('Assessed','Approved') " ),true); 
                            ...
                            params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select docno,u_varpno,'Building' as kind, u_pin, u_ownername from u_rpfaas2 where  docstatus in('Assessed','Approved') " ),true); 
                            params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("ID No.`ARP No.`Kind`PIN`Declared Owner")); 			
                            params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("15`22`15`15`35")); 			
                            params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("````")); 	
                       }else{
                            params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select docno,u_varpno,'Machinery' as kind, u_pin, u_ownername from u_rpfaas3 where  docstatus in('Assessed','Approved') " ),true); 
                            params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("ID No.`ARP No.`Kind`PIN`Declared Owner")); 			
                            params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("15`22`15`15`35")); 			
                            params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("````")); 	
                       }
            ...
            case "df_u_tdnoT1":
                        if(getInput("searchby")=="L"){
                                params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("Select u_varpno,'Land' as kind, u_pin, u_ownername from u_rpfaas1 where  docstatus in('Assessed','Approved') " ),true); 
                                ...
                                params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("Select u_varpno,'Building' as kind, u_pin, u_ownername from u_rpfaas2 where  docstatus in('Assessed','Approved') " ),true); 
                                params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("ARP No.Kind`PIN`Declared Owner")); 			
                                params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("22`15`15`35")); 			
                                params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("```")); 			
                                break;
                        }else{
                                params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("Select u_varpno,'Machinery' as kind, u_pin, u_ownername from u_rpfaas3 where  docstatus in('Assessed','Approved') " ),true); 
                                params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("ARP No.`Kind`PIN`Declared Owner")); 			
                                params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("22`15`15`35")); 			
                                params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("```")); 			
                                break;
			...
			params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("`")); 			
                        break;
            case "df_u_ornoT1":
			params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("Select docno,u_custname,u_totalamount from u_lgupos where u_profitcenter = 'RP'" ),true); 
			params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("OR No`Customer Name`Total Amount")); 			
			params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("15`50`20")); 			
			params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("``")); 			
                        break;
            case "df_u_orno":
			params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("Select docno,u_custname,u_totalamount from u_lgupos where u_profitcenter = ''" ),true); 
			params["params"] += "&cfltitles="+utils.replaceSpecialChar(Base64.encode("OR No`Customer Name`Total Amount")); 			
			params["params"] += "&cflwidths="+utils.replaceSpecialChar(Base64.encode("15`50`20")); 			
			params["params"] += "&cflformats="+utils.replaceSpecialChar(Base64.encode("``")); 			
                        break;
