...
function onElementValidateGPSRPTAS(element,column,table,row) {
		...
		case "T1":
			switch (column) {
                            ...
                            case "u_tdno":
                                if(getInput("searchby")== "L") var result = page.executeFormattedQuery("SELECT u_ownername,docno,u_trxcode,u_pin,u_barangay,'L' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location,b.name as u_oldbarangay,u_totalareasqm from u_rpfaas1 a left join u_oldbarangays b on a.u_oldbarangay = b.code where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_varpno='"+getTableInput(table,column)+"' and u_cancelled <> 1");
                                else if(getInput("searchby")== "B") var result = page.executeFormattedQuery("SELECT u_ownername,docno,u_trxcode,u_pin,u_barangay,'B' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location,b.name as u_oldbarangay,u_totalareasqm   from u_rpfaas2 a left join u_oldbarangays b on a.u_oldbarangay = b.code where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_varpno='"+getTableInput(table,column)+"' and u_cancelled <> 1");
                                else var result = page.executeFormattedQuery("SELECT u_ownername,docno,u_trxcode,u_pin,u_barangay,'M' as u_kind,u_assvalue,u_marketvalue,if(u_location='',u_barangay,u_location) as u_location,b.name as u_oldbarangay,0 AS u_totalareasqm  from u_rpfaas3 a left join u_oldbarangays b on a.u_oldbarangay = b.code where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_varpno='"+getTableInput(table,column)+"' and u_cancelled <> 1");
                                    if (result.getAttribute("result")!= "-1") {
                                            if (parseInt(result.getAttribute("result"))>0) {
                                                ...
                                                setTableInput(table,"u_declaredowner",result.childNodes.item(0).getAttribute("u_ownername"));
                                                setTableInput(table,"u_location",result.childNodes.item(0).getAttribute("u_oldbarangay"));
                                                setTableInputAmount(table,"u_area",result.childNodes.item(0).getAttribute("u_totalareasqm"));
                                                setTableInputAmount(table,"u_assvalue",result.childNodes.item(0).getAttribute("u_assvalue"));
                                                setTableInputAmount(table,"u_marketvalue",result.childNodes.item(0).getAttribute("u_marketvalue"));
                                            }else{
                                                setTableInput(table,"u_tdno","");
                                                setTableInput(table,"u_class","");
                                                setTableInput(table,"u_docno","");
                                                setTableInput(table,"u_declaredowner","");
                                                setTableInput(table,"u_location","");
                                                setTableInputAmount(table,"u_area",0);
                                                setTableInputAmount(table,"u_assvalue",0);
                                                setTableInputAmount(table,"u_marketvalue",0);
...
function onTableBeforeInsertRowGPSRPTAS(table) {
		...
		case "T1":
			    if (isTableInputEmpty(table,"u_tdno")) return false;
                            if (isTableInputNegative(table,"u_docno")) return false;
            }   
        return true;
}
...
function openupdpays() {
	OpenPopup(1024,600,"./udo.php?&objectcode=u_rpfaas1&sf_keys="+getInput("code")+"&formAction=e","UpdPays");	
}
