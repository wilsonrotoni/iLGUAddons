...
function onAfterDefaultGPSHIS() { 
	...
	if ($page->getitemstring("u_department")!="") {
		if ($page->getitemstring("u_trxtype")=="HEARTSTATION") {
			...
			(IF(B.U_ISSTAT=1,B.U_STATUNITPRICE, B.U_UNITPRICE)*(B.U_QUANTITY-B.U_RTQTY)-(B.U_PRICE*(B.U_QUANTITY-B.U_RTQTY))) AS U_DISCAMOUNT, 
			IF(D.U_RFAMOUNT<>0, D.U_RFAMOUNT,(D.U_RFPERC/100)*(IF(B.U_ISSTAT=1,B.U_STATUNITPRICE, B.U_UNITPRICE)*(B.U_QUANTITY-B.U_RTQTY)-(IF(B.U_ISSTAT=1,B.U_STATUNITPRICE, B.U_UNITPRICE)*(B.U_QUANTITY-B.U_RTQTY)-(B.U_PRICE*(B.U_QUANTITY-B.U_RTQTY))))) AS U_PF
