...
function onCustomEventdocumentschema_brPostCashSalesGPSHIS($objTable,$delete=false,$refund=false) {
		...
		if ($actionReturn) {
			...
			while ($objRsItems->queryfetchrow("NAME")) {
				...
				if ($objTable->getudfvalue("u_trxtype")=="CM") {
					...
					//var_dump(array($objRsItems->fields["U_VATAMOUNT"],$objRsItems->fields["U_NONVATCS"]));
					if ($objTable->getudfvalue("u_scdisc")==1 || $objRsItems->fields["U_NONVATCS"]==1) {
				...
				//var_dump(array($objRsItems->fields["U_NONVATCS"],$objMarketingDocumentItems->linetotal,$objMarketingDocumentItems->vatamount));
	...
	//var_dump(array($objMarketingDocuments->totalbefdisc,$objMarketingDocuments->vatamount));
	...
	if ($actionReturn) {
		...
		//var_dump($objMarketingDocuments->settledamount);
		...
		while ($objRs->queryfetchrow("NAME")) {
			...
			while ($objRs2->queryfetchrow("NAME")) {
				...
				if ($objRs2->fields["u_isinvuom"]=="1") {
					...
					$data["lines"][$lineid]["UNITPRICE"] = $objRs2->fields["u_unitprice"];
...
function onCustomEventdocumentschema_brDailyRoomAndBoardFeesGPSHIS($objTable,$date,$time="00:00:00",$roomtype,$roomtypedesc,$roomrate=0,$roomno) {
	global $objConnection;
	$actionReturn = true;
	...
	$u_amount=$roomrate;
	if ($actionReturn) {
		$objRs->queryopen("select a.u_department, a.u_remarks, b.u_itemcode, b.u_itemdesc, b.u_quantity, b.u_uom, ifnull(c.price,0) as u_unitprice, i.taxcodesa, i.itemgroup from u_hisrooms rm inner join u_hismisctemplates a on a.code=rm.u_miscfees inner join u_hismisctemplateitems b on b.code=a.code inner join items i on i.itemcode=b.u_itemcode left join itempricelists c on c.itemcode=b.u_itemcode and c.pricelist='".$objTable->getudfvalue("u_pricelist")."' where rm.code='$roomno'");
		if ($objRs->recordcount()>0) {
			while ($objRs->queryfetchrow("NAME")) {
				if ($objRs->fields["u_department"]!="") $obju_HISMiscs->setudfvalue("u_department",$objRs->fields["u_department"]);
				if ($objRs->fields["u_remarks"]!="") $obju_HISMiscs->setudfvalue("u_remarks",$objRs->fields["u_remarks"]);
