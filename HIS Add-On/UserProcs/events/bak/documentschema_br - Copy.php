		...
		case "u_hisdietaries":
			if (isset($page)) {
				if ($page->getcolumnstring("T50",0,"userid")!="") {
					$objRs = new recordset(null,$objConnection);
					$objRs->queryopen("select username from users where userid='".$page->getcolumnstring("T50",0,"userid")."' and hpwd='".md5($page->getcolumnstring("T50",0,"password"))."'");
			...
			if ($objTable->docstatus=="O" && $objTable->fields["DOCSTATUS"]=="D") {
				...
				if (isset($page)) {
					if ($page->getcolumnstring("T51",0,"userid")!="") {
						$objRs = new recordset(null,$objConnection);
						$objRs->queryopen("select username from users where userid='".$page->getcolumnstring("T51",0,"userid")."' and hpwd='".md5($page->getcolumnstring("T51",0,"password"))."'");
						if (!$objRs->queryfetchrow()) {
...
function onCustomEventdocumentschema_brPostPKGCMGPSHIS($objTable,$delete=false) {
	...
	if ($delete) {
		...
		if (floatval($objTable->getudfvalue("u_pkghc"))!=0) {
			...
			if ($obju_HISBills->getbykey($objTable->getudfvalue("u_billno"))) {
				if ($obju_HISBillFees->getbysql("DOCID='".$obju_HISBills->docid."' AND U_DOCTORID='' AND U_FEETYPE='Hospital Fees' AND U_NETAMOUNT>0")) {
					$obju_HISPronotes->prepareadd();
					$obju_HISPronotes->docseries = getSeries("u_hispronotes","Credit Memo",$objConnection);
					$obju_HISPronotes->docno = getNextSeriesNoByBranch("u_hispronotes",$obju_HISPronotes->docseries,$objTable->getudfvalue("u_enddate"),$objConnection);
					$obju_HISPronotes->docid = getNextIdByBranch("u_hispronotes",$objConnection);
					...
					$obju_HISPronotes->setudfvalue("u_enddate",$objTable->getudfvalue("u_enddate"));
					$obju_HISPronotes->setudfvalue("u_feetype","Hospital Fees");
					$obju_HISPronotes->setudfvalue("u_amount",$obju_HISBillFees->getudfvalue("u_netamount"));
					if ($obju_HISBillFees->getudfvalue("u_netamount")<=$u_pkghc) {
						$obju_HISPronotes->setudfvalue("u_pnamount",$obju_HISBillFees->getudfvalue("u_netamount"));
						$u_pkghc-=$obju_HISBillFees->getudfvalue("u_netamount");
					} else {
...
function onCustomEventdocumentschema_brLISendFileGPSHIS($objTable) {
	...
	if ($actionReturn) {   //lis
		$objRs = new recordset(null,$objConnection);
		$data = array();
		$objRs->queryopen("select a.u_lastname, a.u_firstname, a.u_middlename, a.u_birthdate, a.u_gender from u_hispatients a where a.company='".$_SESSION["company"]."' and a.branch='".$_SESSION["branch"]."' and a.code='".$objTable->getudfvalue("u_patientid")."'");
		if ($objRs->queryfetchrow("NAME")) {
			...
			array_push($data,"PV1||".iif($objTable->getudfvalue("u_reftype")=="OP","O","I")."||||".""."|".""."\r\n");
			array_push($data,"ORC|NW|".$objTable->docno."|||||||".date('Ymdhis',strtotime($objTable->getudfvalue("u_startdate")." ".$objTable->getudfvalue("u_starttime")))."|".""."\r\n");
...
function onCustomEventdocumentschema_brLISendFile2GPSHIS($objTable) {
	...
	if ($actionReturn) {   //lis
		$objRs = new recordset(null,$objConnection);
		$data = array();
		$objRs->queryopen("select a.u_lastname, a.u_firstname, a.u_middlename from u_hispatients a where a.company='".$_SESSION["company"]."' and a.branch='".$_SESSION["branch"]."' and a.code='".$objTable->getudfvalue("u_patientid")."'");
		if ($objRs->queryfetchrow("NAME")) {
			array_push($data,"MSH|^~\&|HIS|".$_SESSION["companyname"]."|LMS||".date('Ymdhis')."||OMG||P|2.5\r\n");
