...
function onBeforeDefaultGPSHIS() { 
	...
	//$u_patientid = $page->getitemstring("u_patientid");
	...
	$page->privatedata["requesttodepartment"] = $page->getvarstring("requesttodepartment");
	return true;
}
...
function onAfterDefaultGPSHIS() { 
	...
	global $userreftype;
	global $u_trxtype;
	$u_hissetupdata = getu_hissetup("U_DFLTPHARMADEPT,U_DFLTLABDEPT,U_DFLTPHARMATFS,U_DFLTIPD,U_DFLTOPD,U_DFLTRADIODEPT,U_DFLTHEARTSTATIONDEPT,U_STOCKLINK, U_PREPAIDIPLAB, U_PREPAIDOPLAB,U_OPDPRICELIST,U_DFLTOPPAYTERM, U_PHAVATPOS");
	...
	if ($page->getitemstring("u_requestdepartment")!="") {
		...
		$objRs->queryopen("select u_allowintsecreq, u_stocklink from u_hissections where code='".$page->getitemstring("u_requestdepartment")."'");
		if ($objRs->queryfetchrow("NAME")) {
			...
			if ($objRs->fields["u_allowintsecreq"]=="0") {
				...
				$page->setitem("u_stocklink",$objRs->fields["u_stocklink"]);
...
$objGrids[0]->columnwidth("u_nonvatcs",10);
...
$objGrids[0]->columntitle("u_nonvatcs","Non-VAT (CS)");
...
$objGrids[0]->columnattributes("u_nonvatcs","disabled");
