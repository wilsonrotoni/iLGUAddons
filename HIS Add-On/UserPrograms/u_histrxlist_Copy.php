...
//	$objGrid->columnlnkbtn("u_patientid","OpenLnkBtnu_hispatients()");
	...
	*/
	...
	switch ($page->getitemstring("u_doctype")) {
		case "INREQ":		
			$filterExp = "";
			$filterExp = genSQLFilterDate("A.U_DUEDATE",$filterExp,$httpVars['df_u_date']);
		...
		case "REQ":		
			$filterExp = "";
			$filterExp = genSQLFilterDate("A.U_REQUESTDATE",$filterExp,$httpVars['df_u_date']);
		...
		case "CHRG":		
			$filterExp = "";
			$filterExp = genSQLFilterDate("A.U_STARTDATE",$filterExp,$httpVars['df_u_date']);
			$filterExp = genSQLFilterString("A.U_DEPARTMENT",$filterExp,$httpVars['df_u_department']);
			$filterExp = genSQLFilterString("A.U_PATIENTNAME",$filterExp,$httpVars['df_u_patientname'],null,null,true);
			$filterExp = genSQLFilterString("A.DOCSTATUS",$filterExp,$httpVars['df_docstatus']);
			...
			$verifiedExp = "";
			if ($httpVars['df_u_verified']=="0") $verifiedExp = " AND A.U_ENDDATE IS NULL";
			else if ($httpVars['df_u_verified']=="1") $verifiedExp = " AND A.U_ENDDATE IS NOT NULL";

			if ($filterExp !="") $filterExp = " AND " . $filterExp;
		
			$objrs->queryopenext("select A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_REFTYPE, A.U_REFNO, A.U_PREPAID, A.U_PAYREFNO, B.NAME AS U_DEPARTMENTNAME, E.U_ITEMCODE, E.U_ITEMDESC, A.U_PAYMENTTERM, E.U_QUANTITY, E.U_PRICE, E.U_RTQTY, E.U_REMARKS, C.U_TEMPLATE, A.U_AMOUNT, A.DOCSTATUS from U_HISCHARGES A, U_HISCHARGEITEMS E, U_HISSECTIONS B, U_HISITEMS C WHERE E.COMPANY=A.COMPANY AND E.BRANCH=A.BRANCH AND E.DOCID=A.DOCID AND C.CODE=E.U_ITEMCODE AND B.CODE=A.U_DEPARTMENT AND B.U_TYPE IN ('".$page->getitemstring("u_trxtype")."') AND A.COMPANY = '$company' AND A.BRANCH = '$branch' $qtyExp $verifiedExp $filterExp", $objGrid->sortby,"",$objGrid->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
			break;
		case "CM":		
			$filterExp = "";
			$filterExp = genSQLFilterDate("A.U_STARTDATE",$filterExp,$httpVars['df_u_date']);
			...
			$objrs->queryopenext("select A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_REFTYPE, A.U_REFNO, A.U_PREPAID, A.U_PAYREFNO, B.NAME AS U_DEPARTMENTNAME, E.U_ITEMCODE, E.U_ITEMDESC, A.U_PAYMENTTERM, E.U_PRICE, E.U_QUANTITY, E.U_REMARKS, C.U_TEMPLATE, A.U_AMOUNT, A.DOCSTATUS from U_HISCREDITS A, U_HISCREDITITEMS E, U_HISSECTIONS B, U_HISITEMS C WHERE E.COMPANY=A.COMPANY AND E.BRANCH=A.BRANCH AND E.DOCID=A.DOCID AND C.CODE=E.U_ITEMCODE AND B.CODE=A.U_DEPARTMENT AND B.U_TYPE IN ('".$page->getitemstring("u_trxtype")."') AND A.COMPANY = '$company' AND A.BRANCH = '$branch' $qtyExp $filterExp", $objGrid->sortby,"",$objGrid->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
			break;
		case "PC":		
			$filterExp = "";
			$filterExp = genSQLFilterDate("A.U_STARTDATE",$filterExp,$httpVars['df_u_date']);
