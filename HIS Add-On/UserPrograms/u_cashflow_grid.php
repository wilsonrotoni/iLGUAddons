<?php
	...
	$page->paging->formid = "./UDP.php?&objectcode=u_cashflow_grid";
	$page->formid = "./UDO.php?&objectcode=U_YBLIPRS";
	$page->objectname = "Cash Flow Dashboard";
	...
	$objGrid->columntitle("name","Particulars");
	...
	*/
	...
	$sql = "select 'Cash oh Hand' as type, sum(totalamount) as balance,
...
select sum(b.gldebit-b.glcredit) as totalamount, 0 as age from chartofaccounts c
  ...
  ) as x union all select 'A/R' as type, sum(totalamount) as balance,
  sum(if(age = 0,totalamount,0)) as current,
...
<script language="JavaScript">
	...
	function onPageLoad() {
		if (getInput("firstview")=="1") {
			parent.barFrame.location = "../FusionCharts/MySamples/his_cashflow_graph.php?valopt=" + getInput("valopt")  + "&byopt=" + getInput("byopt") + "&custgroup=" + getInput("custgroup")   + "&dbname=" + getGlobal("dbname") + "&companycode=" + getGlobal("company")  + "&branchcode=" + getInput("branchcode");
		}	
	}
