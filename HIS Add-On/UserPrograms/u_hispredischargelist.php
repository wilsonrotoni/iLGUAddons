<?php
	...
	$page->objectcode = "u_hispredischargelist";
	$page->paging->formid = "./UDP.php?&objectcode=u_hispredischargelist";
	...
	$objrs->queryopen("select A.ROLEID AS DEPARTMENT, B.U_TYPE AS TYPE FROM USERS A LEFT OUTER JOIN U_HISSECTIONS B ON B.CODE=A.ROLEID WHERE A.USERID='".$_SESSION["userid"]."'");
	if ($objrs->queryfetchrow("NAME")) {
		if ($objrs->fields["TYPE"]=="NURSE") {
	...
	//$objrs->setdebug();
	$objrs->queryopen("select A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_BEDNO, A.U_ROOMDESC, B.NAME AS DEPARTMENTNAME from U_HISIPS A LEFT OUTER JOIN U_HISSECTIONS B ON B.CODE=A.U_DEPARTMENT WHERE A.COMPANY = '$company' AND A.BRANCH = '$branch' AND A.U_PREDISCHARGE=1 AND A.U_MGH=0 $filterExp" . $page->paging_getlimit());
	...
	$pageHeader = "List of Patient tag as Pre-Discharge"
?>	
