<?php
	...
	$page->objectcode = "U_HISIPLIST";
	$page->paging->formid = "./UDP.php?&objectcode=u_hisiplist";
	...
	$objGrid->columninput("u_chrg","tooltip","Create a Charge Slip");
	$objGrid->columnlnkbtn("u_patientid","OpenLnkBtnu_hispatients()");
	$objGrid->columnlnkbtn("u_req","OpenLnkBtRequests()","Show List of Request/Cash Slip");
	$objGrid->columnlnkbtn("u_chrg","OpenLnkBtCharges()","Show List of Charge Slip");
	...
	*/
	...
	$nurseExp = "";
	if ($page->getitemstring("u_nursed")=="1") $nurseExp==" AND A.U_NURSED=1 ";
	$objrs->queryopenext("select A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_BEDNO, A.U_ROOMDESC, A.U_DEPARTMENT, B.NAME AS U_DEPARTMENTNAME, A.DOCSTATUS, A.U_PAYMENTTERM, A.U_EXPIRED, A.U_MGH, A.U_PREDISCHARGE, A.U_BILLING, A.U_FORCEBILLING, A.U_DISCHARGED, A.U_UNTAGMGHREMARKS, A.U_ALLERGIES, A.U_CONFIDENTIAL from U_HISIPS A LEFT OUTER JOIN U_HISSECTIONS B ON B.CODE=A.U_DEPARTMENT WHERE A.COMPANY = '$company' AND A.BRANCH = '$branch'  $nurseExp $filterExp", $objGrid->sortby,"",$objGrid->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
	//var_dump($objrs->sqls);
	$page->paging_recordcount($objrs->recordcount());
	while ($objrs->queryfetchrow("NAME")) {
		...
		$objGrid->setitem(null,"u_req","?");
		$objGrid->setitem(null,"u_chrg","?");
		...
		switch (table) {
			case "T1":
				switch (column) {
					case "u_req":
						var trxtype = "IP";
						if (getInput("u_nursed")=="1") trxtype = "NURSE";
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;In-Patient Registration - List&nbsp;</td>
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td width="168" ><label <?php genCaptionHtml($schema["u_department"],"") ?>>Nurse Section</label></td>
