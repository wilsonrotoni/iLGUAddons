<?php
	...
	$objGrid->columntitle("u_availablecreditperc","Available %");
	...
	$objGrid->columnwidth("u_availablecreditlimit",14);
	$objGrid->columnwidth("u_availablecreditperc",12);
	...
	*/
	...
	$objrs->queryopenext("select 'OP' AS U_TYPE, A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, '' as U_BEDNO, '' as U_ROOMDESC, B.NAME AS U_DEPARTMENTNAME, A.DOCSTATUS, A.U_PAYMENTTERM, A.U_EXPIRED, 1 as U_MGH, A.U_PREDISCHARGE, A.U_BILLING, A.U_FORCEBILLING, A.U_DISCHARGED, '' as U_UNTAGMGHREMARKS, A.U_ALLERGIES, A.U_CONFIDENTIAL, A.U_CREDITLIMIT, A.U_TOTALCHARGES, A.U_AVAILABLECREDITLIMIT, A.U_AVAILABLECREDITPERC from U_HISOPS A LEFT OUTER JOIN U_HISSECTIONS B ON B.CODE=A.U_DEPARTMENT WHERE A.COMPANY = '$company' AND A.BRANCH = '$branch' AND (A.U_BILLING=1 OR A.U_FORCEBILLING=1) $filterExp union all select 'IP' AS U_TYPE, A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_BEDNO, A.U_ROOMDESC, B.NAME AS U_DEPARTMENTNAME, A.DOCSTATUS, A.U_PAYMENTTERM, A.U_EXPIRED, A.U_MGH, A.U_PREDISCHARGE, A.U_BILLING, A.U_FORCEBILLING, A.U_DISCHARGED, A.U_UNTAGMGHREMARKS, A.U_ALLERGIES, A.U_CONFIDENTIAL, A.U_CREDITLIMIT, A.U_TOTALCHARGES, A.U_AVAILABLECREDITLIMIT, A.U_AVAILABLECREDITPERC from U_HISIPS A LEFT OUTER JOIN U_HISSECTIONS B ON B.CODE=A.U_DEPARTMENT WHERE A.COMPANY = '$company' AND A.BRANCH = '$branch' AND (A.U_BILLING=1 OR A.U_FORCEBILLING=1 OR A.U_BEDNO<>'') $filterExp", $objGrid->sortby,"",$objGrid->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
	//var_dump($objrs->sqls);
	$page->paging_recordcount($objrs->recordcount());
	while ($objrs->queryfetchrow("NAME")) {
		...
		$objGrid->setitem(null,"u_availablecreditperc",formatNumericAmount($objrs->fields["U_AVAILABLECREDITPERC"]));
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  ...
	  <td  align=left>&nbsp;<select <?php genSelectHtml($schema["u_department"],array("loadudflinktable","u_hissections:code:name:u_type in ('ER','OP','NURSE')",":[All]")) ?> ></select></td>
	...
	<tr>
	  ...
	  <td align=left>&nbsp;<input type="radio" <?php genInputRadioHtml($schema["u_mgh"],"") ?> /><label class="lblobjs">All</b></label>&nbsp;<input type="radio" <?php genInputRadioHtml($schema["u_mgh"],"0") ?> /><label class="lblobjs">No</b></label>&nbsp;<input type="radio" <?php genInputRadioHtml($schema["u_mgh"],"1") ?> /><label class="lblobjs">Yes</b></label></td>
	</tr>
	<tr>
	  <td >&nbsp;</td>
