<?php
	$progaccessid = "u_hissplroomrequestinlist";
	...
	$page->paging->formid = "./UDP.php?&objectcode=u_hissplroomrequestinlist&df_u_trxtype=" . $page->getitemstring("u_trxtype");
	$page->formid = "./UDO.php?&objectcode=U_HISSPLROOMS";
	...
	*/
	...
	switch ($page->getitemstring("u_trxtype")) {
		case "XRAY":
			$filterExp = $filterExp .= " C.U_TYPE='X-Ray'";
			break;	
		case "CTSCAN":
			$filterExp = $filterExp .= " C.U_TYPE='CT Scan'";
			break;	
		case "ULTRASOUND":
			$filterExp = $filterExp .= " C.U_TYPE='Ultrasound'";
			break;	
		case "HEARTSTATION":
			$filterExp = $filterExp .= " C.U_TYPE='Heart Station'";
			break;	
		default:
			$filterExp = $filterExp .= " C.U_TYPE='Laboratory'";
			break;
	}
	*/		
	...
	$objrs->queryopen("select A.DOCNO, A.U_PATIENTID, A.U_PATIENTNAME, A.U_REFTYPE, A.U_REFNO, A.U_PREPAID, A.U_PAYREFNO, E.ITEMDESC AS U_ITEMDESC, B.NAME AS DEPARTMENTNAME, D.NAME AS REQUESTDEPARTMENTNAME, A.DOCSTATUS from U_HISSPLROOMREQUESTS A, ITEMS E, U_HISSECTIONS B, U_HISSECTIONS D WHERE E.ITEMCODE=A.U_ITEMCODE AND D.CODE=A.U_REQUESTDEPARTMENT AND B.CODE=A.U_DEPARTMENT AND A.COMPANY = '$company' AND A.BRANCH = '$branch' $filterExp" . $page->paging_getlimit());
			...
			case "T1":
				if (getTableInput("T1","docstatus",p_rowIdx)=="O") {
					if (getTableInput("T1","prepaid",p_rowIdx)=="1" && getTableInput("T1","payrefno",p_rowIdx)=="") {
						...
						var targetObjectId = '';
						OpenLnkBtn(1024,570,'./udo.php?objectcode=u_hissplrooms' + '' + '&df_u_trxtype='+getInput("u_trxtype")+'&targetId=' + targetObjectId ,targetObjectId);
					}	
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<tr>
	  ...
	  <td  align=left><select <?php genSelectHtml($schema["u_department"],array("loadudflinktable","u_hissections:code:name:u_type in ('ER','SPLROOM')",":[All]")) ?> ></select></td>
...
<tr><td>
	<div <?php genTableHDivHtml('HdrT1',false,1000) ?> ><table class="tableBox" style="border-bottom:0px;border-top:0px;border-right:0px;border-left:0px"  width="100%" border="0" cellspacing="0" cellpadding="0">
  	<tr class="tableBoxHead">
		...
		<td width="180">&nbsp;Requesting Section&nbsp;</td>
...
<td ><?php genTableRowStatusHtml("T1r" . $ctr) ?>&nbsp;<?php echo @$objrs->fields["U_ITEMDESC"]?>&nbsp;<input type="hidden" <?php genInputHiddenDFHtml("docstatus","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("docno","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("prepaid","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("payrefno","T1r" . $ctr) ?> ></td>
...
<td width="80">&nbsp;<?php echo $enumdocstatus[$objrs->fields["DOCSTATUS"]] ?>&nbsp;</td>
</tr>
<?php  
