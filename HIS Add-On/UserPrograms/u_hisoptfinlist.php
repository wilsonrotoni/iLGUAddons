<?php
	...
	$page->objectcode = "U_HISOPS";
	$page->paging->formid = "./UDP.php?&objectcode=u_hisoptfinlist";
	$page->formid = "./UDO.php?&objectcode=U_HISOPS";
	$page->objectname = "Out-Patient";
	...
	*/
	...
	$objrs = new recordset(null,$objConnection);
	$objrs->queryopen("select A.DOCNO, A.U_PATIENTID, A.U_PATIENTNAME, B.NAME AS DEPARTMENTNAME, A.DOCSTATUS, A.U_PAYMENTTERM, A.U_BILLNO, A.U_BILLING, A.U_FORCEBILLING from U_HISOPS A, U_HISSECTIONS B WHERE B.CODE=A.U_DEPARTMENT AND A.COMPANY = '$company' AND A.BRANCH = '$branch' $filterExp" . $page->paging_getlimit());
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;Out-Patient Transfer - Incoming List&nbsp;</td>
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<tr>
	  ...
	  <td  align=left><select <?php genSelectHtml($schema["u_department"],array("loadudflinktable","u_hissections:code:name:u_type in ('OP','ER','DIALYSIS')",":[All]")) ?> ></select></td>
...
<tr><td>
	<div <?php genTableHDivHtml('HdrT1',false,1000) ?> ><table class="tableBox" style="border-bottom:0px;border-top:0px;border-right:0px;border-left:0px"  width="100%" border="0" cellspacing="0" cellpadding="0">
  	<tr class="tableBoxHead">
		...
		<td width="120">&nbsp;Payment Term&nbsp;</td>
		<td width="120">&nbsp;w/ Billing&nbsp;</td>
		<td width="120">&nbsp;Bill No.&nbsp;</td>
        <td >&nbsp;Status&nbsp;</td>
  	</tr>
	</table></div>  
...
<?php  
	...
	while ($objrs->queryfetchrow('NAME')) {
		...
		$httpVars["df_u_billnoT1r" . $ctr] = $objrs->fields["U_BILLNO"];
		$httpVars["df_u_billingT1r" . $ctr] = $objrs->fields["U_BILLING"];
		$httpVars["df_u_forcebillingT1r" . $ctr] = $objrs->fields["U_FORCEBILLING"];
...
<td width="120"><?php genTableRowStatusHtml("T1r" . $ctr) ?>&nbsp;<?php echo @$objrs->fields["U_PATIENTID"]?>&nbsp;<input type="hidden" <?php genInputHiddenDFHtml("docno","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("u_patientid","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("u_patientname","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("u_billno","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("u_billing","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("u_forcebilling","T1r" . $ctr) ?> ></td>
<td width="240">&nbsp;<?php echo $objrs->fields["U_PATIENTNAME"] ?>&nbsp;</td>
<td width="120">&nbsp;<?php echo $objrs->fields["U_PAYMENTTERM"] ?>&nbsp;</td>
<td width="120">&nbsp;<?php echo iif($objrs->fields["U_BILLING"]==1 || $objrs->fields["U_FORCEBILLING"]==1,"Yes","No") ?>&nbsp;</td>
<td width="120">&nbsp;<?php echo $objrs->fields["U_BILLNO"] ?>&nbsp;</td>
