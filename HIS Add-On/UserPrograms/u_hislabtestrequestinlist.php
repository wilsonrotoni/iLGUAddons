<?php
	$progid = "u_hislabtestrequestinlist";
	...
	$page->paging->formid = "./UDP.php?&objectcode=u_hislabtestrequestinlist&df_u_trxtype=" . $page->getitemstring("u_trxtype");
	...
	*/
	...
	$objrs->queryopen("select A.DOCNO, A.U_PATIENTID, A.U_PATIENTNAME, A.U_REFTYPE, A.U_REFNO, A.U_PREPAID, A.U_PAYREFNO, B.NAME AS DEPARTMENTNAME, D.NAME AS REQUESTDEPARTMENTNAME, E.U_TYPE, E.U_RENDERED, A.U_RENDERED AS U_PARTIAL, C.NAME AS U_TYPEDESC, C.U_TEMPLATE, A.DOCSTATUS from U_HISLABTESTREQUESTS A, U_HISLABTESTREQUESTTESTS E, U_HISSECTIONS B, U_HISITEMS C, U_HISSECTIONS D WHERE E.COMPANY=A.COMPANY AND E.BRANCH=A.BRANCH AND E.DOCID=A.DOCID AND D.CODE=A.U_REQUESTDEPARTMENT AND C.CODE=E.U_TYPE AND B.CODE=A.U_DEPARTMENT AND B.U_TYPE='".iif($page->getitemstring("u_trxtype")=="","LABORATORY",$page->getitemstring("u_trxtype"))."' AND A.COMPANY = '$company' AND A.BRANCH = '$branch' AND A.DOCSTATUS NOT IN ('CN') $filterExp" . $page->paging_getlimit());
...
<script language="JavaScript">
			...
			case "T1":
				if (getTableInput("T1","docstatus",p_rowIdx)=="O") {
					if (getTableInput("T1","prepaid",p_rowIdx)=="1" && getTableInput("T1","payrefno",p_rowIdx)=="") {
						...
						var targetObjectId = '';
						OpenLnkBtn(1024,570,'./udo.php?objectcode=u_hislabtests' + '' + '&df_u_trxtype='+getInput("u_trxtype") + '&targetId=' + targetObjectId ,targetObjectId);
					}	
	...
	function OpenLnkBtnu_hislabtestrequests(targetId) {
		OpenLnkBtn(1024,600,'./udo.php?objectcode=u_hislabtestrequests' + '' + '&targetId=' + targetId ,targetId);
...
<td >&nbsp;<?php echo iif($objrs->fields["U_RENDERED"]==1,"Yes","No") ?>&nbsp;</td>
</tr>
<?php  
