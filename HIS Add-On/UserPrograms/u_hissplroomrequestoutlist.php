<?php
	$progid = "u_hissplroomrequestoutlist";
	...
	$page->objectcode = "U_HISMISCS";
	$page->paging->formid = "./UDP.php?&objectcode=u_hissplroomrequestoutlist";
	$page->formid = "./UDO.php?&objectcode=U_HISSPLROOMS";
	$page->objectname = "Miscellaneuos";
		...
		switch (p_tableId) {
			case "T1":
				//if (getTableInput("T1","docstatus",p_rowIdx)=="O") {
					var targetObjectId = 'u_hismiscrequests';
					OpenLnkBtn(1024,600,'./udo.php?objectcode=u_hissplroomrequests' + '' + '&targetId=' + targetObjectId ,targetObjectId);
...
<td ><?php genTableRowStatusHtml("T1r" . $ctr) ?>&nbsp;<?php echo @$objrs->fields["U_ITEMDESC"]?>&nbsp;<input type="hidden" <?php genInputHiddenDFHtml("docstatus","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("docno","T1r" . $ctr) ?> ></td>
...
<td width="80">&nbsp;<?php echo iif($objrs->fields["DOCSTATUS"]=="O" && $objrs->fields["U_PARTIAL"]==1,"Partial",$enumdocstatus[$objrs->fields["DOCSTATUS"]]) ?>&nbsp;</td>
</tr>
<?php  
