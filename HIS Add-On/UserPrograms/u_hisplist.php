<?php
	...
	*/
	...
	while ($objrs->queryfetchrow("NAME")) {
		...
		$objGrid->setitem(null,"u_req",iif($objrs->fields["U_REQCNT"]>0,"Ca",""));
		$objGrid->setitem(null,"u_chrg",iif($objrs->fields["U_CHRGCNT"]>0,"Ch",""));
		$objGrid->setitem(null,"u_cm",iif($objrs->fields["U_CMCNT"]>0,"Cr",""));
		if ($page->getitemstring("u_reftype")=="IP") {
...
<script language="JavaScript">
					...
					case "u_req":
						if (getTableInput(table,"u_mgh",row)=="0") {
							if (getTableInput(table,"u_billno",row)=="") {
								...
								OpenLnkBtn(1024,600,'./udo.php?objectcode=U_HISREQUESTS' + '&df_u_trxtype='+getInput("u_trxtype")+ '&targetId=' + targetObjectId ,targetObjectId);
							} else {
						...
						if (getTableInput(table,"u_prepaid",row)=="0" || getInput("u_trxtype")=="BILLING") {
							if (getTableInput(table,"u_mgh",row)=="0" || getInput("u_trxtype")=="BILLING") {
								if (getTableInput(table,"u_billno",row)=="") {
									...
									OpenLnkBtn(1024,600,'./udo.php?objectcode=U_HISCHARGES' + '&df_u_trxtype='+getInput("u_trxtype") + '&targetId=' + targetObjectId ,targetObjectId);
								} else {
					...
					case "u_cm":
						if (getTableInput(table,"u_mgh",row)=="0" || getInput("u_trxtype")=="BILLING") {
							if (getTableInput(table,"u_billno",row)=="") {
								...
								OpenLnkBtn(1024,600,'./udo.php?objectcode=U_HISCREDITS' + '&df_u_trxtype='+getInput("u_trxtype") + '&targetId=' + targetObjectId ,targetObjectId);
							} else {
	...
	function OpenLnkBtnCreditDocNo(targetId) {
		OpenLnkBtn(1024,600,'./udo.php?objectcode=u_hiscredits' + '&targetId=' + targetId ,targetId);
	}	
	
	function showDietaryGPSHIS(meal) {
		OpenLnkBtn(1024,600,'./udo.php?objectcode=u_hisdietaries' + '&df_u_meal='+meal+'&targetId=' + targetId ,targetId);
	}
