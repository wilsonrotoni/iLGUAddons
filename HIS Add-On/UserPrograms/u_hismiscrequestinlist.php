<?php
	$progaccessid = "u_hismiscrequestinlist";
	...
	$page->paging->formid = "./UDP.php?&objectcode=u_hismiscrequestinlist&df_u_trxtype=" . $page->getitemstring("u_trxtype");
			...
			case "T1":
				if (getTableInput("T1","docstatus",p_rowIdx)=="O") {
					if (getTableInput("T1","prepaid",p_rowIdx)=="1" && getTableInput("T1","payrefno",p_rowIdx)=="") {
						...
						var targetObjectId = '';
						OpenLnkBtn(1024,570,'./udo.php?objectcode=u_hismiscs' + '' + '&df_u_trxtype='+getInput("u_trxtype")+'&targetId=' + targetObjectId ,targetObjectId);
					}	
...
<td ><?php genTableRowStatusHtml("T1r" . $ctr) ?>&nbsp;<?php echo @$objrs->fields["U_REMARKS"]?>&nbsp;<input type="hidden" <?php genInputHiddenDFHtml("docstatus","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("docno","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("prepaid","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("payrefno","T1r" . $ctr) ?> ></td>
