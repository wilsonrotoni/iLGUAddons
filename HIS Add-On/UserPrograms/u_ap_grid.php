<?php
	...
	include_once("./sls/suppliergroups.php");
	...
	$page->objectcode = "A/P";
	$page->paging->formid = "./UDP.php?&objectcode=u_ap_grid";
	$page->formid = "./UDO.php?&objectcode=U_YBLIPRS";
	$page->objectname = "A/P Dashboard";
	...
	if ($page->getitemstring("byopt")=="group") {
		$objGrid->columntitle("name","Supplier Group");
	} else {
		$objGrid->columntitle("name","Supplier");
	}
	...
	*/
	...
	$custgroupExp = "";
	if ($custgroup!="") $custgroupExp = " and c.suppgroup='$custgroup'";
	...
	$sql = "select bpcode, bpname, custgroupdesc, sum(totalamount) as balance,
...
select a.bpcode, a.bpname, c.suppgroup as custgroup, d.groupname as custgroupdesc, a.docdate, a.balanceamount*-1 as totalamount, datediff('$date', date_format(a.docdate, '%y-%m-%d')) as age from payments a
  ...
  where a.company='".$_SESSION["company"]."' and a.branchcode='".$_SESSION["branch"]."' and a.collfor='DP' and a.docstat='O'
  ) as x group by $byoptExp order by $sortExp";
	...
	while ($objrs->queryfetchrow("NAME")) {
		...
		$objGrid->setitem(null,"name",$objrs->fields[$byoptExp]);
...
<script language="JavaScript">
	...
	function onPageLoad() {
		if (getInput("firstview")=="1") {
			parent.barFrame.location = "../FusionCharts/MySamples/his_ap_graph.php?valopt=" + getInput("valopt")  + "&byopt=" + getInput("byopt") + "&custgroup=" + getInput("custgroup")   + "&dbname=" + getGlobal("dbname") + "&companycode=" + getGlobal("company")  + "&branchcode=" + getInput("branchcode");
		}	
	}
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  ...
	  <td  align=left><input type="radio" <?php genInputRadioHtml($schema["byopt"],"customer") ?> /><label <?php genOptionCaptionHtml($schema["byopt"],"") ?> >Supplier</label>&nbsp;<input type="radio" <?php genInputRadioHtml($schema["byopt"],"group") ?> /><label <?php genOptionCaptionHtml($schema["byopt"],"") ?> >Supplier Group</label>&nbsp;<select <?php genSelectHtml($schema["custgroup"],array("loadsuppliergroups","",":[All]")) ?> ></select></td>
	  </tr>
	</table></td></tr>	
<tr class="fillerRow10px"><td></td></tr>
