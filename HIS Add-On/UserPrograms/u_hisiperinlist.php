<?php
	...
	$objrs->queryopen("select A.DOCNO, A.U_STARTDATE, A.U_STARTTIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_BEDNO, A.U_ROOMDESC, B.NAME AS DEPARTMENTNAME, A.DOCSTATUS from U_HISIPS A LEFT OUTER JOIN U_HISSECTIONS B ON B.CODE=A.U_DEPARTMENT WHERE A.COMPANY = '$company' AND A.BRANCH = '$branch' AND A.U_NURSED=0 AND (A.U_WEIGHT_KG=0 OR A.U_HEIGHT_CM=0 OR A.U_VSDATE IS NULL) $filterExp" . $page->paging_getlimit());
