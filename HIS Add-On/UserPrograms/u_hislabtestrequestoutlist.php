<?php
	$progid = "u_hislabtestrequestoutlist";
	...
	$page->paging->formid = "./UDP.php?&objectcode=u_hislabtestrequestoutlist";
	...
	*/
	...
	if ($objrs->queryfetchrow("NAME")) {
		...
		if ($userdepartment!="") $schema["u_requestdepartment"]["attributes"] = "disabled";
	}

	if (!isset($httpVars['df_docstatus'])) {
		$page->setitem("docstatus","O");
		$page->setitem("u_requestdepartment",$userdepartment);
	}
	$filterExp = "";
	...
	//$objrs->setdebug();
	$objrs->queryopen("select A.DOCNO, A.U_PATIENTID, A.U_PATIENTNAME, A.U_RENDERED AS U_PARTIAL, A.U_REFTYPE, A.U_REFNO, A.U_PREPAID, A.U_PAYREFNO, B.NAME AS DEPARTMENTNAME, D.NAME AS REQUESTDEPARTMENTNAME, E.U_RENDERED, C.NAME AS U_TYPEDESC, A.DOCSTATUS from U_HISLABTESTREQUESTS A,U_HISLABTESTREQUESTTESTS E, U_HISSECTIONS B, U_HISLABTESTTYPES C, U_HISSECTIONS D WHERE E.COMPANY=A.COMPANY AND E.BRANCH=A.BRANCH AND E.DOCID=A.DOCID AND D.CODE=A.U_REQUESTDEPARTMENT AND C.CODE=E.U_TYPE AND B.CODE=A.U_DEPARTMENT AND A.COMPANY = '$company' AND A.BRANCH = '$branch' $filterExp" . $page->paging_getlimit());
	//var_dump($objrs->sqls);
		...
		switch (p_tableId) {
			case "T1":
				//if (getTableInput("T1","docstatus",p_rowIdx)=="O") {
					var targetObjectId = 'u_hislabtestrequests';
					OpenLnkBtn(1024,600,'./udo.php?objectcode=u_hislabtestrequests' + '' + '&targetId=' + targetObjectId ,targetObjectId);
...
<tr><td><table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td class="labelPageHeader" >&nbsp;X-Ray/Laboratory/Diagnostic Requests - Outgoing List&nbsp;</td>
		<td><input type="hidden" id="df_u_trxtype" name="df_u_trxtype" value="<?php echo $page->getitemstring("u_trxtype");  ?>">
			<input type="hidden" name="firstvisualrec" value="<?php echo $firstvisualrec;  ?>">
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<tr>
	  <td width="168" ><label <?php genCaptionHtml($schema["u_requestdepartment"],"") ?>>Requesting Department</label></td>
	  <td  align=left><select <?php genSelectHtml($schema["u_requestdepartment"],array("loadudflinktable","u_hissections:code:name:u_type in ('".$page->getitemstring("u_trxtype")."')",":[All]")) ?> ></select></td>
...
<td width="160"><?php genTableRowStatusHtml("T1r" . $ctr) ?>&nbsp;<?php echo @$objrs->fields["U_TYPEDESC"]?>&nbsp;<input type="hidden" <?php genInputHiddenDFHtml("docstatus","T1r" . $ctr) ?> ><input type="hidden" <?php genInputHiddenDFHtml("docno","T1r" . $ctr) ?> ></td>
...
<td width="80">&nbsp;<?php echo iif($objrs->fields["U_RENDERED"]==1,"Yes","No") ?>&nbsp;</td>
<td >&nbsp;<?php echo iif($objrs->fields["DOCSTATUS"]=="O" && $objrs->fields["U_PARTIAL"]==1,"Partial",$enumdocstatus[$objrs->fields["DOCSTATUS"]]) ?>&nbsp;</td>
</tr>
<?php  
