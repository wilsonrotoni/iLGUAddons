<?php
	...
	$page->paging->formid = "./UDP.php?&objectcode=u_hisrequestlist";
	...
	$objGrid->columntitle("u_duetime","Due Time");
	...
	$objGrid->columnwidth("u_duetime",8);
...
//	$objGrid->columnlnkbtn("u_patientid","OpenLnkBtnu_hispatients()");
	...
	if ($httpVars['df_docstatus']=="C" || $httpVars['df_docstatus']=="") $qtyExp = "";
	...
	*/
	...
	$objrs->queryopenext("select A.DOCNO, A.U_REQUESTDATE, A.U_REQUESTTIME, A.U_DUEDATE, A.U_DUETIME, A.U_PATIENTID, A.U_PATIENTNAME, A.U_REFTYPE, A.U_REFNO, A.U_PREPAID, A.U_PAYREFNO, B.NAME AS U_DEPARTMENTNAME, D.NAME AS U_REQUESTDEPARTMENTNAME, E.U_ITEMCODE, E.U_ITEMDESC, E.U_QUANTITY, E.U_CHRGQTY, E.U_RTQTY, C.U_TEMPLATE, A.U_AMOUNT, A.DOCSTATUS from U_HISREQUESTS A, U_HISREQUESTITEMS E, U_HISSECTIONS B, U_HISITEMS C, U_HISSECTIONS D WHERE E.COMPANY=A.COMPANY AND E.BRANCH=A.BRANCH AND E.DOCID=A.DOCID AND D.CODE=A.U_REQUESTDEPARTMENT AND C.CODE=E.U_ITEMCODE AND B.CODE=A.U_DEPARTMENT AND A.U_TRXTYPE IN ('".$page->getitemstring("u_trxtype")."') AND A.COMPANY = '$company' AND A.BRANCH = '$branch' AND A.DOCSTATUS NOT IN ('CN') $qtyExp $filterExp", $objGrid->sortby,"",$objGrid->sortas,$page->paging->currentrow,$page->paging->numperpage,"");
	//var_dump($objrs->sqls);
	$page->paging_recordcount($objrs->recordcount());
...
<tr><td><table class="tableFreeForm" width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
	  <td width="168" ><label <?php genCaptionHtml($schema["u_requestdepartment"],"") ?>>Section</label></td>
