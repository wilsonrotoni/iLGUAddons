      ...
      from (SELECT a.company,a.branch,a.objectcode, a.docno, sum(b.amount - b.penaltypaid + b.discamount) as BALANCE, c.VALUEDATE AS DOCDATE, c.docno as paydocno
              from arinvoices a, collectionsinvoices b, collections c
                where b.company = a.company
                  ...
                  and a.docdate <= pi_date2
                  and c.cleared <> 0
                  and (c.docstat not in ('CN','BC','D') or (c.docstat = 'CN' and c.pdc=0 and c.cancelleddate > pi_date2) or (c.docstat = 'BC' and c.cancelleddate > pi_date2) )
                  and c.valuedate <= pi_date2 and c.valuedate<>'0000-00-00'
                  and (pi_custno='' or (pi_custno<>'' and a.bpcode=pi_custno))
                group by a.DOCNO
            union all
              SELECT a.company,a.branch,a.objectcode, a.docno, sum(b.amount*-1) as BALANCE, c.DOCDATE, c.docno as paydocno
                from arinvoices a, paymentinvoices b, payments c
                  where b.company = a.company
                    ...
                    and a.docstatus in ('O','C')
                    and a.docdate <= pi_date2
                    and (c.docstat not in ('CN','D') or (c.docstat in ('CN') and c.cancelleddate > pi_date2) )
    ...
    SELECT company,branchcode,objectcode,docno, sum(BALANCE), DOCDATE, PAYDOCNO, 'BANKING'
      from (SELECT a.company,a.branchcode,a.objectcode,a.docno, sum(b.amount - b.penaltypaid + b.discamount) as BALANCE, c.DOCDATE, c.docno as PAYDOCNO
              ...
              where b.company = a.company
                ...
                and a.doctype = 'C' 
                and c.cleared <> 0
                and (c.docstat not in ('CN','D') or (c.docstat = 'CN' and c.cancelleddate > pi_date2))
...
INSERT
    ...
    SELECT arinvoices.COMPANY, arinvoices.BRANCH, arinvoices.OBJECTCODE, arinvoices.DOCNO,
        ...
        and datediff(pi_date2, date_format(arinvoices.DOCDUEDATE, '%y-%m-%d')) > v_MONTH_END
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date2, date_format(arinvoices.DOCDUEDATE, '%y-%m-%d')) >= 91
...
INSERT
    ...
    SELECT journalvoucheritems.COMPANY, journalvoucheritems.BRANCH, journalvoucheritems.OBJECTCODE, journalvoucheritems.DOCNO,
        ...
        and datediff(pi_date2, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d')) > v_MONTH_END
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date2, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d')) >= 91
        ...
        WHERE journalvouchers.company = pi_company 
          ...
          AND journalvouchers.docdate <= pi_date2
...
INSERT
    ...
    SELECT arcreditmemos.COMPANY, arcreditmemos.BRANCH, arcreditmemos.OBJECTCODE, arcreditmemos.DOCNO,
        ...
        and datediff(pi_date2, date_format(arcreditmemos.DOCDUEDATE, '%y-%m-%d')) > v_MONTH_END
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date2, date_format(arcreditmemos.DOCDUEDATE, '%y-%m-%d')) >= 91
...
INSERT
    ...
    SELECT collections.COMPANY, collections.BRANCHCODE, collections.OBJECTCODE, collections.DOCNO,
        ...
        case when datediff(pi_date2, date_format(collections.DOCDUEDATE, '%y-%m-%d')) <=0
        and datediff(pi_date2, date_format(collections.DOCDUEDATE, '%y-%m-%d')) > v_MONTH_END
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date2, date_format(collections.DOCDUEDATE, '%y-%m-%d'))
        between 1 and 15 then (collections.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D7',
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date2, date_format(collections.DOCDUEDATE, '%y-%m-%d')) >= 91
...
and (collections.docstat not in ('CN','BC','D') or (collections.docstat = 'CN' and collections.pdc=0 and collections.cancelleddate > pi_date2) or (collections.docstat = 'BC' and collections.cancelleddate > pi_date2) )
        AND collections.valuedate <= pi_date2 AND collections.valuedate <> '0000-00-00'
...
INSERT
    ...
    SELECT payments.COMPANY, payments.BRANCHCODE, payments.OBJECTCODE, payments.DOCNO,
        ...
        case when datediff(pi_date2, date_format(payments.DOCDUEDATE, '%y-%m-%d')) <=0
        and datediff(pi_date2, date_format(payments.DOCDUEDATE, '%y-%m-%d')) > v_MONTH_END
        ...
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date2, date_format(payments.DOCDUEDATE, '%y-%m-%d'))
        ...
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date2, date_format(payments.DOCDUEDATE, '%y-%m-%d')) >= 91
...
and (payments.docstat not in ('BC','D') or (payments.docstat = 'CN' and payments.cancelleddate > pi_date2))
        AND payments.docdate<= pi_date2
...
INSERT
    ...
    SELECT collections.COMPANY, collections.BRANCHCODE, collections.OBJECTCODE, collections.DOCNO,
        ...
        WHERE collectionsinvoices.company = arinvoices.company
          ...
          and arinvoices.docdate > pi_date2
...
INSERT
    ...
    SELECT journalvouchers.COMPANY,
        ...
        if(journalvouchers.u_hispatientid = '' or journalvouchers.u_hispatientid is null, concat(journalvoucheritems.itemno, ' - ', journalvoucheritems.itemname), concat(journalvouchers.u_hispatientid, ' - ', journalvouchers.u_hispatientname)) as BPNAME,
        '' as BILLTOADDRESS,
        '' as ITEMCODE,
...
INSERT
    ...
    SELECT collections.COMPANY,
        ...
        0-(concatfield.DUEAMOUNT),
        datediff(pi_date2, date_format(collections.DOCDUEDATE, '%y-%m-%d')) AS aging,
        date(pi_date2) - interval 0 month as 1month,
        date(pi_date2) - interval 1 month as 2month,
        date(pi_date2) - interval 2 month as 3month,
        0-(concatfield.advpay),
...
INSERT
    ...
    SELECT payments.COMPANY,
        ...
        (concatfield.DUEAMOUNT),
        datediff(pi_date2, date_format(payments.DOCDUEDATE, '%y-%m-%d')) AS aging,
        date(pi_date2) - interval 0 month as 1month,
        date(pi_date2) - interval 1 month as 2month,
        date(pi_date2) - interval 2 month as 3month,
        (concatfield.advpay),
        ...
        WHERE payments.PAIDAMOUNT > 0 AND payments.collfor in('DP')
         and (payments.docstat not in ('CN','D') or (payments.docstat = 'CN' and payments.cancelleddate > pi_date2) or (payments.docstat = 'BC' and payments.cancelleddate > pi_date2) )
        AND payments.docdate <= pi_date2
        AND payments.doctype = 'C'
...
call sp_processtime('end','');
  ...
  SELECT
    ...
    sum(a.AMOUNT) - sum(a.DUEAMOUNT) as PAYMENT,
    sum(a.DUEAMOUNT) as OUTSTANDING_BALANCE,
    a.AGING,
    SUM(a.DUEAMOUNT-(a.D7+a.D15+a.D30+a.UP30)) as CURRENT,
    0 as NEXT_MONTHS,
    sum(a.D7) as '1-15',
    sum(a.D15) as '16-30',
    sum(a.D30 + a.UP30)  as 'OVER30',
    a.BPREFNO
