...
DROP PROCEDURE IF EXISTS `ar_aging_new3_w_0_bal` $$
CREATE PROCEDURE `ar_aging_new3_w_0_bal`(
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company, branch, objectcode, docno, sum(BALANCE), DOCDATE, paydocno, 'BANKING'
      from (SELECT a.company,a.branch,a.objectcode,d.docno, sum(b.amount - b.penaltypaid) as BALANCE, c.VALUEDATE AS DOCDATE, c.docno as paydocno
...
INSERT
  INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
    SELECT company,branchcode,objectcode,docno, sum(BALANCE), DOCDATE, PAYDOCNO, 'BANKING'
      from (SELECT a.company,a.branchcode,a.objectcode,a.docno, if(a.collfor = 'DP',sum(b.amount - b.penaltypaid)*-1, sum(b.amount - b.penaltypaid)) as BALANCE, c.VALUEDATE AS DOCDATE, c.docno as paydocno
...
call sp_processtime('A/R Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branch,a.objectcode,a.docno, 0 - sum(b.amount - b.penaltypaid) as advancepayment, c.VALUEDATE AS DOCDATE
        from arinvoices a, collectionsinvoices b, collections c
        where b.company = a.company
        and b.branch = a.branch
        and b.refno = a.docno
...
call sp_processtime('Journal Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branch,a.objectcode,d.docno, 0 - sum(b.amount - b.penaltypaid) as advancepayment, c.VALUEDATE AS DOCDATE
        from journalvouchers a, journalvoucheritems d, collectionsinvoices b, collections c
...
call sp_processtime('Advances Advance Payments','start');
  INSERT
    INTO adv (COMPANY,BRANCH,OBJECTCODE,DOCNO,advancepayment,DOCDATE)
        SELECT a.company,a.branchcode,a.objectcode,a.docno, sum(b.amount - b.penaltypaid) as advancepayment, c.VALUEDATE AS DOCDATE
        from payments a, collectionsinvoices b, collections c
        where b.company = a.company
        and b.branch = a.branchcode
        and b.refno = a.docno
...
INSERT
    ...
    SELECT journalvoucheritems.COMPANY, journalvoucheritems.BRANCH, journalvoucheritems.OBJECTCODE, journalvoucheritems.DOCNO,
        ...
        between 1 and 15 then ((journalvoucheritems.debit-journalvoucheritems.credit) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D7',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d'))
        between 16 and 30 then ((journalvoucheritems.debit-journalvoucheritems.credit) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D15',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d'))
        between 31 and 45 then ((journalvoucheritems.debit-journalvoucheritems.credit) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
...
INSERT
    ...
    SELECT collections.COMPANY, collections.BRANCHCODE, collections.OBJECTCODE, collections.DOCNO,
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(collections.DOCDUEDATE, '%y-%m-%d'))
        between 16 and 30 then (collections.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D15',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(collections.DOCDUEDATE, '%y-%m-%d'))
        between 31 and 45 then (collections.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(collections.DOCDUEDATE, '%y-%m-%d')) >= 46
...
INSERT
    ...
    SELECT payments.COMPANY, payments.BRANCHCODE, payments.OBJECTCODE, payments.DOCNO,
        ...
        between 1 and 15 then (payments.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.DOCDUEDATE, '%y-%m-%d'))
        between 16 and 30 then (payments.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else 0.00 end,
        0) as 'D15',
        if((payments.PAIDAMOUNT -
        ...
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.DOCDUEDATE, '%y-%m-%d'))
        between 31 and 45 then (payments.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE)) else 0.00 end,
        0) as 'D30',
        if((payments.PAIDAMOUNT -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE))>0,
        case when datediff(pi_date, date_format(payments.DOCDUEDATE, '%y-%m-%d')) >= 46
...
INSERT
    INTO concatfield (COMPANY,BRANCH,OBJECTCODE, DOCNO,DUEAMOUNT,advpay,current,D7,D15,D30,UP30,AMOUNT)
    SELECT collections.COMPANY, collections.BRANCHCODE, collections.OBJECTCODE, collections.DOCNO,
        sum(collectionsinvoices.amount - collectionsinvoices.penaltypaid) as DUEAMOUNT,
        0 AS advpay,
        sum(collectionsinvoices.amount - collectionsinvoices.penaltypaid) as 'current',
        0 as 'D7',
        0 as 'D15',
        0 as 'D30',
        0 as 'UP30',
        sum(collectionsinvoices.amount)
