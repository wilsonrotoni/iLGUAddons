...
DROP PROCEDURE IF EXISTS `ap_aging_new3` $$
CREATE PROCEDURE `ap_aging_new3`(
...
call sp_processtime('A/P Journals','start');
  ...
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
        SELECT a.company,a.branch,a.objectcode,a.docno, sum(b.credit - b.debit)*-1 as BALANCE, c.DOCDATE, c.docno as paydocno, 'JV'
...
call sp_processtime('Journal Journals','start');
  ...
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
        SELECT a.company,a.branch,d.objectcode,d.docno, sum(b.credit - b.debit)*-1 as BALANCE, c.DOCDATE, c.docno AS paydocno, 'JV'
...
call sp_processtime('A/P CM Journals','start');
  ...
  INSERT
    INTO settlements (COMPANY,BRANCH,OBJECTCODE,DOCNO,BALANCE,DOCDATE,PAYDOCNO,TYPE)
        SELECT a.company,a.branch,a.objectcode,a.docno, sum(b.debit - b.credit)*-1 as BALANCE, c.DOCDATE, c.docno as paydocno, 'JV'
...
INSERT
    ...
    SELECT journalvoucheritems.COMPANY, journalvoucheritems.BRANCH, journalvoucheritems.OBJECTCODE, journalvoucheritems.DOCNO,
        ...
        between 1 and 30 then ((journalvoucheritems.credit-journalvoucheritems.debit) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D7',
        ...
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE))>0,
        case when datediff(pi_date, date_format(journalvouchers.DOCDUEDATE, '%y-%m-%d'))
        between 61 and 90 then ((journalvoucheritems.credit-journalvoucheritems.debit) -
        if (balances.BALANCE is null, 0, balances.BALANCE) -
        if (journalpayments.BALANCE is null, 0, journalpayments.BALANCE) -
        if (sale_return.BALANCE is null, 0, sale_return.BALANCE)) else 0.00 end,
        0) as 'D30',
...
CREATE TEMPORARY TABLE  `mainfield` (
    ...
    `OTHERDOCNO` varchar(100) NULL default '',
    `OTHERTRXTYPE` varchar(100) NULL default '',
    `OTHERBPREFNO` varchar(100) NULL default '',
    `OTHERBPCODE` varchar(100) NULL default '',
    `OTHERBPNAME` varchar(100) NULL default '',
    `OTHERBPCODE2` varchar(100) NULL default '',
    `OTHERBPNAME2` varchar(100) NULL default '',
...
INSERT
    INTO mainfield (COMPANY,BRANCH,BRANCHNAME,BPCODE,BPNAME,BILLTOADDRESS,ITEMCODE,OBJECTCODE,DOCNO,
    ...
    SELECT apinvoices.COMPANY,
        ...
        apinvoices.BPREFNO,
        apinvoices.OTHERDOCNO,
        apinvoices.OTHERTRXTYPE,
        apinvoices.OTHERBPREFNO,
        apinvoices.OTHERBPCODE,
        apinvoices.OTHERBPNAME,
        apinvoices.OTHERBPCODE2,
        apinvoices.OTHERBPNAME2,
        ...
        WHERE apinvoices.trxtype<>'POS'
          ...
          AND concatfield.DUEAMOUNT is not null
          and (pi_suppno='' or (pi_suppno<>'' and apinvoices.bpcode=pi_suppno))
        GROUP BY apinvoices.DOCNO
        ORDER BY apinvoices.docno ASC;
...
INSERT
    ...
    SELECT apcreditmemos.COMPANY,
        ...
        apcreditmemos.OTHERDOCNO,
        apcreditmemos.OTHERTRXTYPE,
        apcreditmemos.OTHERBPREFNO,
        apcreditmemos.OTHERBPCODE,
        apcreditmemos.OTHERBPNAME,
        apcreditmemos.OTHERBPCODE2,
        apcreditmemos.OTHERBPNAME2,
...
INSERT
    ...
    SELECT journalvouchers.COMPANY,
        ...
        journalvouchers.OTHERDOCNO,
        journalvoucheritems.OTHERTRXTYPE,
        journalvouchers.OTHERBPREFNO,
        journalvouchers.OTHERBPCODE,
        journalvouchers.OTHERBPNAME,
        '' OTHERBPCODE2,
        '' OTHERBPNAME2,
