...
function onPageSubmitGPSHIS(action) {
	if (action=="a" || action=="sc") {
		...
		if (getInput("u_isfixedasset")=="0") {
			if (isInputEmpty("u_soacategory")) return false;
		}
...
function setSalesPricingGPSHIS(setattrib,setvalues) {
	...
	switch (getInput("u_salespricing")) {
		case "0":
			...
			if (setattrib) disableInput("u_salesmarkup2");
			if (setvalues) setInputPercent("u_salesmarkup2",0);
			break;
		case "1":
			if (setattrib) enableInput("u_salesmarkup");
			if (setattrib) enableInput("u_salesmarkup2");
			break;
		case "2":
		case "-1":
			if (setattrib) disableInput("u_salesmarkup");
			...
			if (getTableInput("T110","bptype",i)=="C") {
				switch (getInput("u_salespricing")) {
					...
					case "1":
						if (setattrib) disableTableInput("T110","price",i);
						if (setvalues) {
							if (getTableInput("T110","pricelist",i)=="1") setTableInputPrice("T110","price",purchaseprice*(1+((getInputNumeric("u_salesmarkup2")/100))),i);
							else  setTableInputPrice("T110","price",purchaseprice*(1+((getInputNumeric("u_salesmarkup")/100))),i);
						}
						break;
					case "2":
						if (setattrib) disableTableInput("T110","price",i);
