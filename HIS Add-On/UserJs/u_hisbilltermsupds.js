...
function onPageLoadGPSHIS() {
	try {
		if (getVar("formSubmitAction")=="a") {
			...
			setInput("u_billing",window.opener.getTableInput("T1","u_billing",rc));
			setInput("u_prepaid",window.opener.getTableInput("T1","u_prepaid",rc));
		}
	} catch (theError) {
	}
}
...
function onPageSubmitGPSHIS(action) {
	if (action=="a" || action=="sc") {
		...
		if (isInputEmpty("u_paymentterm")) return false;
	...
	switch (table) {
		default:
			switch (column) {
				case "u_paymentterm":
					if (getInput(column)!="") {
						var result = page.executeFormattedSearch("select u_prepaid from u_hispaymentterms where code='"+getInput(column)+"'");
						if (!ajaxError) {
							if (result=="1") setInput("u_billing",0);
							else setInput("u_billing",1);
							
							setInput("u_prepaid",result);
						} else {
							page.statusbar.showError(ajaxErrorMessage);
							return false;
						}
					} else {
...
function onTableSelectRowGPSHIS(table,row) {
	...
	switch (table) {
		case "T1":
			params["focus"] = false;
			if (elementFocused.substring(0,14)=="df_u_prepaidT1") {
				focusTableInput(table,"u_prepaid",row);
			} else {
				focusTableInput(table,"u_creditlimit",row);
			}
			break;
	}
	return params;
