			...
			if (parseInt(result.getAttribute("result"))>0) {
				for (iii = 0; iii < result.childNodes.length; iii++) {
					if (iii==0) {
						...
						if (getVar("objectcode")=="U_HISOPS" || getVar("objectcode")=="U_HISIPS") {
							setInput("u_remarks",result.childNodes.item(0).getAttribute("u_finaldiagnosis"));
							setInput("u_remarks2",result.childNodes.item(0).getAttribute("u_finaldiagnosis"));
						}
	...
	if (value) {
		switch (getVar("objectcode").toUpperCase()) {
			...
			case "U_HISMEDSUPS":
				...
				if (column=="u_itemcode") result = page.executeFormattedQuery("select a.code, a.name, a.u_group, a.u_salespricing, a.u_ispackage, a.u_template, a.u_statperc, a.u_allowdiscount, a.u_billdiscount, if(a.u_allowdiscount=1,b.u_globalperc,0) as u_globalperc, a.u_scdiscamount, a.u_nonvatcs from u_hisitems a inner join itemgroups c on c.itemgroup=a.u_group left join u_hishealthinhfs b on b.code='"+getInput("u_disccode")+"' and b.u_chrgcode=c.u_discgroup where a.code='"+value+"' and a.u_active=1 "+itemgroupexp+departmentexp);	 
				else result = page.executeFormattedQuery("select a.code, a.name, a.u_group, a.u_salespricing, a.u_ispackage, a.u_template, a.u_statperc, a.u_allowdiscount, a.u_billdiscount, if(a.u_allowdiscount=1,b.u_globalperc,0) as u_globalperc, a.u_scdiscamount, a.u_nonvatcs from u_hisitems a inner join itemgroups c on c.itemgroup=a.u_group left join u_hishealthinhfs b on b.code='"+getInput("u_disccode")+"' and b.u_chrgcode=c.u_discgroup where (a.code='"+value+"' or a.name like '"+value+"%') and a.u_active=1 "+itemgroupexp+departmentexp);
				break;
			case "U_HISMEDSUPTFS":
			...
			if (parseInt(result.getAttribute("result"))>0) {
				...
				if (result2.getAttribute("result")!= "-1") {
					if (parseInt(result2.getAttribute("result"))==0) {
						...
						if (isTableInput(table,"u_nonvatcs")) setTableInput(table,"u_nonvatcs",result.childNodes.item(0).getAttribute("u_nonvatcs"));
				...
				setTableInputAmount(table,"u_vatamount",utils.computeVAT(getTableInputNumeric(table,"u_quantity") * getTableInputNumeric(table,"u_unitprice"),getTableInputNumeric(table,"u_vatrate")));
			} else {
				...
				if (isTableInput(table,"u_nonvatcs")) setTableInput(table,"u_nonvatcs",0);
...
function resetPatientPricesGPSHIS(table,totalamountfield,vatamountfield,discamountfield,chrgcode,totalbefvatfield,totalbefdiscfield) {
	...
	for (ii = 0; ii < tables.length; ii++) {
		...
		for (i = 1; i <= rc; i++) {
			if (isTableRowDeleted(tables[ii],i)==false) {
				if (getInput("u_pricelist")!="") {
					...
					//if (isSC || isNonVATCS==1) {
						if (ifGotStat && isStat && isTableInput(tables[ii],"u_statunitprice")) {
							setTableInputAmount(tables[ii],"u_vatamount",utils.computeVAT(getTableInputNumeric(tables[ii],"u_quantity",i) * getTableInputNumeric(tables[ii],"u_statunitprice",i),getTableInputNumeric(tables[ii],"u_vatrate",i)),i);
						} else {
		...
		if (getTableInput(tables[ii],itemcodefield)!="" && getInput("u_pricelist")!="") {
			...
			if (ifGotStat && isStat && isTableInput(tables[ii],"u_statunitprice")) {
				setTableInputAmount(tables[ii],"u_vatamount",utils.computeVAT(getTableInputNumeric(tables[ii],"u_statunitprice")*getTableInputNumeric(tables[ii],"u_quantity"),getTableInputNumeric(tables[ii],"u_vatrate")));
			} else {
				setTableInputAmount(tables[ii],"u_vatamount",utils.computeVAT(getTableInputNumeric(tables[ii],"u_unitprice")*getTableInputNumeric(tables[ii],"u_quantity"),getTableInputNumeric(tables[ii],"u_vatrate")));
			}
...
function computePatientLabTotalGPSHIS(table) {
	...
	//try {
	//	setInputAmount("u_vatamount",utils.computeVAT(getInputNumeric("u_quantity") * getInputNumeric("u_unitprice"),getInputNumeric("u_vatrate")));
	//} catch (theError) {
