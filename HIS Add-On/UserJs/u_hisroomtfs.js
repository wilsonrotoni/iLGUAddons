	...
	if (getVar("formSubmitAction")=="a") {
		try {
			if (window.opener.getVar("objectcode")=="U_HISPLIST") {
				...
				setInput("u_roomnofr",window.opener.getTableInput("T1","u_roomno",row));
				setInput("u_bednofr",window.opener.getTableInput("T1","u_bedno",row));
				setInput("u_roomdescfr",window.opener.getTableInput("T1","u_roomdesc",row));
				setInput("u_pricelistfr",window.opener.getTableInput("T1","u_pricelist",row));
			} else {
				var row = window.opener.getTableSelectedRow("T2");
				setInput("u_refno",window.opener.getInput("docno"));
				setInput("u_patientid",window.opener.getInput("u_patientid"));
				setInput("u_patientname",window.opener.getInput("u_patientname"));
				setInput("u_roomnofr",window.opener.getTableInput("T2","u_roomno",row));
				setInput("u_bednofr",window.opener.getTableInput("T2","u_bedno",row));
				setInput("u_roomdescfr",window.opener.getTableInput("T2","u_roomdesc",row));
				setInput("u_pricelistfr",window.opener.getTableInput("T2","u_pricelist",row));
			}
		} catch (theError) {
		}
	}
...
function onPageSubmitGPSHIS(action) {
	if (action=="a" || action=="sc") {
		...
		if (isInputEmpty("u_roomnofr")) return false;
		if (isInputEmpty("u_bednofr")) return false;
		...
		try {
			if (window.opener.getInput("u_department")!=getInput("u_department")) {
				if (window.opener.getTableRowCount("T112")>0 || window.opener.getTableRowCount("T113")>0) {
					alert("You cannot transfer patient to another nursing unit if Pending Requests still exists or you are still holding Medines & Supplies.");
					return false;
				}
			}
		} catch (theError) {
			page.statusbar.showError("Un error occured while cheking the current nursing unit and pending items.");
			return false;
		}
		//return false;
	}
