<?php
	...
	require_once("./dtw/outgoingpayments.php");
	...
	if ($rawdata!="") file_put_contents("c:/users/post_ap.raw",file_get_contents("php://input"));
	else $rawdata = file_get_contents("c:/users/post_ap.raw");
	$json = json_decode($rawdata,true);
	file_put_contents("c:/users/post_ap.json",serialize($json));
	$ctr=0;
	/*
	...
	$actionReturn = true;
	$authReturn = false;
	$objDTWOutgoingPayments = new dtw_outgoingpayments();
	...
	$data = array();
	$data["Outgoing Payments"] =  array();
	$data["Outgoing Payment - Documents"] =  array();
	$data["Outgoing Payment - Checks"] =  array();
	array_push($data["Outgoing Payments"],array("Document No.", "Document Series", "Posting Date", "Business Partner Type", "Business Partner Code", "Business Partner Name", "Reference No.", "Remarks", "Cash Amount"));
	array_push($data["Outgoing Payment - Documents"],array("Document No.", "Reference Type", "Reference No.", "Amount"));
	array_push($data["Outgoing Payment - Checks"],array("Document No.", "Check Date", "Bank", "Bank Account No.", "Check No.", "Amount"));
	
	foreach($json as $key => $data2) {
		$ctr++;
		if ($key=="outgoingpayments") {
			if ($jwt!="") {
				foreach($data2 as $journalvouchers) {
					...
					foreach($journalvouchers as $jvkey => $jvdata) {
						if ($jvkey=="documents") {
							foreach($jvdata as $journalvoucheritems) {
								...
								array_push($data["Outgoing Payment - Documents"],$dtl);				
								//$ctr++;
					...
					array_push($data["Outgoing Payments"],$hdr);
				}		
			} else {
				$actionReturn = raiseError("JWT was not passed or it was passed after the outgoingpayments data.");
			}	
		} else {
			switch ($key) {
	...
	if ($actionReturn) {
		...
		if ($checkamount!=0) {
			...
			$dtl[1] = $checkdate;
			$dtl[2] = $checkbank;
			$dtl[3] = $checkbankacctno;
			$dtl[4] = $checkno;
			$dtl[5] = $checkamount;
			array_push($data["Outgoing Payment - Checks"],$dtl);
		}	
	}
	//var_dump($data);
