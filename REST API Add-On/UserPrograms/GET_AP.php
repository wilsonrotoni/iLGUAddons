<?php
	...
	foreach($json as $key => $data2) {
		...
		switch ($key) {
			...
			case "bpcode":
				$bpcode = trim($data2);
				$objRs->queryopen("select suppname from suppliers where suppno='$bpcode'");
				if ($objRs->queryfetchrow("NAME")) {
					$bpname = $objRs->fields["suppname"];
	...
	if ($actionReturn && $authReturn) {
		$documents = array();
		$objRs->queryopen("select 'A/P Invoice' as doctype, docno, docdate, docduedate as duedate, bprefno as refno, totalamount, dueamount as balance from apinvoices where company='".$_SESSION["company"]."' and branch='".$_SESSION["branch"]."' and bpcode='$bpcode' and docstatus='O'");
