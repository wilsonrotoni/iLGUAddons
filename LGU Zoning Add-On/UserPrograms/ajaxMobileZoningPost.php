<?php
	...
	switch ($_GET["type"]) {
		case "ClearanceInspectionPictures":
			...
			file_put_contents("c:\users\ZoningClearanceInspectionPictures.txt",serialize($obj));
			...
			foreach($obj['data'] as $data) {
				...
				mkdirr("..//Images/$company/$branch/U_ZONINGCLRINS/" . $data["docno"] ."/Attachments/");
				$file = fopen("..//Images/$company/$branch/U_ZONINGCLRINS/" . $data["docno"] ."/Attachments/". $data["name"], "wb"); // 
				fwrite($file, $binary);
		...
		case "ClearanceInspectionSignatures":
			...
			$obj = json_decode($_GET["data"],true);
			file_put_contents("c:\users\ZoningClearanceInspectionSignatures.txt",serialize($obj));
		...
		case "ClearanceInspections":
			...
			$obju_FSIns = new documentschema_br(null,$objConnection,"u_zoningclrins");
