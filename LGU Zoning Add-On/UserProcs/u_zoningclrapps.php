...
function onAfterDefaultGPSLGUZoning() { 
	...
	global $appdata;
		...
		if ($obju_ZoningApps->getbykey($appdata["u_bpno"])) {
			...
			$page->setitem("u_orsfamt",formatNumericAmount($obju_ZoningApps->getudfvalue("u_orsfamt")));
			...
			while ($objRs->queryfetchrow("NAME")) {
				...
				$objGrids[0]->setitem(null,"u_seqno",$objRs->fields["U_SEQNO"]);
                        ...
                        $page->setitem("u_orsfamt",formatNumericAmount($plantotal));
		}
        }else {
            $objRs->queryopen("select code, name, u_amount,u_seqno from u_zoningfees order by u_seqno asc, name asc");
            while ($objRs->queryfetchrow("NAME")) {
                    $objGrids[0]->addrow();
                    $objGrids[0]->setitem(null,"u_year",date('Y'));
