	...
	switch (table) {
		default:
			switch(column) {
				case "u_appno":
					if (getInput(column)!="") {
						result = page.executeFormattedQuery("SELECT u_docdate,u_bpno,u_businessname,CONCAT(u_baddressno,u_bbldgno,u_bunitno,u_bfloorno, u_bbldgname,u_bblock, u_blotno,u_bphaseno, u_bstreet, u_bvillage,u_bbrgy,u_bcity,u_bprovince ) AS u_address FROM ( select u_docdate, u_bpno, u_businessname,  IF(IFNULL(a.u_baddressno,'')='','',CONCAT(a.u_baddressno,', ')) AS u_baddressno, IF(IFNULL(a.u_bbldgno,'') = '','',CONCAT(a.u_bbldgno,' ')) AS u_bbldgno,IF(IFNULL(a.u_bunitno,'') = '','',CONCAT(a.u_bunitno,', ')) AS u_bunitno, IF(IFNULL(a.u_bfloorno,'') = '','',CONCAT(a.u_bfloorno,', ')) AS u_bfloorno,IF(IFNULL(a.u_bbldgname,'') = '','',CONCAT(a.u_bbldgname,', ')) AS u_bbldgname, IF(IFNULL(a.u_bblock,'')='','',CONCAT('BLK ',a.u_bblock,' ')) AS u_bblock, IF(IFNULL(a.u_blotno,'')='','',CONCAT('LOT ',a.u_blotno,' ')) AS u_blotno, IF(IFNULL(a.u_bphaseno,'')='','',CONCAT('PHASE ',a.u_bphaseno,', ')) AS u_bphaseno, IF(IFNULL(a.u_bstreet,'')='','',CONCAT(a.u_bstreet,', ')) AS u_bstreet, IF(IFNULL(a.u_bvillage,'')='','',CONCAT(a.u_bvillage,', ')) AS u_bvillage, IF(IFNULL(a.u_bbrgy,'')='','',CONCAT(a.u_bbrgy,', ')) AS u_bbrgy, IF(IFNULL(a.u_bcity,'')='','',CONCAT(a.u_bcity,', ')) AS u_bcity, IF(IFNULL(a.u_bprovince,'')='','',CONCAT(a.u_bprovince,' ')) AS u_bprovince from u_bldgapps a where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and docno = '"+getInput(column)+"' and u_insno = '')  AS X");	
...
function onElementCFLGetParamsGPSLGUBuilding(Id,params) {
	switch (Id) {
		case "df_u_appno":
			params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select docno, u_businessname, u_acctno from u_bldgapps where company='"+getGlobal("company")+"' and branch='"+getGlobal("branch")+"' and u_insno=''")); 
