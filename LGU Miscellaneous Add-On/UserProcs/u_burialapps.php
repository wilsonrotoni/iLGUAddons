...
function onAfterDefaultCTC() { 
	...
	$objRs->queryopen("SELECT A.CODE, B.U_AUTOSERIES,B.U_ISSUEDOCNO,B.DOCNO AS U_SERIESDOCNO,B.U_DOCSERIES,B.U_DOCSERIESNAME  FROM U_LGUPOSREGISTERS A
                            ...
                            WHERE A.BRANCH='".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.U_USERID='".$_SESSION['userid']."' AND A.U_STATUS = 'O' AND B.U_NEXTNO <= B.U_LASTNO AND C.U_ISBURIAL = 1 ORDER BY B.U_ISDEFAULT DESC");
...
$page->businessobject->items->setcfl("u_dateofdeath","Calendar");
$page->businessobject->items->setcfl("u_docdate","Calendar");
$page->businessobject->items->seteditable("u_totalamount",true);
...
$pageHeader = "Burial Permit Application";
