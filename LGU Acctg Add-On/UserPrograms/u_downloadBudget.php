	...
	function onFormAction($action,$opt="") {
		...
		switch (strtolower($action)) {
			case "u_download":
				...
				$template .= "Templates\\Budget Requests.xlsx".$file;
				...
				$objRs->queryopen("select a.u_yr, a.u_profitcenter, b.u_glacctno, b.u_glacctname, b.u_sldesc, b.u_yr as u_amount from u_lgubudget a inner join u_lgubudgetgls b on b.company=a.company and b.branch=a.branch and b.code=a.code where a.company='".$_SESSION["company"]."' and a.branch='".$_SESSION["branch"]."' and a.u_yr = '$year' $profitcenterExp");
				while ($objRs->queryfetchrow("NAME")) {
					$data = array();
					array_push($data,$objRs->fields["u_yr"]);
					array_push($data,$objRs->fields["u_profitcenter"]);
					array_push($data,$objRs->fields["u_glacctno"]);
					array_push($data,$objRs->fields["u_glacctname"]);
					array_push($data,$objRs->fields["u_sldesc"]);
					array_push($data,$objRs->fields["u_amount"]);
...
<script language="JavaScript">
	...
	function onPageLoad() {
		try {
			if (window.opener.getVar("objectcode")=="U_LGUBUDGET") {
