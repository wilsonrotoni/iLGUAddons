...
require("./inc/formactions.php");
	...
	function uploadLegacyIntBrCollections($uploadfile) {
		...
		$obju_LGUPPMPGLs = new documentlinesschema_br(NULL,$objConnection,"u_lguppmpgls");
		...
		if (!$error) {
			...
			if ($actionReturn) {
				foreach ($profitcenters as $key => $pcdata) {
					if ($obju_LGUPPMP->getbysql("u_yr='".$pcdata["yr"]."' and u_profitcenter='".$pcdata["code"]."'")) {
						...
						$obju_LGUPPMP->prepareadd();
						$obju_LGUPPMP->docid = getNextIdByBranch("u_lguppmp",$objConnection);
						$obju_LGUPPMP->docseries = getDefaultSeries("u_lguppmp",$objConnection);
						$obju_LGUPPMP->docno = getNextSeriesNoByBranch("u_lguppmp",$obju_LGUPPMP->docseries,'',$objConnection);
						$obju_LGUPPMP->docstatus = "Draft";
						$obju_LGUPPMP->setudfvalue("u_yr",$pcdata["yr"]);
						$obju_LGUPPMP->setudfvalue("u_profitcenter",$pcdata["code"]);
						$obju_LGUPPMP->setudfvalue("u_profitcentername",$pcdata["name"]);
					}
					$totalamount = 0;
					if ($actionReturn) {
						foreach ($pcdata["items"] as $item) {
							...
							$obju_LGUPPMPGLs->prepareadd();
							$obju_LGUPPMPGLs->docid = $obju_LGUPPMP->docid;
							$obju_LGUPPMPGLs->lineid = getNextIdByBranch("u_lguppmpgls",$objConnection);
							$obju_LGUPPMPGLs->setudfvalue("u_glacctno",$item["glacctno"]);
							$obju_LGUPPMPGLs->setudfvalue("u_glacctname",$item["glacctname"]);
							$obju_LGUPPMPGLs->setudfvalue("u_slcode",$item["slcode"]);
							$obju_LGUPPMPGLs->setudfvalue("u_sldesc",$item["sldesc"]);
							$obju_LGUPPMPGLs->setudfvalue("u_yr",$item["amount"]);
							$monthly = round($item["amount"]/12,2);
							
							$obju_LGUPPMPGLs->setudfvalue("u_m1",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m2",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m3",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m4",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m5",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m6",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m7",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m8",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m9",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m10",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m11",$monthly);
							$obju_LGUPPMPGLs->setudfvalue("u_m12",$item["amount"]-($monthly*11));
							
							$actionReturn = $obju_LGUPPMPGLs->add();
							if (!$actionReturn) break;
					...
					$obju_LGUPPMP->setudfvalue("u_totalamount",$totalamount);
					
					if ($obju_LGUPPMP->rowstat=="N") $actionReturn = $obju_LGUPPMP->add();
					else  $actionReturn = $obju_LGUPPMP->update($obju_LGUPPMP->docno,$obju_LGUPPMP->rcdversion);
