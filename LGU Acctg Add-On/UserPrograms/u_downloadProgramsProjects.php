<?php
	...
	$progid = "u_downloadProgramsProjects";
	...
	function onFormAction($action,$opt="") {
		...
		switch (strtolower($action)) {
			case "u_download":
				...
				$template .= "Templates\\Programs & Projects.xlsx".$file;
				
				$year= $page->getitemstring("year");
				$annex= $page->getitemstring("annex");
				...
				$objRs->queryopen("select a.u_annex, a.u_yr, a.u_appcode, a.name, a.u_profitcenter, a.u_procmode, a.u_schedule, a.u_sourcefunds, a.u_estbudget, a.u_estmooe, a.u_estco, a.u_remarks from u_lguprojs a where a.company='".$_SESSION["company"]."' and a.branch='".$_SESSION["branch"]."' and a.u_annex='$annex' and a.u_yr = '$year' $profitcenterExp");
				while ($objRs->queryfetchrow("NAME")) {
					$data = array();
					array_push($data,$objRs->fields["u_annex"]);
					array_push($data,$objRs->fields["u_yr"]);
					array_push($data,$objRs->fields["u_appcode"]);
					array_push($data,$objRs->fields["name"]);
					array_push($data,$objRs->fields["u_profitcenter"]);
					array_push($data,$objRs->fields["u_procmode"]);
					array_push($data,$objRs->fields["u_schedule"]);
					array_push($data,$objRs->fields["u_sourcefunds"]);
					array_push($data,$objRs->fields["u_estbudget"]);
					array_push($data,$objRs->fields["u_estmooe"]);
					array_push($data,$objRs->fields["u_estco"]);
					array_push($data,$objRs->fields["u_remarks"]);
					$objExcel->writerange($sheet,getCellAddress($col,$row).":".getCellAddress($col+count($data),$row), $data);
					$row++;
				}					
				$uploadpath = realpath("../Addons/GPS/LGU Acctg Add-On/UserPrograms/");
	...
	$schema["annex"]=createSchema("annex","Annex");
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="0">
		  <!--DWLayoutTable-->
			<tr>
			  ...
			  <td width="108"><label <?php genCaptionHtml($schema["annex"],"") ?> >Annex</label></td>
				<td>&nbsp;<select <?php genSelectHtml($schema["annex"],array("loadudflinktable","u_lguappannexes:code:name",":")) ?> ></select></td>
				</tr>
