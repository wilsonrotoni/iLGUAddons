...
require("./inc/formactions.php");
	...
	function uploadLegacyIntBrCollections($uploadfile) {
		...
		$obju_LGUActExps = new masterdataschema_br(NULL,$objConnection,"u_lguactexps");
		...
		while($content = $objExcel->readrange($sheet,"A".$row.":J".$row)) {
			...
			if ($row==1) {
				if ($content[0]!="Date" || $content[1]!="Profit Center" || $content[2]!="G/L Account No." || $content[3]!="S/L Description"  || $content[4]!="Amount" ) {
			...
			//var_dump($content);
			$idx = 'T1r'.$rowidx;
			$data['df_date'.$idx] = trim($content[0]);
			$data['df_profitcenter'.$idx] = trim($content[1]);
			$data['df_glacctno'.$idx] = trim($content[2]);
			$data['df_sldesc'.$idx] = trim($content[3]);
			$data['df_amount'.$idx] = floatval(trim($content[4]));
			...
			if ($data['df_profitcenter'.$idx]!="") {
				if (!isset($profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]])) {
					if ($objProfitCenters->getbykey($data['df_profitcenter'.$idx])) {
						$data['df_profitcentername'.$idx] = $objProfitCenters->profitcentername;
						
						$profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]] = array();
						$profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]]["date"] =  $data['df_date'.$idx];
						$profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]]["code"] =  $data['df_profitcenter'.$idx];
						$profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]]["name"] =  $data['df_profitcentername'.$idx];
						$profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]]["items"] = array();
					} else {
						$errormessage .= " Profit Center is invalid.";
						$isvalid=false;
					}
				} else {
					$data['df_profitcentername'.$idx] = $profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]]["name"];
				}				
			}	
			...
			if ($data['df_sldesc'.$idx]!="") {
				$objrs->queryopen("select u_code, u_description from u_lgupcsubsidiaryaccts where code='".$data['df_profitcenter'.$idx]."' and u_glacctno='".$data['df_glacctno'.$idx]."' and u_description='".addslashes($data['df_sldesc'.$idx])."'");
			...
			*/
			...
			if ($isvalid) {
				if ($data['df_amount'.$idx]>0) {
					...
					array_push($profitcenters[$data['df_date'.$idx]."-".$data['df_profitcenter'.$idx]]["items"],$item);
				}	
			} else {
			...
			if ($actionReturn) {
				foreach ($profitcenters as $key => $pcdata) {
					foreach ($pcdata["items"] as $item) {
						...
						if ($obju_LGUActExps->getbysql("u_date='".$pcdata["date"]."' and u_profitcenter='".$pcdata["code"]."' and u_glacctno='".$item["glacctno"]."' and u_slcode='".$item["slcode"]."'")) {
							$actionReturn = raiseError($pcdata["date"] . " Actual Expenditures for ".$pcdata["name"]." [".$pcdata["code"]."/".$item["glacctno"]."/".$item["slcode"]."] cannot be uploaded if already exists.");
							break;
						} else {
							$obju_LGUActExps->prepareadd();
							$obju_LGUActExps->code = $pcdata["date"] . "-" . $pcdata["code"] . "-" . $item["glacctno"] . "-" . $item["slcode"];
							$obju_LGUActExps->setudfvalue("u_date",$pcdata["date"]);
							$obju_LGUActExps->setudfvalue("u_profitcenter",$pcdata["code"]);
							$obju_LGUActExps->setudfvalue("u_glacctno",$item["glacctno"]);
							$obju_LGUActExps->setudfvalue("u_slcode",$item["slcode"]);
						}
						
						$obju_LGUActExps->setudfvalue("u_glacctname",$item["glacctname"]);
						$obju_LGUActExps->setudfvalue("u_sldesc",$item["sldesc"]);
						$obju_LGUActExps->setudfvalue("u_amount",$item["amount"]);
						
						$actionReturn = $obju_LGUActExps->add();
						if (!$actionReturn) break;
