...
$objGrid1->addcolumn(array('name'=>'earlyproc','description'=>'Is this an Early Procument Activity?','length'=>35));
$objGrid1->addcolumn(array('name'=>'procmode','description'=>'Mode of Procurement','length'=>15));
$objGrid1->addcolumn(array('name'=>'ads','description'=>'Advertisement/Posting of IB/REI','length'=>15));
$objGrid1->addcolumn(array('name'=>'opening','description'=>'Submission/Opening of Bids','length'=>15));
$objGrid1->addcolumn(array('name'=>'award','description'=>'Notice of Award','length'=>15));
$objGrid1->addcolumn(array('name'=>'contractsign','description'=>'Contract Signing','length'=>15));
...
require("./inc/formactions.php");
	...
	function uploadLegacyIntBrCollections($uploadfile) {
		...
		while($content = $objExcel->readrange($sheet,"A".$row.":L".$row)) {
			...
			if ($row==1) {
                        if ($content[0]!="Year" || $content[1]!="APP Code (PAP)" || $content[2]!="Procurement Program/Project"  || $content[3]!="PMO/End-User" || $content[4]!="Is this an Early Procument Activity?" || $content[5]!="Mode of Procurement"  || 
				    $content[6]!="Advertisement/Posting of IB/REI" || 
				    $content[7]!="Submission/Opening of Bids" || 
				    $content[8]!="Notice of Award" || 
				    $content[9]!="Contract Signing" || 
					$content[10]!="Source of Funds"  || 
					$content[11]!="Total"  || 
					$content[12]!="MOOE"  || 
					$content[13]!="CO"  || 
					$content[14]!="Remarks (Brief Description of Program/Project)"  ) {
...
*/			
			...
			$data['df_appcode'.$idx] = ltrim($content[1]);
			$data['df_description'.$idx] = ltrim($content[2]);
			$data['df_profitcenter'.$idx] = ltrim($content[3]);
			$data['df_earlyproc'.$idx] = ltrim($content[4]);
			$data['df_procmode'.$idx] = ltrim($content[5]);
			$data['df_ads'.$idx] = ltrim($content[6]);
			$data['df_opening'.$idx] = ltrim($content[7]);
			$data['df_award'.$idx] = ltrim($content[8]);
			$data['df_contractsign'.$idx] = ltrim($content[9]);
			$data['df_sourcefunds'.$idx] = ltrim($content[10]);
			$data['df_estbudget'.$idx] = floatval(ltrim($content[11]));
			$data['df_estmooe'.$idx] = floatval(ltrim($content[12]));
			$data['df_estco'.$idx] = floatval(ltrim($content[13]));
			$data['df_remarks'.$idx] = ltrim($content[14]);
			...
			*/
			...
			if ($isvalid) {
					...
					$item["earlyproc"] = $data['df_earlyproc'.$idx];
					$item["procmode"] = $data['df_procmode'.$idx];
					$item["ads"] = $data['df_ads'.$idx];
					$item["opening"] = $data['df_opening'.$idx];
					$item["award"] = $data['df_award'.$idx];
					$item["contractsign"] = $data['df_contractsign'.$idx];
		...
		if (!$error) {
			...
			if ($actionReturn) {
				foreach ($apps as $data) {
					...
					$obju_LGUProjs->setudfvalue("u_earlyprocurement",$data["earlyproc"]);
					$obju_LGUProjs->setudfvalue("u_procmode",$data["procmode"]);
					$obju_LGUProjs->setudfvalue("u_adsdate",$data["ads"]);
					$obju_LGUProjs->setudfvalue("u_opendate",$data["opening"]);
					$obju_LGUProjs->setudfvalue("u_noadate",$data["award"]);
					$obju_LGUProjs->setudfvalue("u_signdate",$data["contractsign"]);
