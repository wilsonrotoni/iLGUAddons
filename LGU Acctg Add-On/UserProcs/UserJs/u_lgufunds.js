...
function onPageSubmitGPSLGUAcctg(action) {
	if (action=="a" || action=="sc") {
		...
		//if (isInputEmpty("u_fundtype")) return false;
		//if (isInputNegative("u_fundamount")) return false;
			...
			switch(column) {
				...
				case "u_jevno":
					if (getInput("u_jevno")!="" && getInput("u_jevno")!="?") {
						...
						//setInput("u_jevno",getGlobal("branch")+"-"+getInput("u_jevseries")+"-"+year+"-"+month+"-"+getInput("u_fundtype")+getInput("u_jevno").padL(4,"0"));
					}
					break;
				case "u_lvatperc":
				case "u_levatperc":
...
function computeTotalGPSLGUAcctg(adjusted) {
	...
	advanceamount = getInputNumeric("u_fundamount");
