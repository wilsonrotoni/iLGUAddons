	...
	switch (table) {
		...
		case "T2":
			switch (column) {
				...
				case "u_glacctname":
					if (getTableInput(table,column)!="") {
						if (column=="u_glacctno") result = page.executeFormattedQuery("select formatcode, acctname, u_expgroupno, u_expclass from chartofaccounts where budget=1 and u_expclass <>'' and postable=0 and formatcode = '"+getTableInput(table,column)+"'");	
						else  result = page.executeFormattedQuery("select formatcode, acctname, u_expgroupno, u_expclass from chartofaccounts where budget=1 and u_expclass <>'' and  postable=0 and acctname like '"+utils.addslashes(getTableInput(table,column))+"%'");	
						if (result.getAttribute("result")!= "-1") {
							if (parseInt(result.getAttribute("result"))>0) {
...
function onElementCFLGetParamsGPSLGUAcctg(id,params) {
	switch (id) {	
		...
		case "df_u_glacctnameT2":
			params["params"] = "&cflquery="+utils.replaceSpecialChar(Base64.encode("select acctname, formatcode from chartofaccounts where budget=1 and u_expclass <>'' and postable=0")); 
