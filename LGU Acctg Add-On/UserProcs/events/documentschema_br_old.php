...
function onCustomeventValidateCashOBRdocumentschema_brGPSLGUAcctg($objTable) {
	...
	$objRs->queryopen("select u_profitcenter, u_glacctno, u_glacctname, sum(u_obramount) as u_obramount, sum(u_actamount) as u_actamount from (
select if(d.u_profitcenter<>'',d.u_profitcenter,a.u_profitcenter) as u_profitcenter, d.u_glacctno, d.u_glacctname, 0 as u_obramount, u_amount as u_actamount from u_lgucashds a
...
select c.u_profitcenter, d.u_glacctno, d.u_glacctname, d.u_debit-d.u_credit as u_obramount, 0 as u_actamount from u_lgucashds a
...
select c.u_profitcenter, d.u_glacctno, d.u_glacctname, d.u_debit-d.u_credit as u_obramount, 0 as u_actamount from u_lgucashds a
 ...
 ) as x group by u_profitcenter, u_glacctno having u_obramount <> u_actamount");
	
	//var_dump($objRs->sqls);
	$errormsg = "";
	while ($objRs->queryfetchrow("NAME")) {
		$errormsg .= "[".$objRs->fields["u_profitcenter"]." - ".$objRs->fields["u_glacctno"]." - ".$objRs->fields["u_glacctname"]." OBR:".formatNumericAmount($objRs->fields["u_obramount"])." Actual:".formatNumericAmount($objRs->fields["u_actamount"])." Short:".formatNumericAmount($objRs->fields["u_obramount"]-$objRs->fields["u_actamount"])."]`";
	}	
...
function onCustomeventValidateCheckOBRdocumentschema_brGPSLGUAcctg($objTable) {
	...
	$branch = $_SESSION["branch"];
	$objRs->queryopen("select u_profitcenter, u_glacctno, u_glacctname, sum(u_obramount) as u_obramount, sum(u_actamount) as u_actamount from (
select if(d.u_profitcenter<>'',d.u_profitcenter,a.u_profitcenter) as u_profitcenter, d.u_glacctno, d.u_glacctname, 0 as u_obramount, u_amount as u_actamount from u_lgucheckds a
    inner join u_lgucheckdaccts d on d.company=a.company and d.branch=a.branch and d.docid=a.docid
    inner join chartofaccounts e on e.formatcode=d.u_glacctno and e.budget=1
  where a.company='$company' and a.branch='$branch' and a.docno='$objTable->docno' union all
select c.u_profitcenter, d.u_glacctno, d.u_glacctname, d.u_debit-d.u_credit as u_obramount, 0 as u_actamount from u_lgucheckds a
    inner join u_lguobligationslips c on c.company=a.company and c.branch=a.branch and c.docno=a.u_osno
    inner join u_lguobligationslipaccts d on d.company=c.company and d.branch=c.branch and d.docid=c.docid
    inner join chartofaccounts e on e.formatcode=d.u_glacctno and e.budget=1
  where a.company='$company' and a.branch='$branch' and a.docno='$objTable->docno' union all
...
function onCustomeventValidateFundOBRdocumentschema_brGPSLGUAcctg($objTable) {
	...
	$branch = $_SESSION["branch"];
	$objRs->queryopen("select u_profitcenter, u_glacctno, u_glacctname, sum(u_obramount) as u_obramount, sum(u_actamount) as u_actamount from (
select if(d.u_profitcenter<>'',d.u_profitcenter,a.u_profitcenter) as u_profitcenter, d.u_glacctno, d.u_glacctname, 0 as u_obramount, u_amount as u_actamount from u_lgufunds a
    inner join u_lgufundaccts d on d.company=a.company and d.branch=a.branch and d.docid=a.docid
    inner join chartofaccounts e on e.formatcode=d.u_glacctno and e.budget=1
  where a.company='$company' and a.branch='$branch' and a.docno='$objTable->docno' union all
select c.u_profitcenter, d.u_glacctno, d.u_glacctname, d.u_debit-d.u_credit as u_obramount, 0 as u_actamount from u_lgufunds a
    inner join u_lguobligationslips c on c.company=a.company and c.branch=a.branch and c.docno=a.u_osno
    inner join u_lguobligationslipaccts d on d.company=c.company and d.branch=c.branch and d.docid=c.docid
    inner join chartofaccounts e on e.formatcode=d.u_glacctno and e.budget=1
  where a.company='$company' and a.branch='$branch' and a.docno='$objTable->docno' union all
...
function onCustomeventValidateAPOBRdocumentschema_brGPSLGUAcctg($objTable) {
	...
	$branch = $_SESSION["branch"];
	$objRs->queryopen("select u_profitcenter, u_glacctno, u_glacctname, sum(u_obramount) as u_obramount, sum(u_actamount) as u_actamount from (
select a.u_profitcenter, d.u_glacctno, d.u_glacctname, 0 as u_obramount, u_amount as u_actamount from u_lguaps a
