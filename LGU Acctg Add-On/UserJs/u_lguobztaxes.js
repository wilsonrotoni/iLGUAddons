...
function onElementValidateGPSLGUAcctg(element,column,table,row) {
	switch (table) {
		case "T1":
			switch (column) {
				case "u_gross":
					setTableInput(table,"code",getTableInputNumeric(table,"u_gross"));
					setTableInput(table,"name",formatNumericAmount(getTableInputNumeric(table,"u_gross")));
					break;
			}
			break;
	}
...
function onTableBeforeInsertRowGPSLGUAcctg(table) {
	switch (table) {	
		case "T1":
			if (isTableInputNegative(table,"u_gross")) return false;
			break;
	}
	return true;
}
