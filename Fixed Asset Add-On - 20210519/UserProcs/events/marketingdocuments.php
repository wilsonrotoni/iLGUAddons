<?php 
	...
	function onCustomEventmarketingdocumentsFAAPInvoiceGPSFixedAsset($objTable) {
		...
		if ($objTable->doctype=="S") {
			$objRs->setdebug();
			if ($objTable->objectcode=="APINVOICE") {	
				$objRs->queryopen("SELECT A.ITEMDESC, A.GLACCTNO, A.DOCID, A.LINEID, A.PRICE, (A.LINETOTAL-A.VATAMOUNT-A.LINETOTALDISC)*".$objTable->currencyrate." AS ITEMCOST, A.DRCODE AS PROFITCENTER, A.PROJCODE, A.U_FAPROPERTY1, A.U_FAPROPERTY2, A.U_FAPROPERTY3, A.U_FAPROPERTY4, A.U_FAPROPERTY5 FROM APINVOICEITEMS A WHERE A.DOCID='$objTable->docid' AND A.BRANCH = '".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.DOCTYPE='S' AND A.U_FACAPITALIZE='Y' AND A.DOCID='$objTable->docid'");
			} else {
				$objRs->queryopen("SELECT A.ITEMDESC, A.GLACCTNO, A.DOCID, A.LINEID, A.PRICE, (A.LINETOTAL-A.VATAMOUNT-A.LINETOTALDISC)*".$objTable->currencyrate."*-1 AS ITEMCOST, A.DRCODE AS PROFITCENTER, A.PROJCODE, A.U_FAPROPERTY1, A.U_FAPROPERTY2, A.U_FAPROPERTY3, A.U_FAPROPERTY4, A.U_FAPROPERTY5 FROM APCREDITMEMOITEMS A WHERE A.DOCID='$objTable->docid' AND A.BRANCH = '".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.DOCTYPE='S' AND A.U_FACAPITALIZE='Y' AND A.DOCID='$objTable->docid'");
			}	
		...
		if ($actionReturn) {
			if ($objTable->objectcode=="APINVOICE") {	
				$objRs->queryopen("SELECT A.ITEMCODE, A.ITEMDESC, A.WHSCODE, A.GLACCTNO, A.DOCID, A.LINEID, A.QUANTITY, A.PRICE, ((A.LINETOTAL-A.VATAMOUNT-A.LINETOTALDISC)/A.QUANTITY)*".$objTable->currencyrate." AS ITEMCOST, ((A.LINETOTAL-A.VATAMOUNT-A.LINETOTALDISC))*".$objTable->currencyrate." AS LINECOST, A.DRCODE AS PROFITCENTER, A.PROJCODE, A.U_FAPROPERTY1, A.U_FAPROPERTY2, A.U_FAPROPERTY3, A.U_FAPROPERTY4, A.U_FAPROPERTY5, B.U_ISFIXEDASSET, A.U_FACAPITALIZEBY FROM APINVOICEITEMS A, ITEMS B WHERE B.ITEMCODE=A.ITEMCODE AND A.DOCID='$objTable->docid' AND A.BRANCH = '".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.DOCTYPE='I' AND (B.U_ISFIXEDASSET=1 OR (A.U_FACAPITALIZE='Y' AND A.DROPSHIP=1)) AND A.ITEMMANAGEBY='0' AND A.BASETYPE<>'PURCHASEDELIVERY'");
			} else {
				$objRs->queryopen("SELECT A.ITEMCODE, A.ITEMDESC, A.WHSCODE, A.GLACCTNO, A.DOCID, A.LINEID, A.QUANTITY, A.PRICE, ((A.LINETOTAL-A.VATAMOUNT-A.LINETOTALDISC)/A.QUANTITY)*".$objTable->currencyrate."*-1 AS ITEMCOST, ((A.LINETOTAL-A.VATAMOUNT-A.LINETOTALDISC))*".$objTable->currencyrate." AS LINECOST, A.DRCODE AS PROFITCENTER, A.PROJCODE, A.U_FAPROPERTY1, A.U_FAPROPERTY2, A.U_FAPROPERTY3, A.U_FAPROPERTY4, A.U_FAPROPERTY5, B.U_ISFIXEDASSET, A.U_FACAPITALIZEBY FROM APCREDITMEMOITEMS A, ITEMS B WHERE B.ITEMCODE=A.ITEMCODE AND A.DOCID='$objTable->docid' AND A.BRANCH = '".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.DOCTYPE='I' AND (B.U_ISFIXEDASSET=1 OR (A.U_FACAPITALIZE='Y' AND A.DROPSHIP=1)) AND A.ITEMMANAGEBY='0' AND A.BASETYPE<>'PURCHASEDELIVERY'");
			}	
			while ($objRs->queryfetchrow("NAME")) {
				//var_dump($objRs->fields["ITEMCODE"]);
				if ($objRs->fields["U_FACAPITALIZEBY"]=="Unit") {
					...
					$obju_FAXMTAL->setudfvalue("u_srcwhs",$objRs->fields["WHSCODE"]);
					$obju_FAXMTAL->setudfvalue("u_acquidate",$objTable->docdate);
					$obju_FAXMTAL->setudfvalue("u_amount",$objRs->fields["LINECOST"]);
					$obju_FAXMTAL->setudfvalue("u_cost",$objRs->fields["LINECOST"]);
					$obju_FAXMTAL->setudfvalue("u_profitcenter",$objRs->fields["PROFITCENTER"]);
					$obju_FAXMTAL->setudfvalue("u_projcode",$objRs->fields["PROJCODE"]);
					$obju_FAXMTAL->setudfvalue("u_contraglacctno",$objRs->fields["GLACCTNO"]);
					$obju_FAXMTAL->setudfvalue("u_property1",$objRs->fields["U_FAPROPERTY1"]);
	...
	function onCustomEventmarketingdocumentsFAGoodsIssueGPSFixedAsset($objTable) {
		...
		if ($objTable->trxtype=="") {
			if ($actionReturn) {
				//$objRs->setdebug();
				$objRs->queryopen("SELECT A.ITEMCODE, A.ITEMDESC, A.WHSCODE, A.LINEID, A.QUANTITY, A.ITEMCOST, A.LINECOST, A.DRCODE AS PROFITCENTER, A.PROJCODE, A.U_FAPROPERTY1, A.U_FAPROPERTY2, A.U_FAPROPERTY3, A.U_FAPROPERTY4, A.U_FAPROPERTY5, A.GLACCTNO, A.U_FACAPITALIZEBY FROM GOODSISSUEITEMS A WHERE A.BRANCH = '".$_SESSION["branch"]."' AND A.COMPANY = '".$_SESSION["company"]."' AND A.ITEMMANAGEBY='0' AND A.U_FACAPITALIZE='Y' AND A.DOCID='$objTable->docid'");
				//var_dump($objRs->sqls);
				while ($objRs->queryfetchrow("NAME")) {
					if ($objRs->fields["U_FACAPITALIZEBY"]=="Unit") {
						for ($ctr=1;$ctr<=$objRs->fields["QUANTITY"];$ctr++) {
