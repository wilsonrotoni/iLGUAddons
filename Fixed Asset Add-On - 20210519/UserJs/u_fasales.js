...
function onPageSubmitGPSFixedAsset(action) {
	if (action=="a" || action=="sc") {
		...
		if (isInputNegative("u_price")) return false;
		if (isInputEmpty("u_taxcode")) return false;
		...
		default:
			switch(column) {
				case "u_facode":
					if (getInput(column)!="") {
						...
						if (result.getAttribute("result")!= "-1") {
							if (parseInt(result.getAttribute("result"))>0) {
								...
								setInputAmount("u_bookvalue",getInputNumeric("u_cost") - getInputNumeric("u_accumdepre"));	
					...
					setInputAmount("u_gainloss",getInputNumeric("u_price")-(getInputNumeric("u_cost")-getInputNumeric("u_accumdepre")));
					break;
				case "u_bpcode":
					if (getInput(column) != "") { 
						...
						if (result.getAttribute("result") == '0') {
							setInput("u_bpname","");
							setStatusMsg('Invalid Customer!');
							return false;
						}	
						setInput("u_bpcode",result.childNodes.item(0).getAttribute("bpcode"));
						setInput("u_bpname",result.childNodes.item(0).getAttribute("bpname"));
					} else {
						setInput("u_bpname","");
					}
					break;
