...
//			}	
                        ...
                        $total=0;	
                        $firefee=0;	
//                        $objRs->queryopen("SELECT F.u_regulatory,B.u_taxbase,SUBSTRING(A.U_REMARKS,-4,4) AS u_year ,B.u_itemcode,B.u_itemdesc,SUM(B.U_AMOUNT - B.U_SETTLEDAMOUNT) AS u_amount,B.u_businessline,B.u_seqno FROM U_LGUBILLS A
//                                            INNER JOIN U_LGUBILLITEMS B ON A.DOCID = B.DOCID AND A.BRANCH = B.BRANCH AND A.COMPANY = B.COMPANY AND B.U_AMOUNT <> B.U_SETTLEDAMOUNT
//                                            LEFT JOIN U_BPLLINES L ON B.U_BUSINESSLINE = L.CODE
//                                            LEFT JOIN U_BPLTAXCLASSNATURE N ON L.U_BUSINESSNATURE = N.U_BUSINESSNATURE
//                                            LEFT JOIN U_BPLFEES F ON F.CODE = B.U_ITEMCODE
//                                            WHERE a.company='$obju_BPLApps->company' and a.branch='$obju_BPLApps->branch' and a.u_custno ='".$obju_BPLApps->getudfvalue("u_appno")."' and a.u_dueamount > 0 AND U_PAYMODE <> 'A'
//                                            GROUP BY SUBSTRING(A.U_REMARKS,-4,4),B.U_ITEMCODE,B.U_BUSINESSLINE");
//                        while ($objRs->queryfetchrow("NAME")) {
//                                $objGrids[4]->addrow();
//				$objGrids[4]->setitem(null,"u_year",$objRs->fields["u_year"]);
//				$objGrids[4]->setitem(null,"u_feecode",$objRs->fields["u_itemcode"]);
//				$objGrids[4]->setitem(null,"u_feedesc",$objRs->fields["u_itemdesc"]);
...
//                        }
//                        
//                        $objRs->queryopen("SELECT SUBSTRING(A.U_REMARKS,-4,4) AS u_year,B.u_businessline,N.CODE AS u_taxclass,B.u_taxbase,SUM(B.U_AMOUNT - B.U_SETTLEDAMOUNT) AS u_btaxlinetotal FROM U_LGUBILLS A
//                                            INNER JOIN U_LGUBILLITEMS B ON A.DOCID = B.DOCID AND A.BRANCH = B.BRANCH AND A.COMPANY = B.COMPANY AND B.U_AMOUNT <> B.U_SETTLEDAMOUNT AND B.U_ITEMCODE = '0001'
//                                            INNER JOIN U_BPLLINES L ON B.U_BUSINESSLINE = L.CODE
//                                            INNER JOIN U_BPLTAXCLASSNATURE N ON L.U_BUSINESSNATURE = N.U_BUSINESSNATURE 
//                                            where a.company='$obju_BPLApps->company' and a.branch='$obju_BPLApps->branch' and a.u_custno='".$obju_BPLApps->getudfvalue("u_appno")."' and a.u_dueamount > 0 AND U_PAYMODE <> 'A'
//                                            GROUP BY SUBSTRING(A.U_REMARKS,-4,4),B.U_ITEMCODE,B.U_BUSINESSLINE");
//                        while ($objRs->queryfetchrow("NAME")) {
//                                $page->setitem("billingexist",1);
//                                $objGrids[0]->addrow();
//                                $objGrids[0]->setitem(null,"u_year",$objRs->fields["u_year"]);
//                                $objGrids[0]->setitem(null,"u_businessline",$objRs->fields["u_businessline"]);
//                                $objGrids[0]->setitem(null,"u_unitcount",1);
//                                $objGrids[0]->setitem(null,"u_btaxlinetotal",formatNumericAmount($objRs->fields["u_btaxlinetotal"]));
...
//                                $objGrids[0]->setitem(null,"u_nonessential",formatNumericAmount($objRs->fields["u_taxbase"]));
